			...
			uploader.bind("Init", function(up) {
				...
				inputFile.onchange = function() {
					...
					if (dorado.Browser.msie && dorado.Browser.version == 10){
						var nf = this.cloneNode(true);
						nf.onchange = inputFile.onchange;
						this.parentNode.replaceChild(nf, this);
					}
					...
					if (up.features.triggerDialog) {
						plupload.addEvent(browseButton, 'click', function(e) {
							if (browseButton.disabled!==true){
								var inputFileEditor = document.getElementById(up.id + '_html5');
								if (inputFileEditor){
						...
						function prepareAndSend(bin) {
							...
							xhr.onreadystatechange = function() {
								...
								if (xhr.readyState == 4) {
									...
									// Is error status
									if (httpStatus >= 400) {
										var exception = dorado.uploader.Common.parseResponseException({
										...
										var exception = dorado.uploader.Common.parseResponseException(xhr.responseText);
										if(exception){
											file.status = plupload.FAILED;
											var result = up.trigger('Error', {
												code : plupload.GENERIC_ERROR,
												message : exception.message,
												file: file
											});
