...
(function() {
	dorado.widget = {};
	dorado.widget.ViewElement = $extend(
		[dorado.AttributeSupport, dorado.EventSupport],
		{
				...
				view: {
					...
					setter: function(view: any) {
						...
						if (this._innerViewElements) {
							this._innerViewElements.each(function(viewElement: any) {
				...
				if (id) {
					...
					if (!/^[a-zA-Z_$][a-z0-9A-Z_$]*$/.exec(id)) {
						throw new dorado.ResourceException(
							"dorado.widget.InvalidComponentId",
							id,
						);
					}
					this._id = id;
				...
				if (def) {
					if (this._ready && def.writeBeforeReady) {
						throw new dorado.AttributeException(
							"dorado.widget.AttributeWriteBeforeReady",
							attr,
						);
					}
					if (def.componentReference) {
						if (value instanceof dorado.widget.Component) {
							return dorado.AttributeSupport.prototype.doSet.call(
								this,
								attr,
								value,
							...
							if (value instanceof Array) {
								if (value.length > 0) {
									...
									for (let i = 0; i < value.length; i++) {
										component[i] =
											dorado.widget.ViewElement.getComponentReference(
												this,
												attr,
												value[i],
											);
								...
								component = dorado.widget.ViewElement.getComponentReference(
									this,
									attr,
									value,
								);
						...
						if (def.innerComponent != null) {
							if (value) {
								...
								if (value instanceof Array) {
									...
									for (let i = 0; i < value.length; i++) {
										components.push(
											this.createInnerComponent(value[i], defaultType),
										);
									}
									value = components;
								} else {
			...
			createInnerComponent: function(config: any, typeTranslator: any) {
				if (!config) {
					return null;
				}
				...
				if (typeof config === "object") {
					component = dorado.Toolkits.createInstance(
						"widget",
						config,
						typeTranslator,
					);
			...
			registerInnerViewElement: function(viewElement: any) {
				if (!this._innerViewElements) {
				...
				viewElement.set(
					"view",
					this instanceof dorado.widget.View ? this : this.get("view"),
				);
				if (viewElement.parentChanged) {
					viewElement.parentChanged();
				}
			},
			unregisterInnerViewElement: function(viewElement: any) {
				if (!this._innerViewElements) {
					return;
				}
	...
	dorado.widget.RenderableViewElement = $extend(
		[dorado.widget.ViewElement, dorado.RenderableElement],
		{
	...
	dorado.widget.ViewElement.getComponentReference = function (
		object,
		attr,
		value,
	) {
		...
		if (value instanceof dorado.widget.Component) {
			...
			if (!wantedComponents) {
				...
				view.bind(
					"onComponentRegistered._getComponentReference",
					viewOnComponentRegisteredListener,
				);
			}
	...
	function viewOnComponentRegisteredListener(view: any, arg: any) {
		let wantedComponents = view._wantedComponents;
	...
	dorado.widget.ViewElement.findParentViewElement = function(element: any, type: any) {
		function find(win: any, dom: any, className: any) {
			let control = null;
			do {
				let viewElement;
	...
	dorado.Toolkits.registerTypeTranslator("widget", function(type: any) {
...
(function() {
	dorado.widget.DataSet = $extend(dorado.widget.Component, {
		...
		constructor: function(configs: any) {
			this._dataPathCache = {};
			this._observers = [];
			...
			if (dataType) {
				if (data != null) {
					if (
						...
						data = dorado.DataUtil.convert(
							data,
							this.getDataTypeRepository(),
							dataType,
						);
				...
				if (data) {
					if (data.dataType == null) {
						...
						if (dataType !== data.dataType) {
							...
							if (mismatch) {
								throw new dorado.ResourceException(
									"dorado.widget.DataTypeNotAccording",
									this._id,
								);
							}
						}
			...
			if (dataType instanceof dorado.AggregationDataType) {
				...
				if (dataType instanceof dorado.EntityDataType) {
					if (this._data == null) {
						...
						throw new dorado.ResourceException(
							"dorado.widget.DataSetNotEmptyException",
							this._id,
						);
					}
				} else {
					if (dataType) {
						throw new dorado.ResourceException(
							"dorado.widget.DataSetNotSupportInsert",
							this._id,
						);
					} else {
		...
		doLoad: function(callback: any, flush: any) {
			let data = this._data,
			...
			if (data instanceof dorado.DataPipe) {
				...
				if (callback) {
					...
					if (pipe.runningProcNum > 0 && !shouldAbortAsyncProcedures) {
						throw new dorado.ResourceException(
							"dorado.widget.GetDataDuringLoading",
							this._id,
						);
					}
					try {
		...
		loadAsync: function(callback: any) {
			this.doLoad(callback || dorado._NULL_FUNCTION);
		},
		doGetData: function(path: any, options: any, callback: any) {
			...
			function evaluatePath(path: any, options: any, callback: any) {
				let data = this._data;
				if (data instanceof dorado.DataPipe) {
					return null;
				}
		...
		getData: function(path: any, options: any) {
			options = options || {};
		...
		getDataAsync: function(path: any, callback: any, options: any) {
			options = options || {};
			if (options.firstResultOnly == null) {
				options.firstResultOnly = true;
			}
		...
		queryData: function(path: any, options: any) {
			return this.doGetData(path, options);
		},
		queryDataAsync: function(path: any, callback: any, options: any) {
			this.doGetData(path, options, callback || dorado._NULL_FUNCTION);
		},
		flush: function() {
			this.getData(null, { flush: true, loadMode: "always" });
		},
		flushAsync: function(options: any) {
			...
			let self = this,
				taskId;
			if (modal) {
				taskId = dorado.util.TaskIndicator.showTaskIndicator(
					executingMessage ||
					$resource("dorado.data.DataProviderTaskIndicator"),
					"main",
				);
			}
			try {
		...
		getDataType: function(path: any, options: any) {
			let loadMode;
			if (typeof options === "string") {
				loadMode = options;
			} else {
		...
		retrievePreloadConfig: function(observer: any) {
			if (
				dorado.widget.DataTree &&
				dorado.Object.isInstanceOf(observer, dorado.widget.DataTree)
			) {
				let bindingConfigs = observer.get("bindingConfigs");
				if (bindingConfigs) {
					let preloadConfigsMap = this._preloadConfigsMap,
					...
					for (let i = 0; i < bindingConfigs.length; i++) {
						let configs = dorado.widget.DataTree.bindingConfigToPreloadConfig(
							bindingConfigs[i],
						);
						if (configs) {
		...
		entityMessageReceived: function(messageCode: any, args: any) {
			this._dataPathCache = {};
			if (this._ready) {
		...
		sendMessage: function(messageCode: any, args: any) {
		...
		post: function() {
			...
			for (let i = 0, len = observers.length; i < len; i++) {
				let observer = observers[i];
				if (
					dorado.Object.isInstanceOf(observer, dorado.widget.AbstractEditor)
				) {
	...
	dorado.DataSetDataPipe = $extend(dorado.DataProviderPipe, {
		...
		constructor: function(dataSet: any) {
	...
	DataSet.getOwnerDataSet = function(data: any) {
...
dorado.widget.DataControl = $extend(dorado.widget.DataSetObserver, {
	...
	getBindingDataType: function(options: any) {
		if (!options) {
			options = {};
		}
		if (options.loadMode == null) {
...
dorado.widget.PropertyDataControl = $extend(dorado.widget.DataControl, {
	...
	getBindingPropertyDef: function() {
		...
		let entityDataType = this.getBindingDataType(),
			pd;
		if (entityDataType) {
			let properties = this._property.split("."),
				i = 0;
...
(function() {
	...
	dorado.widget.Control = $extend(
		...
		],
		{
			...
			ATTRIBUTES: {
				...
				layoutConstraint: {
					setter: function(layoutConstraint: any) {
						if (
			...
			constructor: function(config: any) {
				this._actualVisible = !dorado.Object.isInstanceOf(
					this,
					dorado.widget.FloatControl,
				);
			...
			isLazyInit: function(config: any) {
				let lazyInit = config && config.$lazyInit,
					isFloatControl;
				if (lazyInit === undefined) {
					isFloatControl = dorado.Object.isInstanceOf(
						this,
						dorado.widget.FloatControl,
					);
			...
			onReady: function() {
				...
				if (this._innerControls) {
					...
					for (let i = 0, len = controls.length; i < len; i++) {
						control = controls[i];
						if (
							!(control instanceof dorado.widget.Control) &&
							!control._ready
						) {
							control.onReady();
						}
			...
			doSet: function(attr: any, value: any, skipUnknownAttribute: any, lockWritingTimes: any) {
				...
				dorado.widget.Component.prototype.doSet.call(
					this,
					attr,
					value,
					skipUnknownAttribute,
			...
			setActualVisible: function(actualVisible: any) {
			...
			onActualVisibleChange: function() {
				function notifyChildren(control: any, parentActualVisible: any) {
					if (control._innerControls) {
						let controls = control._innerControls,
							child;
						for (let i = 0, len = controls.length; i < len; i++) {
							child = controls[i];
							if (
				...
				if (delay) {
					dorado.Toolkits.setDelayedAction(
						...
						function() {
							...
							this._shouldRefreshOnVisible = false;
							let dom = this.getDom(),
								arg = { dom: dom, processDefault: true };
			...
			updateModernScroller: function(delay: any) {
				if (!this._modernScroller) {
					return;
				}
				if (delay) {
			...
			doResetDimension: function(force: any) {
				return dorado.RenderableElement.prototype.resetDimension.call(
					this,
					force,
				);
			},
			...
			notifySizeChange: function(delay: any, force: any) {
			...
			getDom: function() {
				...
				if (!this._dom) {
					let dom = (this._dom = this.createDom()),
						$dom = $fly(this._dom);
					if (!dom.id) {
					...
					let self = this;
					$dom.mousedown(function(evt: any) {
						if (!self._eventBinded) {
							...
							jQuery(this)
								...
								.bind("dblclick", function(evt: any) {
									if (!self.processDefaultMouseListener()) {
										return;
									}
									let defaultReturnValue;
								...
								.bind("contextmenu", function(evt: any) {
			...
			registerInnerControl: function(control: any) {
				if (!this._innerControls) {
					this._innerControls = [];
				}
			...
			unregisterInnerControl: function(control: any) {
				if (!this._innerControls) {
					return;
				}
			...
			scrollIntoView: function() {
				function doScrollIntoView(container: any, dom: any) {
	...
	dorado.widget.onControlGainedFocus = function(control: any) {
	...
	dorado.widget.setFocusedControl = function (
		control,
		ignorePhyscialFocus,
		skipGlobalBoardcast,
	) {
						...
						if (parentFrames) {
							parentFrames.each(function() {
								if (this.contentWindow === win) {
									let frameControl =
										win.parent.dorado.widget.Control.findParentControl(this);
									win.parent.dorado.widget.setFocusedControl(
										frameControl,
										true,
										true,
									);
	...
	dorado.widget.findFocusableControlInElement = function(element: any, reverse: any) {
		function findInChildren(element: any) {
			let el = reverse ? element.lastChild : element.firstChild,
				control = null;
			while (el) {
	...
	function findFocusableControl(control: any, options: any) {
		let focusableControls,
			subControls = control.getFocusableSubControls();
	...
	function findPrevious(from: any) {
		let control = null,
			parent = from._focusParent || from._parent;
		while (parent) {
	...
	dorado.widget.Control.findParentControl = function(element: any, type: any) {
		function find(win: any, dom: any, className: any) {
			let control = null;
			do {
				let control;
...
dorado._queueObject = {};
dorado.queue = function(namespace: any, fn: any) {
	if (!namespace) {
		return;
	}
...
dorado.dequeue = function(namespace: any) {
	if (!namespace) {
		return true;
	}
	let queue = dorado._queueObject[namespace];
...
(function() {
	...
	dorado.doOnBodyClick = function(fn: any) {
		queue.push(fn);
	};
	let layerModalPool = new dorado.util.ObjectPool({
		...
		passivateObject: function(dom: any) {
	...
	dorado.widget.FloatControl = $class({
		...
		show: function(options: any) {
			...
			dorado.queue(control._uniqueId + SHOWHIDE_SUFFIX, function() {
				...
				if (!control._rendered) {
					...
					let oldVisible = control._visible,
		...
		doShow: function(options: any) {
			...
			if (handleModal && control._modal) {
				dorado.ModalManager.show(
					dom,
					dorado.widget.FloatControl.modalTypeClassName[control._modalType],
				);
			}
			let animateType =
				options.animateType || control._showAnimateType || control._animateType;
		...
		doAfterShow: function() {
			let control = this,
				dom = control.getDom();
			if (
				dorado.widget.FloatControl.VISIBLE_FLOAT_CONTROLS.indexOf(control) < 0
			) {
			...
			if (dom) {
				...
				let continuedFocus =
					control._continuedFocus === undefined
						? control._modal
		...
		getShowPosition: function(options: any) {
			...
			if (anchorTarget && (options.align || options.vAlign)) {
				if (anchorTarget instanceof dorado.widget.Control) {
					...
					if (
						dorado.Object.isInstanceOf(anchorTarget, dorado.RenderableElement)
					) {
						fixedElement = anchorTarget._dom;
					} else {
	...
	let slideShow = function(options: any, safe: any) {
		let control = this,
			align = options.align,
	...
	let slideHide = function(options: any, safe: any) {
		let control = this,
			dom = control._dom,
...
(function() {
	...
	dorado.widget.Container = $extend(dorado.widget.Control, {
		...
		ATTRIBUTES: {
			...
			layout: {
				setter: function(layout: any) {
					...
					if (oldLayout) {
						...
						controls = [];
						oldLayout._regions.each(function(region: any) {
					...
					if (layout && !(layout instanceof dorado.widget.layout.Layout)) {
						layout = dorado.Toolkits.createInstance(
							"layout",
							layout,
							function(type: any) {
								type = type || "Dock";
								return dorado.util.Common.getClassType(
									"dorado.widget.layout." + type + "Layout",
									true,
								);
							},
						);
					...
					if (layout) {
						...
						if (controls && controls.length) {
							...
							controls.each(function(control: any) {
			...
			view: {
				setter: function(view: any) {
					...
					let children = container._children,
						child;
					for (let i = 0, len = children.length; i < len; i++) {
		...
		addChild: function(component: any) {
			...
			if (component instanceof dorado.widget.Control) {
				...
				if (layout) {
					if (
						...
						let shouldFireOnAttach =
							this._attached && layout._regions.size === 0 && !layout._rendered;
		...
		removeChild: function(component: any) {
		...
		setContentContainerVisible: function(visible: any) {
			let children = this._children,
				child;
			for (let i = 0, len = children.length; i < len; i++) {
				child = children[i];
		...
		doOnAttachToDocument: function() {
			...
			let layout = container._layout;
			if (
				container._contentContainerVisible &&
				layout &&
				!(layout._regions.size === 0 && !layout._rendered)
			) {
...
dorado.widget.HtmlContainer = $extend(dorado.widget.Container, {
	...
	applyContent: function(dom: any, content: any) {
		let layoutDom;
	...
	pushHtmlElement: function(doms: any, content: any) {
		function doPush(doms: any, content: any, context: any) {
			if (!content) {
				return;
			}
	...
	doOnFocus: function() {},
	getSubDom: function(contextKey: any) {
		this.getDom();
...
(function() {
	...
	dorado.widget.View = $extend(dorado.widget.Container, {
		...
		ATTRIBUTES: {
			...
			view: {
				setter: function(view: any) {
					dorado.widget.Component.prototype.ATTRIBUTES.view.setter.call(
						this,
						view,
					);
				},
			...
			context: {
				...
				setter: function(context: any) {
					this._context = context == null ? null : $map(context);
				},
			},
			children: {
				setter: function(children: any, attr: any) {
		...
		constructor: function(configs: any) {
			...
			if (configs === "$TOP_VIEW") {
				...
				this._dataTypeRepository = new dorado.DataTypeRepository(
					dorado.DataTypeRepository.ROOT,
				);
			}
		...
		loadData: function() {
			if (this._renderMode === "onDataLoaded") {
				let view = this,
					dataSets = this._loadingDataSets.slice();
		...
		onLoadData: function(dataSets: any) {
		...
		registerViewElement: function(id: any, comp: any) {
			if (!id) {
				return;
			}
			...
			if (old) {
				if (old !== comp) {
					throw new dorado.ResourceException(
						"dorado.widget.ComponentIdNotUnique",
						id,
						this._id,
					);
				} else {
		...
		unregisterViewElement: function(id: any) {
			if (!id) {
				return;
			}
		...
		registerComponent: function(id: any, comp: any) {
			this.registerViewElement(id, comp);
		},
		unregisterComponent: function(id: any) {
		...
		tag: function(tags: any, allowNull: any) {
			let group = dorado.TagManager.find(tags),
		...
		getComponentReference: function(id: any) {
			let comp = this.id(id);
			return comp || { view: this, component: id };
		},
		getDataType: function(name: any) {
			return this._dataTypeRepository.get(name);
		},
		getDataTypeAsync: function(name: any, callback: any) {
			return this._dataTypeRepository.getAsync(name, callback);
		},
		liveSet: function(attr: any, value: any, options: any) {
			function doLiveSet(attr: any, value: any, options: any) {
				let i = attr.indexOf(".");
				if (i > 0) {
		...
		liveBind: function(name: any, listener: any, options: any) {
			if (options) {
		...
		render: function(containerElement: any) {
		...
		bindByExpression: function(expression: any, listener: any, options: any) {
	...
	dorado.widget.View.registerDefaultComponent = function(id: any, component: any) {
		let comps = (this.DEFAULT_COMPONENTS = this.DEFAULT_COMPONENTS || {});
		comps[id] = component;
	};
	dorado.widget.View.getDefaultComponent = function(view: any, id: any) {
	...
	window.$waitFor = dorado.widget.View.waitFor = function(tasks: any, callback: any) {
	...
	let resizeCallbacks = [];
	dorado.bindResize = function(callback: any) {
		if (typeof callback !== "function") {
			return;
		}
		resizeCallbacks.push(callback);
	};
	dorado.fireResizeCallback = function(keyboardVisible: any) {
		for (let i = 0, j = resizeCallbacks.length; i < j; i++) {
			let callback = resizeCallbacks[i];
			callback.call(null, keyboardVisible);
		}
	};
	dorado.unbindResize = function(callback: any) {
		if (typeof callback !== "function") {
			return;
		}
	...
	jQuery().ready(function() {
		function getControlByElement(el: any) {
			let node = $DomUtils.findParent(el, function(node: any) {
		...
		$fly(document).mousedown(function(evt: any) {
			let element = evt.target;
			if (!element || !element.style || element.style.tabIndex < 0) {
				return;
			}
			let nodeName = element.nodeName.toLowerCase();
			let ignorePhyscialFocus =
		...
		$fly(document)
			.keydown(function(evt: any) {
				let b,
					c = dorado.widget.getFocusedControl();
				if (c) {
					b = c.onKeyDown(evt);
				}
				if (
					(dorado.widget.HtmlContainer &&
		...
		if (!dorado.Browser.isTouch) {
			$fly(document.body).focusin(function(evt: any) {
		...
		let resizeTopView = function() {
			...
			topView.onResizeTimerId = setTimeout(function() {
				...
				delete topView.onResizeTimerId;
				topView._children.each(function(child: any) {
					if (
		...
		let doInitDorado = function() {
			...
			window.onresize = function() {
				...
				if (dorado.Browser.isTouch) {
					let width = $fly(window).width(),
						height = $fly(window).height();
					if (
						(oldWidth === undefined && oldHeight === undefined) ||
			...
			if (dorado.Browser.isTouch) {
				...
				$fly(document).bind("touchmove", function(event: any) {
		...
		let rootViewport = {
			init: function(fn: any, scope: any) {
				let me = this,
...
(function() {
	...
	dorado.widget.Controller = {
		registerFunctions: function(view: any, configs: any) {
			function doRegister(view: any, configs: any) {
				for (let i = 0; i < configs.length; i++) {
					...
					if (config.global) {
						if (window[name] !== undefined) {
							throw new dorado.Exception(
								'A gloal function or variable named "' +
								name +
								'" is already exists.',
							);
						}
...
dorado.widget.SubViewHolder = $extend(dorado.widget.Control, {
	...
	getAjaxEngine: function() {
		...
		if (!ajax) {
			ajax = dorado.util.AjaxEngine.getInstance(
				$setting["ajax.loadViewOptions"],
			);
		}
		return ajax;
	},
...
dorado.widget.layout.Layout = $extend(dorado.AttributeSupport, {
	...
	constructor: function(config: any) {
		this._regions = new dorado.util.KeyedArray(function(region: any) {
	...
	getRegionDom: function(region: any) {
		if (region) {
			return this._domCache[region.id];
		}
	},
	ensureControlInited: function(control: any, region: any) {
		if (control._lazyInit) {
			control._lazyInit();
			region.constraint = this.preprocessLayoutConstraint(
				control._layoutConstraint,
				control,
			);
		}
	},
	...
	onAttachToDocument: function(containerElement: any) {
		if (!this._attached) {
	...
	getPreviousRegion: function(region: any) {
		let regions = this._regions.items;
		let i = regions.indexOf(region);
		for (i--; i >= 0; i--) {
			region = regions[i];
			if (
				region.constraint !== dorado.widget.layout.Layout.NONE_LAYOUT_CONSTRAINT
			) {
				return region;
			}
		}
	...
	getNextRegion: function(region: any) {
		let regions = this._regions.items;
		let i = regions.indexOf(region),
			len = regions.length;
		for (i++; i < len; i++) {
	...
	addControl: function(control: any) {
		let region = {
			id: dorado.Core.newId(),
			control: control,
			constraint: this.preprocessLayoutConstraint(
				control._layoutConstraint,
				control,
			),
		};
	...
	resetControlDimension: function(region: any, regionDom: any, autoWidth: any, autoHeight: any) {
		...
		let oldWidth = control._currentWidth,
	...
	getRegion: function(control: any) {
		return this._regions.get(control._uniqueId);
	},
	refreshControl: function(control: any) {
		...
		if (region) {
			...
			if (container.isActualVisible()) {
				...
				if (this.doRefreshRegion) {
					let currentWidth = dom.offsetWidth,
	...
	onControlSizeChange: function(control: any, delay: any, force: any) {
		...
		dorado.Toolkits.setDelayedAction(
			...
			function() {
				let containerDom = this._dom.parentNode,
					overflowedDoms = this.overflowedDoms;
