(function () {
	...
	dorado.widget.blockview.ImageBlockRenderer = $extend(
		dorado.widget.blockview.DefaultBlockRenderer,
		{
			...
			render: function (dom, arg) {
				var imageDom = this.getImageDom(dom),
					entity = arg.data;
				var labelHeight = 0;
	...
	dorado.widget.AbstractBlockView = $extend(dorado.widget.ViewPortList, {
		...
		createDom: function () {
			...
			if (dorado.Browser.isTouch || $setting["common.simulateTouch"]) {
				...
				$scroller
					.mouseover(
						$scopify(blockView, function (evt) {
		...
		refreshContent: function (container) {
			...
			if (this._scrollMode === "lazyRender") {
				var count = this._getVisibleBlockRange()[1],
					i = 0;
				fn = function (row) {
					i++;
					return i < count;
		...
		refreshItemDoms: function (itemDomContainer) {
			var currentDecorator = this._currentDecorator,
		...
		refreshItemDomData: function (blockDom, item) {
			...
			var eventArg = { dom: blockDom, data: item, processDefault: false };
			if (
		...
		onScroll: function (event, arg) {
			...
			if (blockView._scrollMode === "viewport") {
				...
				if (blockView._scrollMode === "lazyRender") {
					var range = blockView._getVisibleBlockRange(),
		...
		onMouseDown: function (evt) {
			...
			if (blockDom || this._allowNoCurrent) {
				...
				if (this.setCurrentItemDom(blockDom)) {
					var clickedItem = blockDom ? $fly(blockDom).data("item") : null,
		...
		showDraggingInsertIndicator: function (draggingInfo, insertMode, blockDom) {
			var insertIndicator = dorado.widget.blockview.getDraggingInsertIndicator(
				this._blockLayout,
			);
