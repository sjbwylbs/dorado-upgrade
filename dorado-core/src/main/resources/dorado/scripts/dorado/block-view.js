...
(function () {
    ...
    dorado.widget.blockview.BlockItemModel = $extend(dorado.widget.list.ItemModel, {
        _lineSize: 1,
        ...
        setScrollPos: function (scrollPos) {
            let itemCount = this.getItemCount(), lineCount = this.getLineCount();
            if (lineCount > 0) {
                this._startIndex =
    ...
    dorado.widget.blockview.ImageBlockRenderer = $extend(dorado.widget.blockview.DefaultBlockRenderer, {
        ...
        render: function (dom, arg) {
            let imageDom = this.getImageDom(dom), entity = arg.data;
            let labelHeight = 0;
            if (this.captionProperty && this.labelHeight > 0) {
                labelHeight = this.labelHeight;
                let labelDom = this.getLabelDom(dom);
                let label = entity instanceof dorado.Entity
            ...
            if (this.tipProperty && dorado.TipManager) {
                let tip = entity instanceof dorado.Entity
            ...
            let maxHeight = dom.clientHeight - labelHeight - this.spacing - this.padding * 2;
            if (this.stretchImage) {
                ...
                $fly(imageDom).bind("load", function () {
                    let left, top, width = imageDom.offsetWidth, height = imageDom.offsetHeight;
                    if (width > maxWidth) {
            ...
            let image = entity instanceof dorado.Entity
                ? entity.get(this.imageProperty)
                : entity[this.imageProperty];
            $DomUtils.setImgSrc(imageDom, image || this.blankImage);
        },
    });
    dorado.widget.blockview.TemplateBlockRenderer = $extend(dorado.widget.blockview.DefaultBlockRenderer, {
        template: "Template",
        constructor: function (options) {
            dorado.Object.apply(this, options);
        },
        render: function (dom, arg) {
            let item = arg.data, html = "", context = { control: arg.control, dom: dom, item: item };
            if (item != null) {
    ...
    dorado.widget.AbstractBlockView = $extend(dorado.widget.ViewPortList, {
        ...
        createDom: function () {
            ...
            if (dorado.Browser.isTouch || $setting["common.simulateTouch"]) {
                ...
                $scroller
                    .mouseover($scopify(blockView, function (evt) {
                    if ($DomUtils.isDragging()) {
                        return;
                    }
                    let blockDom = $DomUtils.findParent(evt.target, function (node) {
        ...
        refreshDom: function (dom) {
            ...
            let lineSize = this._lineSize, blockWidth = this._blockWidth, blockHeight = this._blockHeight;
            if (this._blockLayout === "vertical") {
                if (!(lineSize > 0)) {
                    lineSize =
            ...
            let width, height, container = this._container, $container = jQuery(container);
        ...
        _getVisibleBlockRange: function () {
            ...
            if (this._blockLayout === "vertical") {
                ...
                start = parseInt((scroller.scrollLeft - this._horiPadding) / blockSize);
                    ...
                    start +
                        Math.round((scroller.clientWidth - this._horiPadding) / blockSize + 0.5);
            }
        ...
        refreshContent: function (container) {
            ...
            if (this._scrollMode === "lazyRender") {
                let count = this._getVisibleBlockRange()[1], i = 0;
                fn = function (row) {
                    i++;
                    return i < count;
                };
        ...
        refreshViewPortContent: function (container) {
            let itemModel = this._itemModel, itemCount = itemModel.getItemCount(), scroller = this._scroller;
            ...
            if (bottomOrRight > topOrLeft) {
                itemDomCount = this.refreshItemDoms(container, itemModel.isReverse(), function (itemDom) {
                    if (itemDom.subIndex === self._realLineSize - 1) {
                        return ((self._blockLayout === "vertical"
        ...
        refreshItemDoms: function (itemDomContainer) {
            let currentDecorator = this._currentDecorator, hoverDecorator = this._hoverDecorator;
            if (currentDecorator) {
            ...
            $invokeSuper.call(this, arguments);
            if (hoverDecorator) {
                itemDomContainer.insertBefore(hoverDecorator, itemDomContainer.firstChild);
            }
            if (currentDecorator) {
                itemDomContainer.insertBefore(currentDecorator, itemDomContainer.firstChild);
            }
        ...
        refreshItemDomData: function (blockDom, item) {
            ...
            if (processDefault) {
                ...
                if (this._ignoreItemTimestamp ||
                    timestamp <= 0 ||
                    blockDom.timestamp !== timestamp) {
                    (this._renderer ||
                        $singleton(dorado.widget.blockview.DefaultBlockRenderer)).render(blockDom, { blockView: this, data: item });
                    blockDom.timestamp = timestamp;
                }
            }
        },
        ...
        onScroll: function (event, arg) {
            ...
            if (blockView._scrollMode === "viewport") {
                ...
                if (blockView._scrollMode === "lazyRender") {
                    let range = blockView._getVisibleBlockRange(), childNodes = blockView._container.childNodes;
        ...
        onMouseDown: function (evt) {
            ...
            if (blockDom || this._allowNoCurrent) {
                ...
                if (this.setCurrentItemDom(blockDom)) {
                    let clickedItem = blockDom ? $fly(blockDom).data("item") : null, selection = this.getSelection();
        ...
        scrollCurrentIntoView: function () {
            ...
            let itemDom = this._itemDomMap[currentItemId], scroller = this._scroller, itemIndex;
            if (itemDom) {
                ...
                if (itemIndex >= this.startIndex &&
                    ...
                    if (this._blockLayout === "vertical") {
                        if (itemDom.offsetTop < scroller.scrollTop) {
                            ...
                            if (itemDom.offsetTop + itemDom.offsetHeight >
                        ...
                        if (itemDom.offsetLeft < scroller.scrollLeft) {
                            ...
                            if (itemDom.offsetLeft + itemDom.offsetWidth >
        ...
        _getContainerSize: function () {
            let width = -1, height = -1, result;
            let lineCount = this._itemModel.getLineCount();
            let dom = this._dom, contrainer = this._container, hasScroller;
            let blockWidth = this._blockWidth, blockHeight = this._blockHeight;
            if (this._blockLayout === "vertical") {
        ...
        findItemDomByEvent: function (evt) {
            ...
            let target = target || evt, container = this._container;
            return $DomUtils.findParent(target, function (parentNode) {
                return parentNode.parentNode === container;
            });
        },
        findItemDomByPosition: function (pos) {
            let dom = this._dom, x = pos.x + dom.scrollLeft, y = pos.y + dom.scrollTop;
            let xIndex = parseInt((x - this._horiPadding - this._horiSpacing / 2) /
                (this._realBlockWidth + this._horiSpacing));
            let yIndex = parseInt((y - this._vertPadding - this._vertSpacing / 2) /
        ...
        showDraggingInsertIndicator: function (draggingInfo, insertMode, blockDom) {
            let insertIndicator = dorado.widget.blockview.getDraggingInsertIndicator(this._blockLayout);
...
dorado.widget.DataBlockView = $extend([dorado.widget.AbstractBlockView, dorado.widget.DataControl], {
    ...
    getRealCurrentItemId: function () {
        let items = this._itemModel.getItems(), current = items ? items.current : null;
        return current ? this._itemModel.getItemId(current) : null;
    },
    setCurrentItemDom: function (blockDom) {
        let item = $fly(blockDom).data("item"), b = this.setCurrentEntity(item);
        if (b) {
    ...
    onEntityInserted: function (arg) {
        ...
        switch (mode) {
            case "begin":
                ...
                break;
            case "before":
                let refBlockDom = this._itemDomMap[this._itemModel.getItemId(refEntity)], index = refBlockDom.itemIndex;
