...
(function() {
	...
	dorado.widget.UpdateAction.alertException = function(e: any) {
		currentException = e;
	...
	function getExceptionDialog() {
		if (!exceptionDialog) {
			...
			exceptionDialog = new dorado.widget.Dialog({
				...
				afterShow: function(dialog: any) {
					let buttons = dialog._buttons,
						button;
					if (buttons) {
						button = buttons[0];
				...
				beforeShow: function(dialog: any) {
					getDetailLink().render(dialog._textDom);
					let dom = dialog._dom;
					let $dom = jQuery(dom),
						$contentDom = jQuery(contentDom);
					let contentWidth =
					...
					let dialogWidth = $dom.width(),
						panelWidth = $contentDom.width();
					...
					if (contentHeight) {
						let dialogHeight = $dom.height(),
							panelHeight = $contentDom.height();
						dialog._height = contentHeight + dialogHeight - panelHeight;
					}
					dialog.doOnResize();
				},
				onHide: function(options: any) {
	...
	function showDetailExceptionDialog(e: any) {
		function translateMessage(items: any, messages: any) {
		...
		let items = [],
			validateContext = e.validateContext;
		if (validateContext) {
	...
	function getDetailExceptionDialog() {
		if (!detailExceptionDialog) {
			let grid = new dorado.widget.Grid({
				...
				columns: [
					{
						...
						width: 36,
						align: "center",
						onRenderCell: function(self: any, arg: any) {
							let iconClass;
					...
					},
					{
						property: "text",
						caption: $resource(
							"dorado.baseWidget.SubmitValidationDetailMessage",
						),
						wrappable: true,
					},
					{
						property: "property",
						caption: $resource(
							"dorado.baseWidget.SubmitValidationDetailProperty",
						),
						width: 200,
						resizeable: true,
						onRenderCell: function(self: any, arg: any) {
							if (
								arg.data.propertyCaption &&
				...
				onDataRowDoubleClick: function(self: any, arg: any) {
...
(function() {
	let BUTTON_HOVER_CLASS = "-hover",
		BUTTON_CLICK_CLASS = "-click",
		BUTTON_TOGGLED_CLASS = "-toggled",
		BUTTON_TRIGGER_CLASS = "-trigger",
		ICON_CLASS = "d-icon";
	dorado.widget.Button = $extend(dorado.widget.AbstractButton, {
		...
		doSetToggle: function() {
			...
			if (dom) {
				if (button._toggled) {
					...
					$fly(doms.buttonRight).addClass(
						button.needSplit() ? "" : "button-right-toggled",
					);
				} else {
		...
		_createIconSpan: function(dom: any) {
			let button = this,
				doms = button._doms,
		...
		onClick: function() {
			let button = this,
				action = button._action || {},
				disabled = button._disabled || action._disabled || action._sysDisabled;
			if (!disabled) {
		...
		createDom: function() {
			...
			$fly(doms.buttonLeft)
				.hover(
					function() {
						let action = button._action || {},
							disabled =
								button._disabled || action._disabled || action._sysDisabled;
						if (!disabled && !button._toggled) {
							$fly(dom).addClass(cls + BUTTON_HOVER_CLASS);
							$fly(doms.buttonLeft).addClass("button-left-hover");
							$fly(doms.buttonRight).addClass(
								button.needSplit() ? "" : "button-right-hover",
							);
						}
					},
					function() {
				...
				.mousedown(function() {
					...
					if (!disabled) {
						...
						$fly(doms.buttonRight).addClass(
							button.needSplit() ? "" : "button-right-click",
						);
			...
			$fly(doms.buttonRight)
				.hover(
					function() {
						...
						if (!disabled) {
							$fly(dom).addClass(
								button.needSplit() ? "" : cls + BUTTON_HOVER_CLASS,
							);
							$fly(doms.buttonLeft).addClass(
								button.needSplit() ? "" : "button-left-hover",
							);
				...
				.mousedown(function() {
					...
					if (!disabled) {
						$fly(dom).addClass(
							button.needSplit() ? "" : cls + BUTTON_CLICK_CLASS,
						);
						$fly(doms.buttonLeft).addClass(
							button.needSplit() ? "" : "button-left-click",
						);
			...
			$fly(dom)
				.toggleClass(
					"d-button-trigger " + cls + BUTTON_TRIGGER_CLASS,
					button._showTrigger === true ||
			...
			let icon = button._icon || action._icon,
				iconCls = button._iconClass || action._iconClass;
			if (icon || iconCls) {
		...
		refreshDom: function(dom: any) {
			...
			if (button._toggleable) {
				...
				if (!button.needSplit()) {
					$fly(dom).toggleClass(
						cls + BUTTON_TOGGLED_CLASS,
						!!button._triggerToggled,
					);
					$fly(doms.buttonLeft).toggleClass(
						"button-left-toggled",
						!!button._triggerToggled,
					);
					$fly(doms.buttonRight).toggleClass(
						"button-right-toggled",
						!!button._triggerToggled,
					);
				} else {
					$fly(doms.buttonRight).toggleClass(
...
(function() {
	let MENU_ITEM_DISABLED_CLASS = "menu-item-disabled",
		CHECKED_ICON = "checked-icon",
		UN_CHECKED_ICON = "unchecked-icon",
		HAS_GROUP_CLASS = "has-subgroup",
		MENU_ITEM_OVER_CLASS = "menu-item-hover";
	dorado.widget.menu = {};
	dorado.widget.menu.AbstractMenuItem = $extend(
		dorado.widget.RenderableViewElement,
		{
			...
			getTopMenu: function() {
				let menu = this._parent,
					opener = menu.opener,
					result;
				while (opener) {
					let parent = opener._parent;
					if (
						opener instanceof dorado.widget.menu.AbstractMenuItem &&
	...
	dorado.widget.menu.TextMenuItem = $extend(
		[dorado.widget.menu.AbstractMenuItem, dorado.widget.ActionSupport],
		{
			...
			refreshDom: function(dom: any) {
				let item = this,
					action = item._action || {},
					disabled = item._disabled || action._disabled || action._sysDisabled,
					icon = item._icon || action._icon,
					iconCls = item._iconClass || action._iconClass,
					doms = item._doms;
				$fly(dom)
					[disabled ? "addClass" : "removeClass"](MENU_ITEM_DISABLED_CLASS)
	...
	dorado.widget.menu.ControlMenuItem = $extend(
		dorado.widget.menu.TextMenuItem,
		{
				...
				control: {
					setter: function(control: any) {
						...
						if (!(control instanceof dorado.widget.Control)) {
							control = dorado.Toolkits.createInstance(
								"widget",
								control,
								function(type: any) {
	...
	dorado.widget.menu.MenuItem = $extend(dorado.widget.menu.TextMenuItem, {
		...
		ATTRIBUTES: {
			submenu: {
				setter: function(value: any) {
					...
					let dom = this._dom;
					if (dom) {
						$fly(dom)[this._submenu ? "addClass" : "removeClass"](
							HAS_GROUP_CLASS,
						);
					}
				},
		...
		getItem: function(name: any) {
			let menuItem = this,
				submenu = menuItem._submenu;
			if (submenu) {
				return submenu.getItem(name);
			}
			return null;
		},
		addItem: function(item: any, index: any) {
			let menuItem = this,
				submenu = menuItem._submenu;
			if (item) {
				if (!submenu) {
		...
		onFocus: function(showSubmenu: any) {
			let item = this,
				dom = item._dom;
			$fly(dom).addClass(MENU_ITEM_OVER_CLASS);
		...
		showSubmenu: function(focusfirst: any) {
			let item = this,
				submenu = item._submenu;
			if (submenu) {
	...
	dorado.widget.menu.CheckGroupManager = {
		groups: {},
		addCheckItem: function(groupName: any, item: any) {
			if (!groupName || !item) {
				return;
			}
			let manager = this,
		...
		removeCheckItem: function(groupName: any, item: any) {
			if (!groupName || !item) {
				return;
			}
			let manager = this,
		...
		setGroupCurrent: function(groupName: any, item: any) {
			if (!groupName || !item) {
				return;
			}
			let manager = this,
	...
	dorado.widget.menu.CheckableMenuItem = $extend(dorado.widget.menu.MenuItem, {
		...
		ATTRIBUTES: {
			checked: {
				setter: function(value: any) {
					...
					if (value && item._group) {
						dorado.widget.menu.CheckGroupManager.setGroupCurrent(
							item._group,
							item,
						);
					}
			...
			group: {
				setter: function(value: any) {
					let item = this,
						oldValue = item._group;
					if (oldValue) {
						dorado.widget.menu.CheckGroupManager.removeCheckItem(
							oldValue,
							item,
						);
					}
		...
		createDom: function() {
			let item = this,
				dom = $invokeSuper.call(this, arguments);
			$fly(item._doms.icon).addClass(
				item._checked ? CHECKED_ICON : UN_CHECKED_ICON,
			);
...
(function() {
	let GROUP_CONTENT_CLASS = "group-content",
		GROUP_OVERFLOW_CLASS = "d-menu-overflow";
	let SKIP_REFRESH = false;
	dorado.widget.AbstractMenu = $extend(
		[dorado.widget.Control, dorado.widget.FloatControl],
		{
			...
			addItem: function(item: any, index: any) {
				...
				items = menu._items = items
					? menu._items
					: new dorado.util.KeyedArray(function(value: any) {
							return value._name;
						});
				if (
					item.constructor === Object.prototype.constructor ||
					typeof item === "string"
				) {
	...
	dorado.widget.Menu = $extend(dorado.widget.AbstractMenu, {
		...
		createMenuItem: function(config: any) {
			if (!config) {
				return null;
			}
			let menu = this,
		...
		handleOverflow: function(overflowHeight: any) {
			let menu = this,
				dom = menu._dom,
				doms = menu._doms;
			if (dom) {
				...
				let contentHeight =
						...
						$fly(doms.overflowBottomArrow).outerHeight(),
					contentWidth = $fly(dom).width();
				if (dorado.Browser.msie && dorado.Browser.version === 6) {
				...
				$fly([doms.overflowTopArrow, doms.overflowBottomArrow]).outerWidth(
					contentWidth,
				);
			}
		},
		...
		getFocusableItem: function(mode: any) {
			let menu = this,
				items = menu._items,
				focusItem = menu._focusItem,
				focusIndex = -1,
		...
		focusItem: function(item: any, showSubmenu: any) {
			let menu = this,
				focusItem = menu._focusItem;
			if (menu._freeze) {
				return;
			...
			if (item && focusItem !== item) {
				...
				if (menu._overflowing) {
					let itemDom = item._dom,
						doms = menu._doms,
						viewTop = $fly(doms.groupContent).prop("scrollTop"),
						contentTop = $fly(doms.groupContent).prop("offsetTop"),
						itemTop = itemDom.offsetTop,
						itemHeight = itemDom.offsetHeight,
						itemBottom = itemTop + itemHeight,
						contentHeight = $fly(doms.groupContent).innerHeight(),
					...
					if (itemTop < viewTop) {
						...
						if (itemBottom > viewBottom) {
							$fly(doms.groupContent).prop(
								"scrollTop",
								itemBottom - contentHeight,
							);
						}
					}
				}
		...
		doScrollUp: function() {
			let menu = this,
				doms = menu._doms,
				groupContent = doms.groupContent,
				st = $fly(groupContent).scrollTop(),
				target = st - 22;
			if (target >= 0) {
		...
		doScrollDown: function() {
			let menu = this,
				...
				st = $fly(groupContent).scrollTop(),
				target = st + 22,
				scrollHeight = $fly(groupContent).prop("scrollHeight");
			if (target <= scrollHeight) {
				$fly(groupContent).scrollTop(target);
			} else {
				$fly(groupContent).scrollTop(
					scrollHeight - $fly(groupContent).innerHeight(),
				);
			}
		},
		doOnBlur: function() {
		...
		createDom: function() {
			...
			$fly(groupContent).mousewheel(function(event: any, delta: any) {
				if (menu._overflowing) {
					if (delta < 0) {
						menu.doScrollDown();
					} else {
		...
		refreshDom: function(dom: any) {
			...
			if (visibleItemCount === 0) {
				if (!menu._noContentEl) {
					...
					menu._noContentEl.innerHTML = $resource(
						"dorado.baseWidget.MenuEmptyItem",
					);
		...
		getShowPosition: function(options: any) {
			let menu = this,
				...
				result;
			options = options || {};
			options.overflowHandler = function(options: any) {
		...
		doAfterShow: function(options: any) {
			let menu = this,
...
dorado.widget.Slider = $extend(dorado.widget.Control, {
	...
	ATTRIBUTES: {
			...
			setter: function(value: any) {
				...
				if (isNaN(parseValue)) {
					throw new dorado.ResourceException(
						"dorado.baseWidget.NumberFormatInvalid",
						value,
					);
				}
		...
		value: {
			setter: function(value: any) {
				let slider = this,
					minValue = slider._minValue,
					maxValue = slider._maxValue,
				...
				if (parseValue < minValue || parseValue > maxValue) {
					throw new dorado.ResourceException(
						"dorado.baseWidget.NumberRangeInvalid",
						minValue,
						maxValue,
						value,
					);
	...
	createDom: function() {
		let slider = this,
			dom,
			doms = {},
			orientation = slider._orientation;
		dom = $DomUtils.xCreate(
			{
				tagName: "div",
				content: [
		...
		jQuery(doms.thumb)
			...
			.draggable({
				...
				stop: function(event: any, ui: any) {
					...
					slider.set(
						"value",
						((maxValue - minValue) * offset) / size + minValue,
					);
					if (tip) {
						tip.hide();
					}
				...
				drag: function(event: any, ui: any) {
					...
					if (tip) {
						tip.set(
							"text",
							slider.getValidValue(((maxValue - minValue) * offset) / size) +
								minValue,
						);
						tip.refresh();
						if (!tip._dom) {
	...
	getValidValue: function(value: any) {
		function formatDecimal(value: any, precision: any) {
			if (precision == null) {
				precision = 0;
			}
		...
		if (value !== undefined) {
			...
			if (step != null) {
				let total = (maxValue - minValue) / step,
					left = Math.floor((result - minValue) / step),
					right = left + 1;
				if (right > total) {
					result = formatDecimal(maxValue, slider._precision);
				} else {
					if (
						Math.abs(minValue + step * right - result) >
	...
	onClick: function(event: any) {
		...
		slider.set(
			"value",
			slider._minValue + (slider._maxValue - slider._minValue) * percent,
		);
	},
	doOnResize: function() {
		if (!this._ready) {
	...
	refreshDom: function(dom: any) {
		...
		if (orientation === "vertical") {
			...
			$fly(doms.thumb).css(
				"top",
				((height - thumbSize) * (value - minValue)) / (maxValue - minValue),
			);
			$fly(doms.current).css(
				"height",
				((height - thumbSize) * (value - minValue)) / (maxValue - minValue),
			);
			if (handleIncrease) {
			...
			$fly(doms.thumb).css(
				"left",
				((width - thumbSize) * (value - minValue)) / (maxValue - minValue),
			);
			$fly(doms.current).css(
				"width",
				((width - thumbSize) * (value - minValue)) / (maxValue - minValue),
...
(function() {
	let TAB_CLOSEABLE_CLASS = "-closeable",
	...
	dorado.widget.tab.Tab = $extend(dorado.widget.RenderableViewElement, {
		...
		refreshDom: function(dom: any) {
			let tab = this,
				closeable = tab._closeable,
				disabled = tab._disabled,
				visible = tab._visible,
				doms = tab._doms,
				captionDom = doms.caption,
				closeEl = doms.close,
				width = tab._width,
				tabbar = tab._parent,
				tabMinWidth;
		...
		render: function(ctEl: any, index: any) {
			let dom = this.getDom();
			if (!dom) {
				return;
			}
		...
		createDom: function() {
				...
				dom = $DomUtils.xCreate(
					{
						tagName: "li",
						className:
							tab._className + (tab._exClassName ? " " + tab._exClassName : ""),
						content: [
							{
								tagName: "span",
								className: "tab-left",
			...
			jQuery(dom)
				...
				.bind("contextmenu", function(event: any) {
					event = jQuery.event.fix(event || window.event);
					let tabbar = tab._parent,
						arg = { tab: tab, event: event, processDefault: true };
		...
		createCloseDom: function(dom: any, doms: any) {
			let tab = this,
				closeEl = $DomUtils.xCreate(
					{ tagName: "span", className: "close", contextKey: "close" },
					null,
					doms,
				);
		...
		doSetParentViewElement: function(parentViewElement: any) {
...
(function() {
	let LEFT_BUTTON_CLASS = "left-button",
		RIGHT_BUTTON_CLASS = "right-button",
		MENU_BUTTON_CLASS = "menu-button",
		TOP_BUTTON_CLASS = "top-button",
		BOTTOM_BUTTON_CLASS = "bottom-button";
	dorado.widget.TabGroup = $extend(dorado.widget.Control, {
		...
		ATTRIBUTES: {
			...
			currentIndex: {
				...
				getter: function() {
					let tabgroup = this,
						tabs = tabgroup._tabs,
						currentTab = tabgroup._currentTab;
					if (currentTab) {
		...
		constructor: function() {
			this._tabs = new dorado.util.KeyedArray(function(value: any) {
				return value._name;
			});
			$invokeSuper.call(this, arguments);
		},
		...
		disableTab: function(tab: any) {
			let tabgroup = this,
				tabDom,
				navmenu = tabgroup._navmenu,
				tabs = tabgroup._tabs,
				index;
		...
		doSetTabVisible: function(tab: any, visible: any) {
			let tabgroup = this,
				tabDom,
				navmenu = tabgroup._navmenu,
				tabs = tabgroup._tabs,
		...
		addTab: function(tab: any, index: any, current: any) {
			if (!tab) {
		...
		doAddTab: function(tab: any, index: any, current: any) {
			let tabgroup = this,
				tabs = tabgroup._tabs,
				doms = tabgroup._doms,
				navmenu = tabgroup._navmenu;
		...
		getAvialableTab: function(tab: any) {
			let tabgroup = this,
				tabs = tabgroup._tabs,
				index,
				result,
		...
		closeTab: function(tab: any) {
			if (tab) {
				tab.close();
			}
		},
		closeOtherTabs: function(tab: any, force: any) {
			if (!tab) {
				return;
			}
			let tabgroup = this,
				tabs = tabgroup.get("tabs").toArray();
			jQuery.each(tabs, function(index: any, target: any) {
				if (
					target !== tab &&
		...
		closeAllTabs: function(force: any) {
			let tabgroup = this,
				tabs = tabgroup.get("tabs").toArray();
			jQuery.each(tabs, function(index: any, tab: any) {
				if (force || (!tab._disabled && tab._closeable)) {
					tab.close();
				}
			});
		},
		doFilterTabs: function(tabs: any) {
			let result = new dorado.util.KeyedArray(function(value: any) {
				return value._name;
			});
			for (let i = 0, j = tabs.size; i < j; i++) {
				let tab = tabs.get(i);
		...
		doChangeCurrentTab: function(tab: any, force: any) {
			let tabgroup = this,
				doms = tabgroup._doms || {},
				currentTab = tabgroup._currentTab;
		...
		doOnTabChange: function(eventArg: any) {
			let tabgroup = this;
			tabgroup.fireEvent("onTabChange", tabgroup, eventArg);
		},
		doChangeTabPlacement: function(value: any) {
			let tabgroup = this,
				cls = tabgroup._className,
				doms = tabgroup._doms,
				tabbarDom = doms.tabbar;
			if (tabbarDom) {
				let oldValue = tabgroup._tabPlacement;
				$fly(tabbarDom).addClass("d-tabbar-" + value + " " + cls + "-" + value);
				if (oldValue) {
					$fly(tabbarDom).removeClass(
						"d-tabbar-" + oldValue + " " + cls + "-" + oldValue,
					);
				}
			}
	...
	dorado.widget.TabColumn = $extend(dorado.widget.TabGroup, {
		...
		createDom: function() {
			...
			if (tabcolumn._verticalText) {
				...
				if (dorado.Browser.msie && dorado.Browser.version === 6) {
					jDom.addClass(
						tabcolumn._className + "-vtext-" + tabcolumn._tabPlacement,
					);
				}
			}
			jDom.addClass(
				tabcolumn._tabPlacement === "left"
			...
			let tabsEl = doms.tabs,
				currentTab = tabcolumn._currentTab;
			if (tabs) {
				for (let i = 0, j = tabs.size; i < j; i++) {
					let tab = tabs.get(i);
					if (tab._current) {
			...
			$fly(doms.tabsWrap).mousewheel(function(event: any, delta: any) {
		...
		doChangeTabPlacement: function(value: any) {
			...
			if (tabbarDom) {
				let oldValue = tabgroup._tabPlacement;
				$fly(tabbarDom).addClass(
					this._className + "-" + value + " " + cls + "-" + value,
				);
				if (oldValue) {
					$fly(tabbarDom).removeClass(
						this._className + "-" + oldValue + " " + cls + "-" + oldValue,
					);
				}
			}
			tabgroup._tabPlacement = value;
		...
		scrollTabIntoView: function(tab: any) {
			let tabbar = this,
				doms = tabbar._doms,
				tabDom = tab._dom,
				tabsEl = doms.tabs,
				offsetTop = tabDom.offsetTop,
				offsetHeight = tabDom.offsetHeight,
				top = (parseInt(tabsEl.style.top, 10) || 0) * -1,
				viewHeight = $fly(doms.tabsWrap).height();
		...
		refreshNavButtons: function() {
			let tabcolumn = this,
				dom = tabcolumn._dom,
				tabs = tabcolumn._tabs,
				doms = tabcolumn._doms;
			if (!dom || !tabs) {
				return;
			}
			let topButton = tabcolumn._topButton,
				bottomButton = tabcolumn._bottomButton;
			let tabsHeight = tabcolumn.getTabsHeight(),
				tabsEl = doms.tabs,
				currentTop = parseInt(tabsEl.style.top || 0, 10);
			let visibleHeight = $fly(doms.tabsWrap).height();
			if (tabsHeight > 0) {
				if (
					tabcolumn._alwaysShowNavButtons !== true &&
		...
		showNavButtons: function() {
			...
			if (dom) {
				...
				if (modifyTop) {
					let tabsEl = doms.tabs,
						top = parseInt(tabsEl.style.top, 10) || 0;
					$fly(tabsEl).top(top);
				}
				tabcolumn.onToolButtonVisibleChange();
			}
		},
		hideNavButtons: function(force: any) {
			let tabcolumn = this,
				dom = tabcolumn._dom,
				doms = tabcolumn._doms;
			if (!dom) {
			...
			let topButton = doms.topButton,
				bottomButton = doms.bottomButton;
		...
		createNavButtons: function(dom: any) {
			let tabcolumn = this;
			if (!dom) {
				return;
			}
			let doms = tabcolumn._doms,
				tabbarDom = doms.tabbar,
				topBtn,
				bottomBtn;
		...
		doScrollTop: function(anim: any, length: any) {
			let tabcolumn = this,
				doms = tabcolumn._doms,
				tabsEl = doms.tabs,
				to = parseInt(tabsEl.style.top, 10) + (length > 0 ? length : 100);
			if (anim) {
		...
		doScrollBottom: function(anim: any, length: any) {
			let tabcolumn = this,
				doms = tabcolumn._doms,
				tabs = tabcolumn._tabs,
				tabsEl = doms.tabs,
	...
	dorado.widget.TabBar = $extend(dorado.widget.TabGroup, {
		...
		refreshNavButtons: function() {
			...
			let leftButton = tabbar._leftButton,
				rightButton = tabbar._rightButton;
			let tabsWidth = tabbar.getTabsWidth(),
				tabsEl = doms.tabs,
		...
		showNavButtons: function() {
			...
			if (dom) {
				...
				if (modifyLeft) {
					let tabsEl = doms.tabs,
						left = parseInt(tabsEl.style.left, 10) || 0,
						leftButtonWidth = $fly(doms.leftButton).outerWidth(true),
						rightButtonWidth = $fly(doms.rightButton).outerWidth(true);
		...
		hideNavButtons: function(force: any) {
			...
			let leftButton = doms.leftButton,
				rightButton = doms.rightButton;
		...
		doScrollLeft: function(anim: any, length: any) {
			let tabbar = this,
				doms = tabbar._doms,
				tabsEl = doms.tabs,
				to = parseInt(tabsEl.style.left, 10) + (length > 0 ? length : 100);
			if (anim) {
		...
		doScrollRight: function(anim: any, length: any) {
			let tabbar = this,
				doms = tabbar._doms,
				tabs = tabbar._tabs,
				tabsEl = doms.tabs,
			...
			if (tabs) {
				let tabsWidth = tabbar.getTabsWidth(),
					visibleWidth = $fly(doms.tabsWrap).innerWidth(),
					to = currentLeft - length,
					rightWidth = tabsWidth + currentLeft - visibleWidth;
				if (rightWidth <= 0) {
					return false;
				} else {
		...
		createMenuButton: function(dom: any) {
			let tabbar = this;
			if (!dom) {
				return;
			}
			let wrapEl = dom.lastChild,
				doms = tabbar._doms,
				rightButtonEl = doms.rightButton,
				refEl = wrapEl;
			if (rightButtonEl) {
				refEl = rightButtonEl;
			}
			let navmenu = (tabbar._navmenu = new dorado.widget.Menu({
					listener: {
						beforeShow: function(self: any, configs: any) {
		...
		insertNavMenuItem: function(tab: any, index: any) {
			let tabbar = this,
				navmenu = tabbar._navmenu;
			if (navmenu && tab) {
				navmenu.addItem(
					{
		...
		addRightToolButton: function(button: any) {
			if (!button) {
				return;
			}
			let tabbar = this,
		...
		scrollTabIntoView: function(tab: any) {
			let tabbar = this,
				...
				offsetWidth = tabDom.offsetWidth,
				left = (parseInt(tabsEl.style.left, 10) || 0) * -1,
				viewWidth = $fly(doms.tabsWrap).width();
			$fly(tabDom).addClass("tab-selected");
			if (left > offsetLeft) {
		...
		onToolButtonVisibleChange: function() {
			...
			let rightToolButtons = tabbar._rightToolButtons,
				buttonsWidth = menuButtonWidth;
			if (rightToolButtons) {
				let tabsWrapHeight = $fly(doms.tabsWrap).height();
				for (let i = rightToolButtons.length - 1; i >= 0; i--) {
					let toolButton = rightToolButtons[i],
						toolButtonWidth = $fly(toolButton._dom).outerWidth(true),
						toolButtonHeight = $fly(toolButton._dom).outerHeight(true);
...
dorado.widget.AbstractPanel = $extend(dorado.widget.Container, {
	...
	ATTRIBUTES: {
		...
		collapsed: {
			getter: function(attr: any, value: any) {
				let panel = this;
				if (
					panel._parent instanceof dorado.widget.SplitPanel &&
	...
	toggleCollapsed: function(animate: any) {
		let panel = this,
			collapsed = panel.get("collapsed");
		panel.doSetCollapsed(!collapsed, animate);
	},
	setContentContainerVisible: function(collapsed: any) {
		let panel = this,
	...
	doSetCollapsed: function(collapsed: any, animate: any) {
		function beforeCollapsedChange(panel: any, collapsed: any) {
			if (collapsed) {
				panel._heightBeforeCollapse = panel.getRealHeight();
			}
			panel.setContentContainerVisible(!collapsed);
		}
		function onCollapsedChange(panel: any, collapsed: any) {
		...
		let panel = this,
			dom = panel._dom,
			doms = panel._doms,
			collapseButton = panel._collapseButton,
			eventArg = {};
	...
	_createButtonPanel: function(dom: any) {
		let panel = this,
			doms = panel._doms,
	...
	initButtons: function(dom: any) {
		let panel = this,
			doms = panel._doms;
		if (panel._buttons) {
			let buttons = panel._buttons,
				button,
				buttonPanel;
...
dorado.widget.Panel = $extend(dorado.widget.AbstractPanel, {
	...
	_doOnResize: function(collapsed: any) {
		let panel = this,
			dom = panel._dom,
			doms = panel._doms,
			height = panel.getRealHeight();
		if (typeof height === "number" && height > 0) {
			...
			if (collapsed) {
				...
				$fly(doms.contentPanel).outerHeight(
					height - captionBarHeight - buttonPanelHeight,
				);
				$fly(dom).height("auto");
			}
		}
...
dorado.widget.FieldSet = $extend(dorado.widget.AbstractPanel, {
	...
	refreshDom: function(dom: any) {
		let fieldset = this;
		$fly(dom)[fieldset._collapsed ? "addClass" : "removeClass"](
			"i-field-set-collapsed " + fieldset._className + "-collapsed",
		);
	...
	_doOnResize: function(collapsed: any) {
		...
		if (typeof height === "number" && height > 0) {
			...
			if (collapsed) {
				...
				let $dom = jQuery(dom),
					edgeHeight = $dom.edgeHeight();
				$fly(doms.contentPanel).outerHeight(
					height - captionCtHeight - buttonPanelHeight - edgeHeight,
				);
				$fly(dom).height("auto");
			}
		}
...
dorado.widget.GroupBox = $extend(dorado.widget.AbstractPanel, {
	...
	refreshDom: function(dom: any) {
		let groupBox = this;
		$fly(dom)[groupBox._collapsed ? "addClass" : "removeClass"](
			groupBox._className + "-collapsed",
		);
	...
	_doOnResize: function(collapsed: any) {
		...
		if (typeof height === "number" && height > 0) {
			...
			if (collapsed) {
				...
				$fly(doms.contentPanel).outerHeight(
					height - captionCtHeight - buttonPanelHeight,
				);
				$fly(dom).height("auto");
			}
		}
...
dorado.widget.TabControl = $extend(dorado.widget.TabBar, {
	...
	doOnTabChange: function(eventArg: any) {
		let tabControl = this,
			tabs = tabControl._tabs,
			tab = eventArg.newTab,
			index = typeof tab === "number" ? tab : tabs.indexOf(tab),
			card = tabControl._cardBook;
		if (card) {
	...
	doChangeTabPlacement: function(value: any) {
		...
		let tabcontrol = this,
			dom = tabcontrol._dom;
		if (dom) {
			let tabbarDom = tabcontrol._tabbarDom,
				cardDom = tabcontrol._cardBook._dom;
			if (dorado.Browser.msie && dorado.Browser.version === 6) {
				if (value === "top") {
	...
	createDom: function() {
		let tabcontrol = this,
			...
			tabbarDom = $invokeSuper.call(this, arguments),
			tabPlacement = tabcontrol._tabPlacement;
	...
	refreshDom: function(dom: any) {
		...
		let tabcontrol = this,
			card = tabcontrol._cardBook,
			tabbarDom = tabcontrol._tabbarDom,
			cardDom = tabcontrol._cardBook._dom;
		...
		let tabs = tabcontrol._tabs,
			currentTab = tabcontrol._currentTab,
...
dorado.widget.VerticalTabControl = $extend(dorado.widget.TabColumn, {
	...
	doOnTabChange: function(eventArg: any) {
		let tabcolumnControl = this,
			tabs = tabcolumnControl._tabs,
			tab = eventArg.newTab,
			index = typeof tab === "number" ? tab : tabs.indexOf(tab),
			card = tabcolumnControl._cardBook;
		if (card) {
			card.set("currentControl", index);
		}
		$invokeSuper.call(this, arguments);
	...
	doChangeTabPlacement: function(value: any) {
		...
		let tabcolumnControl = this,
			dom = tabcolumnControl._dom;
		if (dom) {
			let tabcolumnDom = tabcolumnControl._tabcolumnDom,
				cardDom = tabcolumnControl._cardBook._dom;
			if (dorado.Browser.msie && dorado.Browser.version === 6) {
				if (value === "left") {
					dom.appendChild(cardDom);
				} else {
	...
	createDom: function() {
		let tabcolumnControl = this,
			card = tabcolumnControl._cardBook,
			dom = document.createElement("div"),
			tabcolumnDom = $invokeSuper.call(this, arguments),
			tabPlacement = tabcolumnControl._tabPlacement;
	...
	refreshDom: function(dom: any) {
		...
		let tabcolumnControl = this,
			card = tabcolumnControl._cardBook,
			tabcolumnDom = tabcolumnControl._tabcolumnDom,
			cardDom = tabcolumnControl._cardBook._dom;
		...
		let tabs = tabcolumnControl._tabs,
			currentTab = tabcolumnControl._currentTab,
			currentTabIndex = tabs.indexOf(currentTab);
		if (currentTabIndex !== -1) {
			card._currentControl = card._controls.get(currentTabIndex);
		}
...
dorado.widget.CaptionBar = $extend(dorado.widget.Control, {
	...
	ATTRIBUTES: {
		...
		buttons: {
			...
			setter: function(value: any) {
				let bar = this,
					oldValue = bar._buttons;
				if (oldValue) {
					bar.clearButtons();
				}
				if (!bar._buttons) {
					bar._buttons = new dorado.util.KeyedArray(function(value: any) {
	...
	addButton: function(button: any, index: any) {
		let bar = this,
			...
			dom = bar._dom;
		if (!bar._buttons) {
			buttons = bar._buttons = new dorado.util.KeyedArray(function(value: any) {
				return value._uniqueId;
			});
		}
		if (index == null) {
	...
	removeButton: function(button: any) {
		let bar = this,
			buttons = bar._buttons;
	...
	_createButtonGroup: function(dom: any) {
		let bar = this,
			doms = bar._doms;
		dom = dom ? dom : bar._dom;
...
(function() {
	...
	dorado.MessageBox = {
		...
		declineButtons: [],
		onButtonClick: function(buttonIndex: any) {
			let buttonId;
			if (dorado.MessageBox._runStack.length > 0) {
				...
				if (
					typeof config.detailCallback === "function" ||
		...
		executeCallback: function() {
			...
			setTimeout(function() {
				let config = dorado.MessageBox._callbackConfig,
		...
		getDialog: function() {
			if (!dorado.MessageBox._dialog) {
				dorado.MessageBox.defaultTitle = $resource(
					"dorado.baseWidget.MessageBoxDefaultTitle",
				);
				dorado.MessageBox._dialog = new dorado.widget.Dialog({
					...
					modal: true,
					modalType:
						$setting["widget.MessageBox.defaultModalType"] || "transparent",
					closeAction: "hide",
					buttons: [
						{
							width: 60,
				...
				dorado.MessageBox._dialog.doOnAttachToDocument = function() {
					let dialog = this,
						dom = dialog.getContentContainer(),
						doms = dialog._doms;
					...
					if (dom) {
						...
						dorado.MessageBox.updateText(
							dorado.MessageBox._lastText,
							dorado.MessageBox._lastIcon,
							dorado.MessageBox._lastEditor,
							dorado.MessageBox._lastValue,
						);
					}
					dialog.bind("beforeShow", function(self: any, arg: any) {
						let dom = self._dom;
						...
						let doms = self._doms,
							contentWidth =
								$fly(doms.msgText).outerWidth(true) +
						...
						let dialogWidth = $fly(dom).width(),
							panelWidth = $fly(doms.contentPanel).width();
					...
					dialog.bind("afterShow", function(self: any) {
						let buttons = self._buttons,
							button;
						if (buttons) {
							button = buttons[0];
							if (button && button._dom.style.display !== "none") {
					...
					dialog.bind("beforeHide", function(self: any, arg: any) {
						if (dorado.MessageBox._runStack.length > 0) {
							arg.processDefault = false;
					...
					dialog.bind("beforeClose", function(self: any, arg: any) {
		...
		alert: function(msg: any, options: any) {
		...
		confirm: function(msg: any, options: any) {
			if (typeof options === "function") {
				let callback = options;
				options = { callback: callback };
			} else {
		...
		prompt: function(msg: any, options: any) {
			if (typeof options === "function") {
				let callback = options;
				options = { callback: callback };
			} else {
		...
		promptMultiLines: function(msg: any, options: any) {
			if (typeof options === "function") {
				let callback = options;
				options = { callback: callback };
			} else {
		...
		resetEditorWidth: function(editor: any) {
			let dialog = dorado.MessageBox.getDialog(),
				doms = dialog._doms,
				width;
		...
		updateText: function(text: any, icon: any, editor: any, value: any) {
			let dialog = dorado.MessageBox.getDialog(),
				doms = dialog._doms;
		...
		doShow: function(options: any) {
			options = options || {};
			let dialog = dorado.MessageBox.getDialog(),
				msg = options.message,
				defaultText = options.defaultText,
				title = options.title || dorado.MessageBox.defaultTitle,
				icon = options.icon,
				editor = options.editor || "none";
...
(function() {
	...
	let mouseEnterfunc = function(event: any) {
		let panel = event.data.panel;
		if (panel._hidePreviewTimer) {
			clearTimeout(panel._hidePreviewTimer);
		}
	};
	let mouseLeavefunc = function(event: any) {
		let panel = event.data.panel;
		panel._delayHidePreview();
	};
	let documentMouseDown = function(event: any) {
	...
	dorado.widget.SplitPanel = $extend(dorado.widget.Control, {
		...
		ATTRIBUTES: {
			...
			animate: {
				defaultValue:
					dorado.Browser.msie && dorado.Browser.version < 9 ? false : true,
			},
			direction: {
				defaultValue: "left",
				setter: function(value: any) {
		...
		_openPreview: function() {
			...
			$fly([animPanel, doms.collapseBar, doms.splitter]).bind(
				"mouseenter",
				{ panel: panel },
				mouseEnterfunc,
			);
			$fly([animPanel, doms.collapseBar]).bind(
				"mouseleave",
				{ panel: panel },
				mouseLeavefunc,
			);
		...
		createDom: function() {
				...
				.draggable({
					...
					start: function(event: any, ui: any) {
						...
						if (panel._maxPosition != null || panel._minPosition != null) {
							let width = $fly(dom).width(),
								height = $fly(dom).height(),
								min = panel._minPosition || 50,
								max,
								sideMin,
								sideMax,
								range;
					...
					drag: function(event: any, ui: any) {
						let inst = jQuery.data(this, "ui-draggable"),
							horiChange = event.pageX - inst.originalPageX,
							vertChange = event.pageY - inst.originalPageY;
						ui.position = {
			...
			$fly(doms.sidePanel).click(function(event: any) {
				if (panel._collapsed === true) {
					event.stopImmediatePropagation();
				}
			});
			$fly(doms.mainPanel).click(function(event: any) {
				if (panel._collapsed === "main") {
					event.stopImmediatePropagation();
				}
			});
		...
		doSetCollapsed: function(collapsed: any, callback: any, slience: any) {
			let panel = this,
				dom = panel._dom,
				doms = panel._doms,
				oldValue = panel._collapsed,
				eventArg = {};
			panel.fireEvent("beforeCollapsedChange", panel, eventArg);
			if (eventArg.processDefault === false) {
				return;
			...
			if (dom) {
				let width = $fly(dom).width(),
					...
					left;
				let animConfig,
					position = panel.getPixelPosition(),
					animPanel = doms.sidePanel;
				if (collapsed === "main") {
					animPanel = doms.mainPanel;
				}
		...
		getPixelPosition: function() {
			let panel = this,
				position = panel._position,
				dir = panel._direction;
		...
		doOnAttachToDocument: function() {
			let panel = this,
				sideControl = panel._sideControl,
				mainControl = panel._mainControl,
				doms = panel._doms;
			if (sideControl) {
		...
		refreshDom: function(dom: any) {
			...
			let panel = this,
				...
				direction = panel._direction,
				previewable = panel._previewable && panel._collapseable,
			...
			$fly(doms.splitter).removeClass(
				"splitter-h-resizeable splitter-v-resizeable",
			);
			if (panel._resizeable) {
				$fly(doms.splitter).addClass(
					vertical ? "splitter-v-resizeable" : "splitter-h-resizeable",
				);
			}
			let sidePanelStyle,
				splitterStyle,
			...
			if (panel._collapseable && panel._collapsed) {
				if (previewable) {
					...
					$fly(doms.collapseBar)
						...
						.removeClass(
							"collapse-bar-left collapse-bar-right collapse-bar-top collapse-bar-bottom",
						)
				...
				if (panel._collapsed === "main") {
					...
					$fly(doms.splitter)
						.removeClass(
							"splitter-left splitter-right splitter-top splitter-bottom",
						)
...
(function() {
	let CONST_MOUSE_POS_ADJ_X = 5,
		CONST_MOUSE_POS_ADJ_Y = 15,
		TOOLTIP_KEY = "dorado.tooltip",
		DOMS_KEY = "dorado.tip.doms";
	let elementMouseEnter = function(event: any) {
		let element = this,
			tip = dorado.TipManager.getTip(element);
		if (tip._hideTimer) {
	...
	let elementMouseMove = function(event: any) {
		let element = this,
			tip = dorado.TipManager.getTip(element);
		if (tip) {
			if (tip._showTimer) {
	...
	dorado.widget.ToolTip = $extend(dorado.widget.Tip, {
		...
		getShowPosition: function(options: any) {
			let tip = this,
				dom = tip.getDom(),
				event = tip._latestEvent || options.event;
		...
		_updatePosition: function(event: any) {
			let tip = this;
			if (event) {
				let mouseOffset = tip._mouseOffset || [
					CONST_MOUSE_POS_ADJ_X,
	...
	dorado.TipManager = {
		_previousTip: null,
		hasTip: function(element: any) {
			return !!dorado.TipManager.getTip(element);
		},
		getTip: function(element: any) {
			let result;
			if (element) {
		...
		allocTip: function(element: any, options: any) {
			let result;
			options = options || {};
		...
		initTip: function(element: any, options: any) {
			let manager = this;
			if (element) {
		...
		updateTip: function(element: any, options: any) {
			if (dorado.Object.isInstanceOf(element, dorado.RenderableElement)) {
				element = element._dom;
				if (!element) {
					return;
		...
		deleteTip: function(element: any) {
			if (dorado.Object.isInstanceOf(element, dorado.RenderableElement)) {
				element = element._dom;
				if (!element) {
					return;
		...
		showTip: function(element: any, delay: any, event: any) {
			let manager = this,
				tip = dorado.TipManager.getTip(element);
		...
		hideTip: function(tip: any, delay: any) {
			let manager = this;
			if (tip) {
...
(function() {
	function refreshCurrentSection(currentSection: any) {
	...
	dorado.widget.Accordion = $extend(dorado.widget.Control, {
		...
		getSection: function(name: any) {
			if (this._sections) {
				return this._sections.get(name);
			}
			return null;
		},
		doSetCurrentSection: function(section: any, animate: any) {
			let accordion = this,
				oldCurrent = accordion._currentSection,
				newCurrent = section;
			...
			if (animate) {
				let oldCurrentContainer = oldCurrent._doms.container,
					oldContainerHeight = $fly(oldCurrentContainer).height();
				accordion._sliding = true;
				if (accordion._dynaHeight) {
					let newCurrentControlHeight = 0,
						oldControlHeight = 0;
					...
					$fly(newCurrent._doms.container)
						...
						.animate(
							{ dummy: 1 },
							{
								...
								step: function(now: any, animate: any) {
									let defaultEasing =
											animate.options.easing ||
											(jQuery.easing.swing ? "swing" : "linear"),
										pos = jQuery.easing[defaultEasing](
											animate.state,
											animate.options.duration * animate.state,
											0,
											1,
											animate.options.duration,
										);
									$fly(newCurrent._doms.container).css(
										"height",
										Math.ceil(newCurrentControlHeight * pos),
									);
									$fly(oldCurrentContainer).css(
										"height",
										Math.floor(oldControlHeight * (1 - pos)),
									);
								},
							},
						);
		...
		changeToAvailableSection: function() {
			let accordion = this,
				sections = accordion._sections;
			if (sections) {
				let startIndex = -1,
					currentSection = accordion._currentSection,
					i,
					j,
					section;
				if (currentSection) {
		...
		addSection: function(section: any, index: any) {
			let accordion = this,
				sections = accordion._sections,
				refDom;
			if (!sections) {
				accordion._sections = sections = new dorado.util.KeyedArray(
					function(value: any) {
						return value._name;
					},
				);
		...
		removeSection: function(section: any) {
			let accordion = this,
				sections = accordion._sections;
			if (sections) {
		...
		bindAction: function(section: any) {
			let accordion = this;
		...
		refreshDom: function(dom: any) {
			...
			let accordion = this,
				sections = accordion._sections,
				currentSection = accordion._currentSection,
				dynaHeight = accordion._dynaHeight;
			if (sections && !currentSection) {
				currentSection = accordion.changeToAvailableSection();
			}
			let sectionMinHeight,
				ctHeight,
				accordionHeight = $fly(dom).height(),
				visibleCount = accordion.getVisibleSectionCount();
			if (sections) {
...
dorado.widget.ToolBar = $extend(dorado.widget.Control, {
	...
	ATTRIBUTES: {
		...
		items: {
			setter: function(value: any) {
				let toolbar = this,
					items = toolbar._items,
					rendered = toolbar._rendered,
					i,
					l,
					item;
				if (items) {
				...
				if (!items) {
					toolbar._items = items = new dorado.util.KeyedArray(function(value: any) {
						return value._id;
					});
				}
				if (value) {
	...
	createItem: function(config: any) {
		if (!config) {
			return null;
		}
		let item;
				...
				if (item._visibleByOverflow === false) {
					...
					for (let i = 0, j = menuItems.length; i < j; i++) {
						let menuItem = menuItems[i],
							control = item._itemObjects[menuItem.itemCode.key];
						if (control) {
							menuItem.set({
	...
	addItem: function(item: any, index: any) {
		...
		if (!items) {
			items = toolbar._items = new dorado.util.KeyedArray(function(value: any) {
				return value._id;
			});
		}
	...
	hideOverflowItem: function (
		item,
		overflowMenu,
		dataPilotItemCode,
		dataPilot,
	) {
		function addItem(itemCode: any, innerControl: any, dataPilot: any) {
				...
				if (item instanceof dorado.widget.toolbar.Separator) {
					...
					if (
						...
						for (let i = 0, j = compiledItemCodes.length; i < j; i++) {
							let itemCode = compiledItemCodes[i],
								innerControl = item._itemObjects[itemCode.key];
	...
	showUnoverflowItem: function(item: any, dataPilotItem: any) {
	...
	showDataPilot: function(item: any) {
		let compiledItemCodes = item._compiledItemCodes || [];
		item._visibleByOverflow = true;
		for (let i = 0, j = compiledItemCodes.length; i < j; i++) {
			let itemCode = compiledItemCodes[i],
	...
	hideDataPilot: function(item: any, overflowMenu: any, currentLeft: any, leftVisibleWidth: any) {
		let toolbar = this,
			compiledItemCodes = item._compiledItemCodes || [],
			startHide = false,
			bindMenuItems = [];
		item._bindMenuItems = bindMenuItems;
		item._visibleByOverflow = false;
	...
	doOnResize: function() {
		...
		let toolbar = this,
			dom = toolbar._dom,
			doms = toolbar._doms,
			overflowMenu = toolbar._overflowMenu,
			overflowButton = toolbar._overflowButton,
			items = toolbar._items,
			lastChild = doms.toolbarLeft.lastChild,
			overflow = false;
		...
		if (items && lastChild) {
			let leftRealWidth = lastChild.offsetWidth + lastChild.offsetLeft,
...
dorado.widget.toolbar.Button = $extend(dorado.widget.Button, {
	...
	createDom: function() {
		let button = this,
			dom = $invokeSuper.call(button, arguments);
		$fly(dom)
			.mouseenter(function() {
				let menu = button._menu,
					toolbar = button._parent;
...
dorado.widget.FloatPanel = $extend(
	[dorado.widget.Panel, dorado.widget.FloatControl],
	{
...
(function() {
	...
	dorado.widget.Dialog = $extend(dorado.widget.FloatPanel, {
		...
		doHandleOverflow: function(options: any) {
			let dialog = this;
		...
		maximize: function() {
			...
			if (dom) {
				...
				let maximizeTarget = dialog._maximizeTarget,
					originalMaimizeTarget = maximizeTarget;
		...
		doSetCollapsed: function(collapsed: any) {
			$invokeSuper.call(this, arguments);
			let dialog = this;
			if (dialog._resizeable) {
				if (collapsed) {
		...
		_doOnResize: function(collapsed: any) {
			let dialog = this,
				dom = dialog._dom,
				doms = dialog._doms,
				height = dialog.getRealHeight(),
				width = dialog.getRealWidth();
			...
			if (typeof height === "number" && height > 0) {
				...
				if (collapsed) {
					...
					let headerHeight = $fly(doms.header).outerHeight(true),
						footerHeight = $fly(doms.footer).outerHeight(true),
						captionBarHeight = 0,
						buttonPanelHeight = 0;
					if (doms.captionBar) {
						captionBarHeight = $fly(doms.captionBar).outerHeight(true);
		...
		createDom: function() {
				...
				jQuery(dom)
					...
					.draggable({
						...
						start: function(event: any, ui: any) {
							...
							let bodyEl = $fly(document.body),
								width = bodyEl.outerWidth(true),
								height = bodyEl.outerHeight(true),
								offset = bodyEl.offset();
							bodyRect = {
								left: offset.left,
								top: offset.top,
						...
						stop: function(event: any, ui: any) {
							if (useDraggingFakeDialog) {
								let helper = ui.helper,
									left = parseInt(helper.css("left"), 10),
									top = parseInt(helper.css("top"), 10);
								$fly(dom)
			...
			if (dialog._resizeable) {
				...
				jQuery(dom)
					.addClass("d-dialog-resizeable")
					.find(".dialog-resize-handle")
					.each(function(index: any, handle: any) {
						let className = handle.className.split(" ")[0],
							config = handleConfigMap[className];
						if (!config) {
							return;
						}
						jQuery(handle).draggable({
							...
							drag: function(event: any, ui: any) {
								let horiStyle = config.horiStyle,
									vertStyle = config.vertStyle,
									heightRatio = config.heightRatio,
									widthRatio = config.widthRatio,
									minWidth = dialog._minWidth || 200,
									minHeight = dialog._minHeight || 100,
									maxWidth = dialog._maxWidth,
									maxHeight = dialog._maxHeight;
								ui.position = {
							...
							stop: function(event: any, ui: any) {
								let wrapEl = ui.helper,
									offset = wrapEl.offset();
...
(function() {
	...
	dorado.widget.DataPilot = $extend(
		[dorado.widget.Control, dorado.widget.DataControl],
		{
			...
			refreshDom: function(dom: any) {
				...
				if (this._itemCodeExpression !== this._currentItemCodeExpression) {
					...
					let itemObjects = this._itemObjects,
						oldItemObjects = itemObjects;
					if (itemObjects) {
			...
			createItem: function(itemCode: any) {
				function fireOnActionEvent(code: any, control: any) {
				...
				switch (itemCode.code) {
					...
					case ">":
						item = new PageButton({
							...
							onClick: function() {
								...
								let list = pilot.getBindingData();
								if (
									list instanceof dorado.EntityList &&
					...
					case "goto":
						item = new GotoPageControl({
							onAction: function(self: any, arg: any) {
								if (arg.pageNo < 1) {
									arg.pageNo = 1;
								}
								if (!fireOnActionEvent.call(pilot, itemCode.code, self)) {
								...
								if (arg.pageSize > 0) {
									...
									if (
										...
										if (
											...
											let pd = parent.dataType.getPropertyDef(
												list.parentProperty,
											);
											if (pd) {
												item.set("disabled", true);
						...
						item = new ToolBarButton({
							...
							onClick: function() {
								...
								dorado.MessageBox.confirm(
									$resource("dorado.baseWidget.DataPilotDeleteConfirm"),
									function() {
										let list = pilot.getBindingData();
					...
					case "x":
						item = new ToolBarButton({
							...
							onClick: function() {
								...
								dorado.MessageBox.confirm(
									$resource("dorado.baseWidget.DataPilotCancelConfirm"),
									function() {
										let list = pilot.getBindingData();
										if (list instanceof dorado.EntityList && list.current) {
											list.current.cancel();
			...
			compileItemCodes: function() {
				...
				let compiledItemCodes = (this._compiledItemCodes = []),
					itemCodeExpression = "";
				for (let i = 0; i < itemCodes.length; i++) {
					let itemCode = itemCodes[i],
			...
			refreshItem: function(itemCode: any) {
				...
				let pageNo = list ? list.pageNo : 1,
					pageCount = list ? list.pageCount : 1,
					pageSize = list ? list.pageSize : 0,
					entityCount = list ? list.entityCount : 0;
				let current = list ? list.current : null,
					disabled = this._disabled;
				switch (itemCode.code) {
					case "|<":
					...
					case "info":
						let text = $resource(
							"dorado.baseWidget.DataPilotInfo",
							pageNo,
							pageCount,
							entityCount,
						);
					...
					case "+":
						item.set(
							"disabled",
							disabled || (this._dataSet ? this._dataSet._readOnly : true),
						);
						break;
					case "-":
						item.set(
							"disabled",
							disabled || !current || this._dataSet._readOnly,
						);
						break;
					case "x":
						item.set(
	...
	let GotoPageControl = $extend(dorado.widget.Control, {
		...
		createDom: function(dom: any) {
			...
			let spinner = (this._spinner = new dorado.widget.NumberSpinner({
				...
				onPost: function(self: any, arg: any) {
					gotoPage.fireEvent("onAction", gotoPage, {
						pageNo: spinner.get("value"),
					});
				},
				onKeyDown: function(self: any, arg: any) {
					if (arg.keyCode === 13) {
						spinner.post();
		...
		refreshDom: function(dom: any) {
			...
			$fly(this._labelPrefix).text(
				$resource(
					"dorado.baseWidget.DataPilotGotoPagePrefix",
					this._pageNo,
					this._pageCount,
					this._entityCount,
				),
			);
			$fly(this._labelSuffix).text(
				$resource(
					"dorado.baseWidget.DataPilotGotoPageSuffix",
					this._pageNo,
					this._pageCount,
					this._entityCount,
				),
	...
	let PageSizeControl = $extend(dorado.widget.Control, {
		...
		refreshDom: function(dom: any) {
			...
			let spinner = this._spinner;
			$fly(this._labelPrefix).text(
				$resource("dorado.baseWidget.DataPilotPageSize"),
			);
