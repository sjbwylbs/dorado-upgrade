...
(function () {
	...
	dorado.widget.treegrid.ItemModel = $extend(
		[dorado.widget.tree.ItemModel, dorado.widget.grid.ItemModel],
		{
			constructor: function () {
				dorado.widget.grid.ItemModel.prototype.constructor.apply(
					this,
					arguments,
				);
	...
	dorado.widget.treegrid.RowRenderer = $extend(
		dorado.widget.grid.DefaultRowRenderer,
		{
			renderCell: function (cellRenderer: any, dom: any, arg: any) {
				let node = (arg.node = arg.data._node);
				arg.dataForSelection = node;
				dorado.widget.grid.DefaultRowRenderer.prototype.renderCell.call(
					this,
					cellRenderer,
					dom,
					arg,
	...
	dorado.widget.treegrid.TreeColumnCellRenderer = $extend(
		[
		...
		],
		{
			...
			doRender: function (dom: any, arg: any) {
				...
				dorado.widget.tree.TreeNodeRenderer.prototype.doRender.call(
					this,
					dom.firstChild,
					arg.node,
					arg,
	...
	dorado.widget.treegrid.TreeColumnCellEditor = $extend(
		dorado.widget.grid.DefaultCellEditor,
		{
			...
			resize: function () {
				...
				let cell = this.cell,
					$gridDom = jQuery(this.grid.getDom());
				if (!dom || !cell) {
					return;
				}
				let $cellDom = $fly(cell);
				let offsetGrid = $gridDom.offset(),
					offsetCell = $fly(cell).offset();
				let lp = $cellDom.find(".node-label").offset().left;
				let l = lp - offsetGrid.left - $gridDom.edgeLeft(),
					t =
						offsetCell.top -
						offsetGrid.top -
						$gridDom.edgeTop() +
				...
				$fly(dom)
					.css({ left: l, top: t })
					.outerWidth(w - (lp - offsetCell.left))
					.outerHeight(h)
					.bringToFront();
				if (control) {
	...
	dorado.widget.TreeGrid = $extend(
		[dorado.widget.AbstractGrid, dorado.widget.AbstractTree],
		{
			...
			ATTRIBUTES: {
				...
				treeColumn: { writeBeforeReady: true },
				currentNode: {
					skipRefresh: true,
					getter: function (p: any) {
						if (this._innerGrid || this._fixedInnerGrid) {
							return (
								this._domMode === 2 ? this._fixedInnerGrid : this._innerGrid
							).get(p);
						} else {
							return this._currentNode;
						}
					},
					setter: function (v: any, p: any) {
						if (this._innerGrid || this._fixedInnerGrid) {
							(this._domMode === 2 ? this._fixedInnerGrid : this._innerGrid).set(
								p,
								v,
							);
						} else {
				...
				currentEntity: {
					setter: function (currentEntity: any) {
						let node = currentEntity
							? this._entityMap[currentEntity.entityId]
							: null;
						this.set("currentNode", node);
					},
					getter: function () {
						let data = this.get("currentNode.data");
			...
			createInnerGrid: function (fixed: any) {
				let innerGrid = new dorado.widget.treegrid.InnerTreeGrid(this, fixed);
				innerGrid._root = this._root;
				return innerGrid;
			},
			createItemModel: function () {
				let im = new dorado.widget.treegrid.ItemModel(this);
				im.setItems(this._root);
				return im;
			},
			onCellValueEdit: function (entity: any, column: any) {
				let node = this._entityMap[entity.entityId];
				if (node) {
			...
			refreshDom: function (dom: any) {
				if (this._treeColumn) {
					let cols = this.findColumns(this._treeColumn),
						col = cols[0];
					if (!col) {
						throw new dorado.Exception(
							"TreeColumn [" + this._treeColumn + "] not found.",
						);
					}
			...
			getCellEditor: function (column: any, node: any) {
				let cellEditor = $invokeSuper.call(this, [column, node._data]);
				if (cellEditor) {
			...
			findItemDomByEvent: function (evt: any) {
				let row = this._innerGrid.findItemDomByEvent(evt);
			...
			_refreshAndScroll: function (node: any, mode: any, parentNode: any, nodeIndex: any) {
				...
				if (parentNode._expanded) {
					let row = this._innerGrid._itemDomMap[node._uniqueId];
					if (!row) {
						let index;
						if (mode === "remove") {
			...
			_doOnKeyDown: function (evt: any) {
				...
				let items = this._itemModel.getItems(),
					currentNode = this.get("currentNode");
				switch (evt.keyCode) {
					case 36:
						if (evt.ctrlKey) {
					...
					case 35:
						if (evt.ctrlKey) {
							let it = this._itemModel.iterator(0);
							it.last();
					...
					case 38:
						if (currentNode) {
							let index = this._itemModel.getItemIndex(currentNode);
							if (index > 0) {
								let node = this._itemModel.iterator(index - 1).next();
								if (node) {
									this.set("currentNode", node);
									currentChanged = true;
								}
			...
			getHeaderOptionMenu: function (create: any) {
				function hiddenItems(items: any) {
					jQuery.each(items, function (i: any, item: any) {
						menu.findItem(item).set("visible", false);
					});
				}
				let hasMenu = !!this._headerOptionMenu;
				let menu = $invokeSuper.call(this, arguments);
				if (!hasMenu && menu) {
					hiddenItems([
						"sortAsc",
						"sortDesc",
			...
			doOnDraggingSourceMove:
				dorado.widget.AbstractTree.prototype.doOnDraggingSourceMove,
			processItemDrop: dorado.widget.AbstractTree.prototype.processItemDrop,
		},
	);
	let TreePrototype = dorado.widget.Tree.prototype;
	dorado.widget.treegrid.InnerTreeGrid = $extend(
		dorado.widget.grid.AbstractInnerGrid,
		{
			ATTRIBUTES: {
				currentNode: {
					skipRefresh: true,
					setter: function (currentNode: any) {
						...
						if (this._rendered && this._itemDomMap) {
							let row = currentNode
								? this._itemDomMap[currentNode._uniqueId]
								: null;
						...
						grid.doInnerGridSetCurrentRow(
							this,
							currentNode ? currentNode._uniqueId : null,
						);
					},
				},
			},
			...
			setCurrentRowByItemId: function (itemId: any) {
				let node = this._itemModel.getItemById(itemId);
...
dorado.widget.DataTreeGrid = $extend(
	[dorado.widget.TreeGrid, dorado.widget.DataTree],
	{
		$className: "dorado.widget.DataTreeGrid",
		ATTRIBUTES: {
			autoCreateColumns: { defaultValue: true },
			selection: {
		...
		addColumn: function () {
			...
			if (
				this._autoCreateColumns &&
				column instanceof dorado.widget.grid.DataColumn &&
				column._property &&
				column._property !== "none"
			) {
		...
		refreshDom: function (dom: any) {
			...
			if (!bindingConfigs || !bindingConfigs.length) {
				throw new dorado.Exception(
					"DataTreeGrid " + this._id + ".bindingConfigs is undefined.",
				);
			}
			...
			if (this._dataSet) {
				...
				if (dataType) {
					...
					if (this._autoCreateColumns && !this._listeningDataTypeRepository) {
						...
						let grid = this;
						this.get("dataTypeRepository").bind(
							"onDataTypeRegister",
							function (self: any, arg: any) {
