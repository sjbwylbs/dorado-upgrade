...
let dorado = { id: "_" + parseInt(Math.random() * Math.pow(10, 8)), _ID_SEED: 0, _TIMESTAMP_SEED: 0, _GET_ID: function (obj) {
        return obj._id;
...
dorado.Core = { VERSION: "8.0.0", newId: function () {
    ...
    }, createClassFromString: function (className) {
...
(function () {
    ...
    let invokeSuper = window.$invokeSuper = function (args) {
...
(function () {
    ...
    dorado.AttributeWatcher = $class({ $className: "dorado.AttributeWatcher", constructor: function (watcherData) {
            this._watcherData = watcherData;
...
(function () {
    ...
    // CSP 兼容：解释器模式的格式化函数
    Date.getFormatCodeValue = function (date, character) {
        switch (character) {
            case "d":
...
dorado.util.AjaxEngine = $extend([dorado.AttributeSupport, dorado.EventSupport], { $className: "dorado.util.AjaxEngine", constructor: function (options) {
        ...
        if (options) {
            ...
            } }, minConnectInterval: { defaultValue: 20 }, maxBatchSize: { defaultValue: 20 } }, EVENTS: { beforeRequest: {}, onResponse: {}, beforeConnect: {}, onDisconnect: {} }, request: function (options, callback) {
        if (typeof options === "string") {
            options = { url: options };
        }
        let id = dorado.Core.newId();
        dorado.util.AjaxEngine.ASYNC_REQUESTS[id] = true;
        let callbackWrapper = { callback: function (success, result) {
        ...
        let batchCallback = { scope: this, callback: function (success, batchResult) {
...
(function ($) {
    ...
    let disableMouseWheel = function (event) {
...
(function () {
    ...
    let backupStyle = function (element, type) {
    ...
    let getAnimateConfig = function (type, direction, element, safe) {
    ...
    let slide = function (type, element, options, safe) {
    ...
    let zoom = function (type, element, options) {
    ...
    let modernZoom = function (type, el, options) {
        if (!el) {
            return;
        }
        options = options || {};
    ...
    let flip = function (type, el, options) {
        if (!el) {
            return;
        }
        options = options || {};
...
dorado.ModalManager = { _controlStack: [], getMask: function () {
        ...
        if (!maskDom) {
            ...
            $fly(maskDom).mousedown(function (evt) {
                let repeat = function (fn, times, delay) {
                    let first = true;
                    return function () {
                        if (times-- >= 0) {
                            if (first) {
...
(function ($) {
    ...
    let ModernScroller = dorado.util.Dom.ModernScroller = $class({ constructor: function (container, options) {
