...
(function () {
    ...
    dorado.widget.RowListDropDown = $extend(dorado.widget.DropDown, {
        ...
        createDropDownBox: function () {
            let dropDown = this, box = $invokeSuper.call(this, arguments), rowList;
            let config = {
                style: "border: none",
                allowNoCurrent: true,
        ...
        initDropDownData: function (box, editor) {
            ...
            let value = editor.get("value"), currentIndex = -1;
            if (items && value) {
        ...
        doOnEditorKeyDown: function (editor, evt) {
            ...
            let dropdown = this, retValue = true;
            if (this.get("opened")) {
                let rowList = this.get("box.control");
                switch (evt.keyCode) {
                    case 38:
    ...
    dorado.widget.AutoMappingDropDown = $extend(dorado.widget.RowListDropDown, {
        ...
        getDropDownItems: function () {
            let editor = this._editor, pd = editor._propertyDef;
...
dorado.widget.DataSetDropDown = $extend(dorado.widget.ListDropDown, {
    ...
    open: function (editor) {
        let dropdown = this, dataSet = dropdown._dataSet, superClass = $getSuperClass();
        let doOpen = function (flush) {
    ...
    createDropDownBox: function () {
        if (this._useDataBinding) {
            let dropDown = this, box = dorado.widget.DropDown.prototype.createDropDownBox.call(this), rowList;
            let config = {
    ...
    createDropDownBox: function () {
        ...
        if (dorado.Object.isInstanceOf(control, dorado.widget.FloatControl)) {
            ...
            box.bind("beforeShow", function () {
                let $box = jQuery(box.getDom().firstChild), boxWidth = $box.width(), boxHeight = $box.height();
                let $dom = jQuery(control.getDom()), realWidth = $dom.outerWidth(), realHeight = $dom.outerHeight(), shouldRefresh;
...
(function () {
    dorado.widget.NumberGridPicker = $extend(dorado.widget.Control, {
        ...
        EVENTS: { onPick: {} },
        createDom: function () {
            let picker = this, min = picker._min === undefined ? 1 : picker._min, max = picker._max, step = picker._step || 1, columnCount = picker._columnCount || 1, rowCount = picker._rowCount || 1, doms = {};
            ...
            for (let i = 0; i < rowCount; i++) {
                ...
                for (let j = 0; j < columnCount; j++) {
                    let td = document.createElement("td"), value = min + step * (i * columnCount + j);
            ...
            $fly(dom).click(function (event) {
                let position = $DomUtils.getCellPosition(event), step = picker._step || 1;
                if (position && position.element) {
                    if (position.row >= picker._rowCount) {
                        return;
                    }
                    let min = picker._min === undefined ? 1 : picker._min, step = (step = picker._step || 1), value = min + step * (position.row * columnCount + position.column);
                    picker._value = value;
        ...
        refreshTable: function () {
            let picker = this, dom = picker._doms.body, formatter = picker._formatter;
            let step = picker._step || 1, min = picker._min === undefined ? 1 : picker._min, columnCount = picker._columnCount, rowCount = picker._rowCount, row = Math.floor((value - min) / columnCount), column = (value - min) % columnCount;
            for (let i = 0; i < rowCount; i++) {
                let rows = dom.rows[i];
                for (let j = 0; j < columnCount; j++) {
                    let cell = rows.cells[j], value = min + step * (i * columnCount + j);
        ...
        refreshValue: function () {
            let picker = this, dom = picker._doms.body, lastSelectedCell = picker._lastSelectedCell, value = picker._value;
            if (isNaN(picker._value)) {
                return;
            }
            let step = picker._step || 1, min = picker._min === undefined ? 1 : picker._min, columnCount = picker._columnCount, rowCount = picker._rowCount, row = Math.floor((value - min) / columnCount), column = (value - min) % columnCount, cell;
            if (dom.rows[row]) {
    ...
    dorado.widget.MonthPicker = $extend(dorado.widget.NumberGridPicker, {
        ...
        doOnKeyDown: function (event) {
            let picker = this, month = picker.get("value");
    ...
    dorado.widget.YearPicker = $extend(dorado.widget.NumberGridPicker, {
        ...
        ATTRIBUTES: {
            ...
            value: {
                setter: function (value) {
                    let picker = this, oldValue = picker._value, startYear, remainder;
                    remainder = value % 10;
        ...
        createDom: function () {
            let picker = this, dom = $invokeSuper.call(picker, arguments);
            ...
            let buttonRow = document.createElement("tr"), prevYearCell = document.createElement("td"), nextYearCell = document.createElement("td");
        ...
        doOnKeyDown: function (event) {
            let picker = this, year = picker.get("value");
            switch (event.keyCode) {
                case 38:
    ...
    dorado.widget.YearMonthPicker = $extend(dorado.widget.Control, {
        ...
        createDom: function () {
            let monthLabel = $resource("dorado.baseWidget.AllMonths") || "", monthLabels = monthLabel.split(",");
            let picker = this, doms = {}, dom = $DomUtils.xCreate({
                tagName: "div",
        ...
        doOnResize: function () {
            $invokeSuper.call(this, arguments);
            let picker = this, dom = picker._dom, doms = picker._doms, height = picker.getRealHeight();
            if (typeof height === "number" && height > 0) {
                let innerHeight = dom.clientHeight, blockHeight = innerHeight - doms.buttonPanel.offsetHeight;
                $fly([doms.yearTable, doms.monthTable]).css("height", blockHeight);
            }
        },
        doOnKeyDown: function (event) {
            let picker = this, year = picker.get("year"), month = picker.get("month");
            switch (event.keyCode) {
                case 37:
    ...
    dorado.widget.YearDropDown = $extend(dorado.widget.DropDown, {
        ...
        createDropDownBox: function (editor) {
            let dropDown = this, box = $invokeSuper.call(this, arguments), picker = new dorado.widget.YearPicker({
                onPick: function () {
        ...
        doOnEditorKeyDown: function (editor, evt) {
            let dropdown = this, retValue = true, yearPicker = dropdown.get("box.control");
            switch (evt.keyCode) {
                case 27:
                    dropdown.close();
                    retValue = false;
        ...
        initDropDownBox: function (box, editor) {
            let dropDown = this, picker = dropDown.get("box.control");
            if (picker) {
    ...
    dorado.widget.MonthDropDown = $extend(dorado.widget.DropDown, {
        ...
        createDropDownBox: function (editor) {
            let dropDown = this, box = $invokeSuper.call(this, arguments), picker = new dorado.widget.MonthPicker({
    ...
    dorado.widget.YearMonthDropDown = $extend(dorado.widget.DropDown, {
        ...
        createDropDownBox: function (editor) {
            let dropDown = this, box = $invokeSuper.call(this, arguments), picker = new dorado.widget.YearMonthPicker({
        ...
        doOnEditorKeyDown: function (editor, event) {
            let dropDown = this, retValue = true, ymPicker = dropDown.get("box.control");
            if (this.get("opened")) {
                switch (event.keyCode) {
                    case 27:
                        dropDown.close();
...
(function () {
    let DateHelper = {
        getWeekCountOfYear: function (date) {
            ...
            let duration = date.getTime() - begin.getTime() + (day - 1) * 3600 * 24 * 1000;
    ...
    let CELL_UNSELECTABLE_CLASS = "unselectable", CELL_SELECTED_CLASS = "selected-date", PREV_MONTH_CLASS = "pre-month", NEXT_MONTH_CLASS = "next-month";
    dorado.widget.DatePicker = $extend(dorado.widget.Control, {
        ...
        setYear: function (year, refresh, animate) {
            let picker = this, date = picker._date, oldDay = date.getDate(), oldMonth = date.getMonth(), oldYear = date.getFullYear();
        ...
        getMonthAnimateType: function (source, target) {
            if (source && target) {
                let syear = source.getFullYear(), tyear = target.getFullYear(), smonth = source.getMonth(), tmonth = target.getMonth();
        ...
        doMonthAnimate: function (source, target) {
            let picker = this, animateType = picker.getMonthAnimateType(source, target);
            ...
            let dateTable = picker._doms.dateTable, dateBlock = picker._doms.dateBlock, dateTableWidth = dateBlock.offsetWidth, dateTableHeight = dateBlock.offsetHeight;
        ...
        setMonth: function (month, refresh, animate) {
            let picker = this, date = picker._date, oldDay = date.getDate(), oldYear = date.getFullYear(), oldMonth = date.getMonth();
            let source = new Date(date.getTime());
            if (month === "prev") {
                if (oldMonth !== 0) {
                    month = oldMonth - 1;
        ...
        setDate: function (day, refresh) {
            let picker = this, date = picker._date;
            switch (day) {
                case "next":
        ...
        refreshYearMonth: function () {
            let picker = this, doms = picker._doms, date = picker._date;
            let format = this._yearMonthFormat ||
        ...
        refreshSpinner: function () {
            let picker = this, spinner = picker._timeSpinner, date = picker._date;
        ...
        createDom: function () {
            let allWeeks = $resource("dorado.baseWidget.AllWeeks") ||
            ...
            if (picker._showTodayButton) {
                let todayButton = new dorado.widget.Button({
                    ...
                    listener: {
                        onClick: function () {
                            let now = new Date(), oldDate = picker._date;
                            picker.set("date", now);
            ...
            if (picker._showConfirmButton) {
                let confirmButton = new dorado.widget.Button({
                    ...
                    listener: {
                        onClick: function () {
                            let date, pickerDate = picker._date;
            ...
            $fly(dateTable)
                ...
                let position = $DomUtils.getCellPosition(event), element = position.element, date = picker._date;
                if (position && element) {
                    ...
                    if (isSingleSelect) {
                        ...
                        let selectDate = new Date(date.getFullYear(), date.getMonth(), parseInt(element.innerHTML, 10));
        ...
        doShowTimeSpinner: function () {
            let picker = this, spinner = picker._timeSpinner;
            if (!spinner) {
        ...
        showYMPicker: function () {
            let picker = this, dom = picker._dom, ymPicker = picker._yearMonthPicker;
            if (!ymPicker && dom) {
                ymPicker = picker._yearMonthPicker = new dorado.widget.YearMonthPicker({
                    ...
                    listener: {
                        onPick: function () {
                            let ymPicker = picker._yearMonthPicker, year = ymPicker.get("year"), month = ymPicker.get("month");
        ...
        hideYMPicker: function (animate) {
            let picker = this, ymPicker = picker._yearMonthPicker;
            if (ymPicker) {
                if (animate === false) {
        ...
        doOnResize: function () {
            ...
            let picker = this, dom = picker._dom, doms = picker._doms, height = picker.getRealHeight();
            if (typeof height === "number" && height > 0) {
                let innerHeight = dom.clientHeight, blockHeight = innerHeight -
        ...
        doOnKeyDown: function (event) {
            let picker = this, date, ymPicker = picker._yearMonthPicker;
    ...
    dorado.widget.DateDropDown = $extend(dorado.widget.DropDown, {
        ...
        createDropDownBox: function () {
            let dropDown = this, box = $invokeSuper.call(this, arguments), picker = new dorado.widget.DatePicker({
        ...
        doOnEditorKeyDown: function (editor, evt) {
            let dropDown = this, retValue = true, datePicker = dropDown.get("box.control");
            if (this.get("opened")) {
                switch (evt.keyCode) {
                    case 27:
                        dropDown.close();
        ...
        initDropDownBox: function (box, editor) {
            let dropDown = this, datePicker = dropDown.get("box.control");
            if (datePicker) {
                ...
                if (datePicker._yearMonthPicker &&
...
(function () {
    dorado.widget.SpinnerTrigger = $extend(dorado.widget.Trigger, {
        ...
        createTriggerButton: function (editor) {
            let trigger = this, control = new dorado.widget.HtmlContainer({
    ...
    dorado.widget.Spinner = $extend(dorado.widget.AbstractTextBox, {
        ...
        getSpinnerTriggers: function () {
            ...
            if (!this._spinTrigger) {
                let triggers = (this._spinTrigger = []), self = this;
        ...
        createDom: function () {
            let dom = $invokeSuper.call(this, arguments), self = this;
    ...
    dorado.widget.NumberSpinner = $extend(dorado.widget.Spinner, {
        ...
        post: function (force) {
            let text = this.get("text"), value = text ? parseInt(text, 10) : null;
            if (text !== value) {
    ...
    dorado.widget.MultiSlotSpinner = $extend(dorado.widget.Spinner, {
        ...
        initSlotConfigs: function () {
            let slotConfigs = this.slotConfigs, slotMap = (this._slotMap = {}), values = (this._values = []);
            for (let i = 0, j = slotConfigs.length; i < j; i++) {
                let config = slotConfigs[i], name = config.name;
        ...
        createTextDom: function () {
            let spinner = this, doms = {}, dom = $DomUtils.xCreate({
                tagName: "div",
        ...
        doSetSlotValue: function (slotIndex, value) {
            ...
            if (value != null) {
                let config = spinner.slotConfigs[slotIndex], range = config.range || [], minValue = range[0], maxValue = range[1];
        ...
        doGetSlotText: function (slotIndex) {
            ...
            if (text == null) {
                ...
                let num = text, negative = num < 0, text = Math.abs(num) + "";
        ...
        doStepUp: function () {
            let spinner = this, currentSlotIndex = spinner._currentSlotIndex;
            if (!currentSlotIndex) {
                currentSlotIndex = spinner.doChangeCurrentSlot();
            }
            let value = spinner.doGetSlotValue(currentSlotIndex) + spinner._step;
            let range = spinner.doGetSlotRange(currentSlotIndex), minValue = range[0], maxValue = range[1];
            if (value == null) {
                value = minValue;
            }
            else {
            ...
            switch (event.keyCode) {
                ...
                case 8:
                    if (spinner._currentSlotIndex >= 0 && !spinner._realReadOnly) {
                        let currentSlotIndex = spinner._currentSlotIndex, value, range = spinner.doGetSlotRange(currentSlotIndex);
                ...
                case 187:
                    if (spinner._currentSlotIndex >= 0 && !spinner._realReadOnly) {
                        let currentSlotIndex = spinner._currentSlotIndex, value = spinner.doGetSlotValue(currentSlotIndex) || 0;
                        if (value) {
                            value = Math.abs(value);
                            spinner.doSetSlotValue(currentSlotIndex, value);
                        }
                ...
                default:
                    if (spinner._currentSlotIndex >= 0 && !spinner._realReadOnly) {
                        if ((event.keyCode >= 48 && event.keyCode <= 57) ||
                            (event.keyCode >= 96 && event.keyCode <= 105)) {
                            let number = event.keyCode >= 96 ? event.keyCode - 96 : event.keyCode - 48, currentSlotIndex = spinner._currentSlotIndex, range = spinner.doGetSlotRange(currentSlotIndex), maxValue = range[1];
                            let value = spinner._neverEdit
                                ? 0
                                : spinner.doGetSlotValue(currentSlotIndex) || 0, ignore = false;
                            let config = spinner.slotConfigs[currentSlotIndex], digit = config.digit;
        ...
        doChangeCurrentSlot: function (slotIndex) {
            ...
            let oldSlotIndex = spinner._currentSlotIndex, doms = spinner._doms;
            let config = spinner.slotConfigs[slotIndex];
            if (config) {
                if (oldSlotIndex >= 0) {
        ...
        doAfterSlotBlur: function (slotIndex) {
            let spinner = this, value = spinner.doGetSlotValue(slotIndex);
            if (value == null) {
                spinner.doSetSlotValue(slotIndex, spinner.slotConfigs[slotIndex].defaultValue);
            }
        },
        doOnFocus: function () {
            let spinner = this, currentSlotIndex = spinner._currentSlotIndex, doms = spinner._doms;
        ...
        doRefreshSlots: function () {
            let spinner = this, doms = spinner._doms;
            if (!spinner._dom) {
                return;
            }
        ...
        doGetText: function () {
            let spinner = this, slotConfigs = spinner.slotConfigs, text = "";
    ...
    dorado.widget.DateTimeSpinner = $extend(dorado.widget.MultiSlotSpinner, {
        ...
        ATTRIBUTES: {
            type: {
                ...
                setter: function (type) {
                    ...
                    let configs, template = this.slotConfigTemplate;
                    switch (type) {
                        case "date":
            ...
            value: {
                ...
                setter: function (d) {
                    let year = 0, month = 1, date = 1, hours = 0, minutes = 0, seconds = 0;
                    if (d) {
                        year = d.getFullYear();
                        month = d.getMonth() + 1;
                        date = d.getDate();
        ...
        doSetSlotValue: function (slotIndex, value) {
            ...
            let dateSlotIndex = spinner.getSlotIndexByName("date"), date = spinner._values[dateSlotIndex], newDate = 0;
            if (date >= 28) {
        ...
        doGetSlotRange: function (slotIndex) {
            ...
            if (slotName === "date" && spinner._slotMap["date"]) {
                let year = spinner._values[0], month = spinner._values[1], dayCount = new Date(year, month - 1).getDaysInMonth();
                return [1, dayCount];
            }
            else {
                return $invokeSuper.call(this, arguments);
    ...
    dorado.widget.CustomSpinner = $extend(dorado.widget.MultiSlotSpinner, {
        ...
        parsePattern: function (pattern) {
            function parseSlotConfig(slotConfig) {
                let slot = {}, sections = slotConfig.split("|");
                if (sections[0] !== "*") {
            ...
            this.slotConfigs = [];
            let i = 0, c, text = "", slotConfig = "", inSlot = false;
