    ...
    refreshDom: function (dom) {
        ...
        if (entity) {
            ...
            if (timestamp !== this._timestamp) {
                renderer.render(dom, {
                    text: this._property && entity != null
...
dorado.widget.Image = $extend([dorado.widget.Control, dorado.widget.PropertyDataControl], {
    ...
    doStretchAndPack: function () {
        ...
        let image = this, dom = image._dom, imageDom = dom.firstChild, stretchMode = image._stretchMode, packMode = image._packMode || "center", controlWidth = dom.clientWidth || image._width, controlHeight = dom.clientHeight || image._height, left = 0, top = 0, imageWidth = image._originalWidth, imageHeight = image._originalHeight, forceFit = false;
        if (stretchMode === "fill") {
            forceFit = true;
    ...
    createDom: function () {
        let image = this, dom = $DomUtils.xCreate({
            tagName: "DIV",
            content: { tagName: "IMG" },
            style: { position: "relative" },
        });
        let imageDom = (image._imageDom = dom.firstChild), $imageDom = $fly(imageDom), stretchMode = image._stretchMode;
        if (stretchMode === "keepRatio" ||
            stretchMode === "fitWidth" ||
            stretchMode === "fitHeight" ||
            stretchMode === "fill") {
            $imageDom
...
dorado.widget.TemplateField = $extend([dorado.widget.Control, dorado.widget.DataControl], {
    ...
    refreshDom: function (dom) {
        ...
        if (this._template) {
            let html, context = { view: this.get("view"), control: this, entity: entity };
            context.data = entity
...
(function () {
    ...
    dorado.widget.AbstractEditor = $extend(dorado.widget.Control, {
        ...
        doRefreshData: function () {
            let p = this._property, e = this._entity;
...
dorado.widget.CheckBox = $extend(dorado.widget.AbstractDataEditor, {
    ...
    refreshDom: function (dom) {
        $invokeSuper.call(this, arguments);
        let checkBox = this, checked = checkBox._checked, caption = checkBox._caption || "";
...
dorado.widget.RadioButton = $extend(dorado.widget.Control, {
    ...
    _isReadOnly: function () {
        let radioButton = this, radioGroup = radioButton._radioGroup;
        let extraReadOnly = false;
    ...
    refreshDom: function (dom) {
        $invokeSuper.call(this, arguments);
        let radioButton = this, doms = radioButton._doms, checked = radioButton._checked, text = radioButton._text, jDom;
        if (dom) {
...
dorado.widget.RadioGroup = $extend(dorado.widget.AbstractDataEditor, {
    ...
    ATTRIBUTES: {
        ...
        radioButtons: {
            setter: function (radioButtons) {
                let radioGroup = this, oldValue = this._radioButtons, dom = radioGroup._dom;
                if (oldValue) {
    ...
    setValue: function (value) {
        let radioGroup = this, radioButtons = radioGroup._radioButtons;
        if (radioButtons) {
            let found = false;
    ...
    addRadioButton: function (radioButton, index) {
        let radioGroup = this, radioButtons = radioGroup._radioButtons, dom = radioGroup._dom, refDom;
        if (!radioButtons) {
    ...
    removeRadioButton: function (radioButton) {
        let radioGroup = this, radioButtons = radioGroup._radioButtons, index;
        if (!radioButtons) {
            return;
        }
    ...
    clearRadioButtons: function () {
        let radioGroup = this, radioButtons = radioGroup._radioButtons || [], radioButton;
        for (let i = 0; i < radioButtons.length; i++) {
    ...
    _setValue: function (radioButton) {
        let radioGroup = this, value = radioButton ? radioButton._value : null;
        if (radioGroup.currentRadioButton === radioButton) {
            return;
        }
    ...
    createDom: function () {
        let radioGroup = this, layout = radioGroup._layout, radioButtons = radioGroup._radioButtons;
    ...
    refreshDom: function (dom) {
        $invokeSuper.call(this, arguments);
        this.refreshExternalReadOnly();
        let group = this, layout = group._layout, radioButtonsRebuilded = false;
        if (group._dataSet) {
            let value, dirty;
            if (group._property) {
                ...
                if (!group._dontGenerateButtons && bindingInfo.propertyDef) {
                    let oldMapping = group._propertyDefMapping, mapping = bindingInfo.propertyDef._mapping;
    ...
    doOnResize: function () {
        let group = this, radioButtons = group._radioButtons, layout = group._layout, columnCount = group._columnCount || 3;
        if (radioButtons) {
            if (layout === "grid") {
                let width = $fly(group._dom).width(), averageWidth = Math.floor(width / columnCount);
                for (let i = 0, j = radioButtons.length; i < j; i++) {
                    let radioButton = radioButtons[i];
    ...
    doOnKeyDown: function (event) {
        ...
        let group = this, radioButtons = group._radioButtons, currentRadioButton = group.currentRadioButton, currentButtonIndex = currentRadioButton
            ? radioButtons.indexOf(currentRadioButton)
            : -1, buttonCount = radioButtons.length, newIndex, newRadioButton, retValue = true;
...
(function () {
    ...
    dorado.widget.AbstractTextBox = $extend(dorado.widget.AbstractDataEditor, {
        ...
        refreshTriggerDoms: function () {
            let triggerButtons = this._triggerButtons, triggerButton;
            if (triggerButtons) {
        ...
        refreshDom: function (dom) {
            ...
            this.resetReadOnly();
            if (this._dataSet) {
                let value, dirty, timestamp = 0;
                this._entity = null;
                if (this._property) {
                    let bindingInfo = this._bindingInfo, state, messages;
        ...
        setValidationState: function (state, messages) {
            ...
            if (this._dom) {
                let dom = this._dom, warnCls = this._className + "-warn", errorCls = this._className + "-error";
                $fly(dom)
        ...
        resetReadOnly: function () {
            ...
            if (!readOnly && this._editable) {
                ...
                if (isInputOrTextArea(this._textDom)) {
                    let triggers = this.get("trigger"), realEditable = true;
        ...
        doOnKeyDown: function (evt) {
            ...
            let retValue = true, trigger = this.get("trigger"), firstTrigger;
            if (trigger) {
        ...
        post: function (force, silent) {
            ...
            try {
                let text = this.get("text"), state, result, modified = this._lastPost !== text, validationResults;
                if (force ||
                    modified ||
                    ...
                    if (validationResults) {
                        let topValidationResult = dorado.Toolkits.getTopMessage(validationResults);
    ...
    dorado.widget.AbstractTextEditor = $extend(dorado.widget.AbstractTextBox, {
        ...
        createDom: function () {
            let text = this._text, dom = $invokeSuper.call(this);
            if (!text) {
                this.doSetText("");
            }
            return dom;
    ...
    dorado.widget.TextEditor = $extend(dorado.widget.AbstractTextEditor, {
        ...
        ATTRIBUTES: {
            ...
            value: {
                ...
                getter: function () {
                    ...
                    if (this._value !== undefined && text === this._valueText) {
                        ...
                        if (dataType) {
                            try {
                                let value = (this._value = dataType.parse(text, this._editorFocused ? this._typeFormat : this._displayFormat));
            ...
            text: {
                skipRefresh: true,
                getter: function () {
                    return ((!this.get("dataType") || this._editorFocused
                        ? this.doGetText()
                        : this._text) || "");
                },
                setter: function (text) {
            ...
            trigger: {
                getter: function (p, v) {
                    ...
                    if (trigger === undefined && this._view) {
                        let dataType = this.get("dataType"), dtCode = dataType ? dataType._code : 0;
        ...
        doOnFocus: function () {
            ...
            if (!this._realReadOnly && !this._mapping) {
                ...
                if (dataType) {
                    ...
                    if (dCode === dorado.DataType.PRIMITIVE_CHAR ||
        ...
        doOnBlur: function () {
            if (this._realReadOnly) {
                ...
                try {
                    ...
                    let text, dataType = this.get("dataType");
...
(function () {
    dorado.widget.TextArea = $extend(dorado.widget.AbstractTextEditor, {
        ...
        refreshTriggerDoms: function () {
            let triggerButtons = this._triggerButtons, triggerButton, triggerPanel = this._triggerPanel;
        ...
        resizeTextDom: function () {
            ...
            let w = this._dom.clientWidth, h = this._dom.clientHeight;
...
dorado.widget.DropDown = $extend(dorado.widget.Trigger, {
    ...
    open: function (editor) {
        ...
        let dropdown = this, editorDom = editor.getDom();
        ...
        if (!box) {
            ...
            if (box instanceof dorado.widget.Container) {
                let boxDom = box.getDom(), containerDom = box.get("containerDom");
    ...
    doLocate: function () {
        let dropdown = this, box = dropdown._box, editor = dropdown._editor;
        let editorDom = editor.getDom(), boxDom = box.getDom(), boxContainer = boxDom.parentNode;
        let $boxDom = $fly(boxDom);
        let boxContainerHeight = boxContainer.clientHeight, realMaxHeight = boxContainerHeight;
        ...
        let offsetTargetBottom = boxContainerHeight -
            offsetTargetTop -
            dropdown._currentEditorOffsetHeight;
        let align = "innerLeft", vAlign = "bottom";
        ...
        let currentBoxWidth = boxWidth, currentBoxHeight = boxHeight;
        dropdown.initDropDownBox(box, editor);
        if (!dropdown._boxVisible) {
            box._visible = false;
            box.setActualVisible(false);
        }
        let control = box.get("control"), controlDom = control ? control.getDom() : containerDom.firstChild;
        if (!dropdown._width) {
            if (controlDom) {
        ...
        let widthOverflow = boxDom.parentNode.clientWidth -
    ...
    assignValue: function (editor, entityForAssignment, eventArg) {
        ...
        let targetEntity = editor._entity || (editor._cellEditor && editor._cellEditor.data);
        if (this._assignmentMap &&
            entityForAssignment &&
            entityForAssignment instanceof Object &&
            targetEntity &&
            targetEntity instanceof Object) {
            let assignmentMap = this._assignmentMap, maps = [];
            assignmentMap = assignmentMap.replace(/,/g, ";").split(";");
            for (let i = 0; i < assignmentMap.length; i++) {
                let map = assignmentMap[i], index = map.indexOf("=");
                if (index >= 0) {
                    maps.push({
            ...
            if (selectedValue instanceof dorado.Entity ||
...
(function () {
    ...
    dorado.widget.DataMessage = $extend([dorado.widget.Control, dorado.widget.PropertyDataControl], {
        ...
        refreshSingleMessageDom: function (dom, message) {
            ...
            let iconDom = dom.firstChild, textDom = dom.lastChild;
        ...
        refreshDom: function (dom) {
            $invokeSuper.call(this, arguments);
            let entity, messages = this._messages;
...
(function () {
    ...
    dorado.widget.FormProfile = $extend([dorado.widget.Component, dorado.widget.FormConfig], {
        ...
        getConfig: function () {
            ...
            let attrs = formProfile.ATTRIBUTES, attrWatcher = formProfile.getAttributeWatcher(), config = (formProfile._config = {});
            for (let attr in attrs) {
                ...
                if (specialFormConfigProps.indexOf(attr) >= 0 &&
    ...
    dorado.widget.FormElement = $extend([dorado.widget.AbstractFormElement, dorado.widget.FormConfig], {
        ...
        createDom: function () {
            ...
            let doms = {}, dom = $DomUtils.xCreate({ tagName: "DIV", content: config }, null, doms);
        ...
        createEditor: function (editorType) {
            let editor = dorado.Toolkits.createInstance("widget", editorType, function () {
                return (dorado.Toolkits.getPrototype("widget", editorType) ||
        ...
        getEditor: function (create) {
            ...
            if (this._controlRegistered) {
                let config1 = {}, config2 = {}, attrs = control.ATTRIBUTES;
            ...
            if (!control && create) {
                ...
                if (propertyDef) {
                    if (!this._editorType) {
                        ...
                        if (propertyDataType) {
                            if (propertyDataType._code === dorado.DataType.PRIMITIVE_BOOLEAN ||
        ...
        resetBinding: function () {
            ...
            let editor = this.getEditor(false), hintControl = this.getHintControl(false);
            if (editor) {
                editor.set(config);
            }
            if (hintControl) {
        ...
        refreshDom: function (dom) {
            ...
            if (editor) {
                ...
                let autoWidth = !editor.ATTRIBUTES.width.independent &&
                    !attrWatcher.getWritingTimes("width");
                let autoHeight = !editor.ATTRIBUTES.height.independent &&
    ...
    dorado.widget.AutoForm = $extend([
        ...
        _constructor: function (config) {
            ...
            autoform.bind("onAttributeChange", function (self, arg) {
                let attr = arg.attribute;
                if (attr === "readOnly") {
                    let readOnly = autoform._readOnly, objects = autoform._bindingElements.objects;
        ...
        addElement: function (element) {
            let elements = this._elements, constraint;
        ...
        generateDefaultElements: function () {
            ...
            if (dataType && dataType instanceof dorado.EntityDataType) {
                ...
                let layout = this.get("layout");
                layout.disableRendering();
                let self = this, elements = self._elements, config;
                dataType.get("propertyDefs").each(function (propertyDef) {
                    if (!propertyDef._visible) {
                        return;
                    }
                    let name = propertyDef._name, element = elements.get(name);
                    if (!element) {
                        config = {
                            name: name,
        ...
        validate: function (silent) {
            let result = true, elements = this._elements, errorMessages;
