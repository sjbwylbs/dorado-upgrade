...
dorado.widget.tree.TreeNodeIterator = $extend(dorado.util.Iterator, {
	...
	constructor: function (root, options) {
		...
		var includeInvisibleNodes = (this.includeInvisibleNodes = options
...
dorado.widget.tree.TreeNodeRenderer = $extend(dorado.Renderer, {
	...
	doRender: function (cell, node, arg) {
		...
		if (tree._showLines) {
			var linesContainer = container.firstChild,
				lines = [];
		...
		var cls = ["collapse-button", "expand-button"],
			buttonDomIndex = tree._showLines ? 1 : 0;
		var buttonDom = container.childNodes[buttonDomIndex],
		...
		var checkable = node.get("checkable"),
			checkbox;
	...
	ATTRIBUTES: {
		...
		view: {
			setter: function (view) {
				...
				var nodes = this._identifiedNodes,
					oldView = this._view;
				for (var p in nodes) {
			...
			if (this._scrollMode !== "viewport") {
				...
				if (parentNode._expanded) {
					...
					if (nodeIndex >= 0) {
						...
						try {
							var it = new dorado.widget.tree.TreeNodeIterator(
								this._itemModel.getItems(),
								{ nextIndex: nodeIndex },
							);
							it.next();
							var nextsibling = it.next(),
								tbody = this._dataTBody,
								refRow;
						} finally {
...
dorado.widget.tree.DataBindingNode = $extend(dorado.widget.tree.DataNode, {
	...
	ATTRIBUTES: {
		...
		checked: {
			...
			setter: function (checked) {
				var currentChecked = this._checked,
	...
	_prepareChildren: function (callback) {
		...
		var bindingConfig = this._bindingConfig,
			tree = this._tree;
	...
	[dorado.widget.Tree, dorado.widget.DataControl],
	{
		...
		processDataSetMessage: function (messageCode, arg, data) {
			switch (messageCode) {
				...
				case dorado.widget.DataSet.MESSAGE_DELETED:
					...
					break;
				case dorado.widget.DataSet.MESSAGE_INSERTED:
					var parentEntity = arg.entity.parent,
						parentEntityList;
					if (parentEntity) {
		...
		processItemDrop: function (draggingInfo, evt) {
			...
			if (
				...
				var sourceNode = object,
					targetNode = targetObject;
				var sourceEntity = sourceNode.get("data"),
...
dorado.widget.DataTree.bindingConfigToPreloadConfig = function (
	...
	function getChildPreloadConfigs(bindingConfig, level) {
		...
		if (bindingConfig.childBindingConfigs) {
			for (var i = 0; i < bindingConfig.childBindingConfigs.length; i++) {
				var config = toPreloadConfig.call(
					this,
