...
dorado.widget.tree.BaseNodeList = $extend(dorado.util.KeyedArray, {
    ...
    afterInsert: function (node, ctx) {
        let parentNode = ctx.parentNode, originHasChild = ctx.originHasChild, tree = parentNode._tree;
    ...
    beforeRemove: function (node) {
        let tree = node._tree, parentNode = this.parent, index = this.indexOf(node);
    ...
    afterRemove: function (node, ctx) {
        let parentNode = ctx.parentNode, index = ctx.index, tree = parentNode._tree;
        parentNode._changeVisibleChildNodeCount(-1 - (node._expanded ? node._visibleChildNodeCount : 0));
        if (tree &&
...
dorado.widget.tree.BaseNode = $extend(dorado.widget.ViewElement, {
    ...
    ATTRIBUTES: {
        ...
        checked: {
            ...
            setter: function (checked) {
                ...
                let tree = this._tree, arg = { node: this, processDefault: true };
                if (tree) {
    ...
    _resetNodeAutoCheckedState: function () {
        ...
        if (node.get("autoCheckChildren")) {
            ...
            if (tree._autoCheckingParent && node && node.get("checkable")) {
                ...
                let checkedCount = 0, checkableCount = 0, halfCheck = false;
    ...
    expandAsync: function (callback) {
        ...
        let tree = this._tree, self = this, called = false;
        let callDefault = function (success, result) {
            if (called) {
                return;
            }
            called = true;
...
dorado.widget.tree.Node = $extend([dorado.widget.tree.BaseNode, dorado.EventSupport], {
    $className: "dorado.widget.tree.Node",
    _setTree: function (tree) {
        if (this._tree !== tree && this._id) {
            let oldTree = this._tree, oldView, newView;
            if (oldTree) {
...
dorado.widget.tree.TreeNodeIterator = $extend(dorado.util.Iterator, {
    ...
    constructor: function (root, options) {
        ...
        if (nextIndex > 0) {
            let skiped = 0, it = this._iterators.peek();
            while (skiped < nextIndex) {
                let node = it.next(), processChildren = false;
                if (node) {
                    let tmpSkiped = skiped +
                    ...
                    if (tmpSkiped < nextIndex) {
                        ...
                        if (tmpSkiped === nextIndex) {
                            while ((node._expanded || includeInvisibleNodes) &&
                                node._nodes.size > 0) {
    ...
    _findLastSubNode: function (node) {
        ...
        while ((node._expanded || this.includeInvisibleNodes) &&
            node._nodes.size > 0) {
    ...
    next: function () {
        let node, current, its = this._iterators;
...
dorado.widget.tree.TreeNodeRenderer = $extend(dorado.Renderer, {
    ...
    doRender: function (cell, node, arg) {
        let tree = node._tree, level = node.get("level"), hasChild = node.get("hasChild");
        let container = cell.tagName.toLowerCase() === "div" ? cell : cell.firstChild;
        container.style.paddingLeft = (level - 1) * tree._indent + "px";
        if (tree._showLines) {
            let linesContainer = container.firstChild, lines = [];
            linesContainer.style.width = level * tree._indent + "px";
            linesContainer.style.height = tree._rowHeight + "px";
            let n = node, i = 0;
            while (n && n._parent) {
                let p = n._parent, pNodes = p._nodes;
                if (i === 0) {
        ...
        let cls = ["collapse-button", "expand-button"], buttonDomIndex = tree._showLines ? 1 : 0;
        let buttonDom = container.childNodes[buttonDomIndex], $buttonDom = jQuery(buttonDom);
        if (hasChild) {
            if (node._expanded) {
                cls.reverse();
            }
        ...
        let checkable = node.get("checkable"), checkbox;
...
dorado.widget.AbstractTree = $extend(dorado.widget.RowList, {
    ...
    ATTRIBUTES: {
        ...
        view: {
            setter: function (view) {
                ...
                let nodes = this._identifiedNodes, oldView = this._view;
                for (let p in nodes) {
    ...
    createItemDomDetail: function (row, node) {
        let tree = node._tree, rowHeight = tree._rowHeight + "px";
        let cellConfig = {
            tagName: "TD",
        ...
        let context = {}, cell = $DomUtils.xCreate(cellConfig, null, context);
        let buttonDom = context.buttonDom, $buttonDom = jQuery(buttonDom), self = this;
        $buttonDom
            .mousedown(function () {
            return false;
        })
            .click(function () {
            let row = $DomUtils.findParent(buttonDom, function (parentNode) {
    ...
    doOnDraggingSourceMove: function (draggingInfo, evt, targetObject, insertMode, refObject, itemDom) {
        ...
        if (itemDom) {
            if (insertMode) {
                let dom = this._dom, node = $fly(itemDom).data("item");
    ...
    processItemDrop: function (draggingInfo, evt) {
        ...
        if (object instanceof dorado.widget.tree.BaseNode &&
    ...
    dorado.widget.Tree = $extend(dorado.widget.AbstractTree, {
        ...
        _refreshRearRows: function (fromRow) {
            if (fromRow && this._forceRefreshRearRows !== false) {
                let nextRow = fromRow, tbody = this._dataTBody;
                while (nextRow) {
        ...
        _insertChildNodes: function (node, row, animated, callback) {
            ...
            let tbody = this._dataTBody, refRow = row ? row.nextSibling : null;
            let it = new dorado.widget.tree.TreeNodeIterator(node), count = 0, bottom = -1;
            while (it.hasNext()) {
                let child = it.next();
            ...
            if (animated && !this._duringAnimation) {
                let indicator = this.createExpandingIndicator(), self = this;
                ...
                $fly(indicator).animate({ height: "+=" + self._rowHeight * count }, 200, "swing", function () {
                    ...
                    while (it.hasNext()) {
                        let child = it.next(), childRow = self._itemDomMap[child._uniqueId];
                        if (childRow) {
        ...
        _refreshAndScroll: function (node, mode, parentNode, nodeIndex) {
            let shouldScroll = false, itemModel = this._itemModel;
            ...
            if (this._scrollMode !== "viewport") {
                ...
                if (parentNode._expanded) {
                    ...
                    if (nodeIndex >= 0) {
                        ...
                        try {
                            let it = new dorado.widget.tree.TreeNodeIterator(this._itemModel.getItems(), { nextIndex: nodeIndex });
                            it.next();
                            let nextsibling = it.next(), tbody = this._dataTBody, refRow;
                        }
                        finally {
...
dorado.widget.tree.DataBindingNode = $extend(dorado.widget.tree.DataNode, {
    ...
    ATTRIBUTES: {
        ...
        hasChild: {
            getter: function () {
                ...
                let hasChild = false, entity = this._data;
        ...
        checked: {
            ...
            setter: function (checked) {
                let currentChecked = this._checked, bg = this._bindingConfig;
                ...
                let entity = this._data, property = bg.checkedProperty;
                if (entity && property) {
    ...
    _prepareChildren: function (callback) {
        function processBindingConfig(bindingConfig, entity, startIndex, processDefaultExpand) {
                    ...
                    if (processDefaultExpand) {
                        if (!bindingConfig.recursive) {
                            ...
                            if (bindingConfig.expandLevel) {
                                let parentNode = node._parent, i = 0;
                                while (parentNode &&
            ...
            let tree = this._tree, nodes = this._nodes, expandedNodes = {}, currentNode = tree.get("currentNode");
        ...
        let bindingConfig = this._bindingConfig, tree = this._tree;
        let isRoot = this === tree._root, data = this._data;
        if (isRoot && tree) {
        ...
        if (callback && data instanceof dorado.Entity) {
            ...
            if (bindingConfig.recursive && !isRoot) {
                asyncTasks.push(function (callback) {
                    if (processPreload) {
                        let preloadConfigs = dorado.widget.DataTree.bindingConfigToPreloadConfig(bindingConfig, 0);
                        if (preloadConfigs) {
            ...
            if (bindingConfig.childBindingConfigs) {
                for (let i = 0; i < bindingConfig.childBindingConfigs.length; i++) {
                    ...
                    asyncTasks.push(function (callback) {
                        if (processPreload) {
                            let preloadConfigs = dorado.widget.DataTree.bindingConfigToPreloadConfig(childBindingConfig, 0);
                            if (preloadConfigs) {
        ...
        $waitFor(asyncTasks, {
            callback: function (success, result) {
                let nodesTimestamp = 0, infos = [];
                if (data instanceof dorado.Entity) {
                    if (bindingConfig.recursive) {
                        let e = isRoot
                            ? data
                            : data.get(bindingConfig.childrenProperty, callback ? "auto" : "always");
                        if (e) {
                    ...
                    if (bindingConfig.childBindingConfigs) {
                        for (let i = 0; i < bindingConfig.childBindingConfigs.length; i++) {
                            let childBindingConfig = bindingConfig.childBindingConfigs[i];
                            let e = data.get(childBindingConfig.childrenProperty, callback ? "auto" : "always");
                            if (!e) {
                                continue;
                            }
                            nodesTimestamp += e.timestamp;
    ...
    doExpandAsync: function (callback) {
        if (!this._childrenPrepared) {
            let self = this, superClass = $getSuperClass();
            this._prepareChildren({
                callback: function (success, result) {
...
dorado.widget.DataTree = $extend([dorado.widget.Tree, dorado.widget.DataControl], {
    ...
    refreshDom: function (dom) {
        ...
        if (this._dataSet) {
            ...
            if (!this._root._childrenPrepared ||
                this._data !== data ||
    ...
    filterDataSetMessage: function (messageCode, arg) {
        switch (messageCode) {
            ...
            case dorado.widget.DataSet.MESSAGE_LOADING_END:
                if (arg.entityList) {
                    return (this._data === arg.entityList ||
    ...
    processDataSetMessage: function (messageCode, arg, data) {
        switch (messageCode) {
            ...
            case dorado.widget.DataSet.MESSAGE_DELETED:
                ...
                break;
            case dorado.widget.DataSet.MESSAGE_INSERTED:
                let parentEntity = arg.entity.parent, parentEntityList;
                if (parentEntity) {
    ...
    processItemDrop: function (draggingInfo, evt) {
        ...
        if (object instanceof dorado.widget.tree.DataBindingNode &&
            ...
            let sourceNode = object, targetNode = targetObject;
            let sourceEntity = sourceNode.get("data"), targetEntity = targetNode.get("data");
            ...
            if (sourceBindingConfig === bindingConfig ||
...
dorado.widget.DataTree.bindingConfigToPreloadConfig = function (bindingConfig, level) {
    ...
    function getChildPreloadConfigs(bindingConfig, level) {
        ...
        if (bindingConfig.childBindingConfigs) {
            for (let i = 0; i < bindingConfig.childBindingConfigs.length; i++) {
                let config = toPreloadConfig.call(this, bindingConfig.childBindingConfigs[i], level - 1);
