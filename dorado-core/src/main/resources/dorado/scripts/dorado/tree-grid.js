...
(function () {
    ...
    dorado.widget.treegrid.ItemModel = $extend([dorado.widget.tree.ItemModel, dorado.widget.grid.ItemModel], {
        ...
        refreshSummary: function () {
            ...
            let totalSummary = this.footerEntity._data, self = this;
            this.initSummary(totalSummary);
            for (let it = new dorado.widget.tree.TreeNodeIterator(this._root, {
    ...
    dorado.widget.treegrid.RowRenderer = $extend(dorado.widget.grid.DefaultRowRenderer, {
    ...
    dorado.widget.treegrid.TreeColumnCellRenderer = $extend([
        ...
        doRender: function (dom, arg) {
            if (!dom.firstChild || dom.firstChild.className !== "d-tree-node") {
                let tree = arg.grid, rowHeight = tree._rowHeight + "px";
                let cellConfig = {
                    tagName: "DIV",
    ...
    dorado.widget.treegrid.TreeColumnCellEditor = $extend(dorado.widget.grid.DefaultCellEditor, {
        ...
        resize: function () {
            ...
            let cell = this.cell, $gridDom = jQuery(this.grid.getDom());
            if (!dom || !cell) {
                return;
            }
            ...
            let l = lp - offsetGrid.left - $gridDom.edgeLeft(), t = offsetCell.top -
                offsetGrid.top -
                $gridDom.edgeTop() +
                $cellDom.edgeTop(), w = cell.offsetWidth, h = cell.offsetHeight;
    ...
    dorado.widget.TreeGrid = $extend([dorado.widget.AbstractGrid, dorado.widget.AbstractTree], {
        ...
        refreshDom: function (dom) {
            if (this._treeColumn) {
                let cols = this.findColumns(this._treeColumn), col = cols[0];
                if (!col) {
        ...
        refreshEntity: function (entity) {
            let itemId = entity._uniqueId, row, innerGrid;
            if (this._domMode === 2) {
        ...
        _doOnKeyDown: function (evt) {
            let retValue = true, currentChanged;
            let items = this._itemModel.getItems(), currentNode = this.get("currentNode");
            switch (evt.keyCode) {
                case 36:
                    if (evt.ctrlKey) {
...
dorado.widget.DataTreeGrid = $extend([dorado.widget.TreeGrid, dorado.widget.DataTree], {
    ...
    refreshDom: function (dom) {
        ...
        if (this._dataSet) {
            let dataType, data = this.getBindingData({
