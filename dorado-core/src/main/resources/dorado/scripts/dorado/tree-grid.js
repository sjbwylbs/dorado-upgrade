		...
		],
		{
			...
			doRender: function (dom, arg) {
				if (!dom.firstChild || dom.firstChild.className !== "d-tree-node") {
					...
					var buttonDom = context.buttonDom,
	...
	dorado.widget.treegrid.TreeColumnCellEditor = $extend(
		dorado.widget.grid.DefaultCellEditor,
		{
			...
			resize: function () {
				...
				var offsetGrid = $gridDom.offset(),
					offsetCell = $fly(cell).offset();
				var lp = $cellDom.find(".node-label").offset().left;
				var l = lp - offsetGrid.left - $gridDom.edgeLeft(),
					t =
						offsetCell.top -
						offsetGrid.top -
						$gridDom.edgeTop() +
	...
	dorado.widget.TreeGrid = $extend(
		[dorado.widget.AbstractGrid, dorado.widget.AbstractTree],
		{
			...
			refreshDom: function (dom) {
				if (this._treeColumn) {
					var cols = this.findColumns(this._treeColumn),
						col = cols[0];
					if (!col) {
