...
(function () {
    dorado.widget.Action = $extend(dorado.widget.Component, {
        $className: "dorado.widget.Action", ATTRIBUTES: {
            caption: {}, icon: {}, iconClass: {}, tip: {}, disabled: {
                ...
                writeBeforeReady: true, setter: function (hotkey) {
                    this._hotkey = hotkey;
                    let self = this;
                    if (hotkey) {
        ...
        }, doExecute: dorado._NULL_FUNCTION, execute: function (callback) {
            if (this._disabled) {
    ...
    dorado.widget.ActionSupport = $class({
        $className: "dorado.widget.ActionSupport", ATTRIBUTES: {
            action: {
                componentReference: true, setter: function (action) {
...
(function () {
    ...
    dorado.widget.AjaxAction = $extend(dorado.widget.AsyncAction, {
        ...
        doExecuteAsync: function (callback) {
            ...
            ajax.request(ajaxOptions, {
                scope: this, callback: function (success, result) {
                    if (success) {
                        let data;
    ...
    dorado.widget.UpdateAction = $extend(dorado.widget.AsyncAction, {
        $className: "dorado.widget.UpdateAction", ATTRIBUTES: {
            ...
            let dataItems = [], updateInfos = [], aliasMap = {}, hasUpdateData = false, updateItems = this.get("updateItems");
            ...
            for (let i = 0; i < dataItems.length; i++) {
                let dataItem = dataItems[i], updateItem = dataItem.updateItem, data = dataItem.data, options = updateItem.options;
    ...
    dorado.widget.LongTask = $extend(dorado.widget.Action, {
        ...
        connect: function () {
            ...
            let socket = task._socket = dorado.Socket.connect({
                service: "dorado.connectLongTask", parameter: task._taskName, onReceive: function (self, arg) {
        ...
        doStart: function (callback) {
            let task = this;
            let ajaxOptions = task.getAjaxOptions("dorado.startLongTask"), ajax = dorado.util.AjaxEngine.getInstance(ajaxOptions);
            task._starting = true;
            ajax.request(ajaxOptions, {
                callback: function (success, result) {
                    try {
...
(function () {
    let BUTTON_CLICK_CLASS = "-click", BUTTON_HOVER_CLASS = "-hover", BUTTON_TOGGLED_CLASS = "-toggled", BUTTON_DISABLED_CLASS = "-disabled";
    dorado.widget.SimpleButton = $extend(dorado.widget.AbstractButton, {
        ...
        doShowMenu: function () {
            let button = this, menu = button._menu, dom = button._dom, cls = button._className, toggledClass = button._toggledClassName;
            if (menu) {
        ...
        createDom: function () {
            let button = this, dom = document.createElement("div"), hoverClass = button._hoverClassName, mouseDownClass = button._mouseDownClassName;
        ...
        refreshDom: function (dom) {
            $invokeSuper.call(this, arguments);
            let button = this, cls = button._className, disabledClass = button._disabledClassName, toggledClass = button._toggledClassName;
    ...
    dorado.widget.SimpleIconButton = $extend(dorado.widget.SimpleButton, {
        ...
        refreshDom: function (dom) {
            $invokeSuper.call(this, arguments);
            let button = this, cls = button._className, action = button._action || {}, icon = button._icon || action._icon, iconClass = button._iconClass || action._iconClass;
...
(function () {
    ...
    dorado.widget.IFrame = $extend(dorado.widget.Control, {
        ...
        }, EVENTS: { onLoad: {} }, getDomainInfo: function (domain) {
...
dorado.widget.CardBook = $extend(dorado.widget.Control, {
    $className: "dorado.widget.CardBook", ATTRIBUTES: {
        className: { defaultValue: "d-cardbook" }, currentControl: {
            skipRefresh: true, setter: function (control) {
            ...
            writeOnce: true, innerComponent: "", setter: function (value) {
                if (value) {
...
dorado.widget.ProgressBar = $extend([dorado.widget.Control, dorado.widget.PropertyDataControl], {
    ...
    refreshDom: function (dom) {
        ...
        let min = bar._minValue, max = bar._maxValue, doms = bar._doms, percent = value / (max - min), showText = bar._showText, pattern = bar._textPattern || "";
        if (percent === 0) {
...
(function () {
    ...
    dorado.widget.Tip = $extend([dorado.widget.Control, dorado.widget.FloatControl], {
        ...
        refreshDom: function (dom) {
            $invokeSuper.call(this, arguments);
            let tip = this, text = (tip._text === undefined) ? "" : tip._text, doms = tip._doms, arrowDirection = tip._arrowDirection, content = this._content;
            let classNames = [];
    ...
    let getRegionOffsets = function (region1, region2) {
        return {
    ...
    let intersect = function (element1, element2) {
        if (element1 == null) {
            return false;
        }
    ...
    dorado.widget.NotifyTipManager = {
        ...
        getAvialablePosition: function (tip) {
            let docWidth = $fly(window).width(), dom = tip._dom, width = dom.offsetWidth, height = dom.offsetHeight, left, top, region, position = this.position;
            if (position === "tr") {
