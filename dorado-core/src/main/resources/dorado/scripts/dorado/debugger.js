...
(function () {
    ...
    dorado.Debugger = {
        ...
        format2HTML: function (text) {
            let reg1 = new RegExp("&", "g"), pattern = new RegExp('[<>"\n\t]', "g"), map = {
                "<": "&lt;",
                ">": "&gt;",
                "\n": "<br/>",
                "\t": "&nbsp;&nbsp;",
...
dorado.debug.initProcedures.push(function () {
    ...
    dorado.debug.Logger = $extend(dorado.widget.Control, {
        ...
        ATTRIBUTES: {
            ...
            level: {
                ...
                setter: function (value) {
                    let logger = this, oldValue = logger._level, dom = logger._dom, i;
                    value = value || "debug";
                    if (oldValue && dom) {
        ...
        clear: function () {
            let logger = this, logs = logger._logs;
            if (logs) {
                logs = [];
                let dom = logger._dom;
                if (dom) {
        ...
        log: function (msg, level) {
            ...
            logs.push(level + ":" + msg);
            if (logger._rendered) {
                let dom = logger._dom, logDom = $DomUtils.xCreate({
                    tagName: "div",
        ...
        scrollToEnd: function () {
            let logger = this, dom = logger._dom;
            if (dom && dom.style.display !== "none") {
                let scrollHeight = dom.scrollHeight, offsetHeight = dom.offsetHeight;
    ...
    dorado.debug.ConsolePanel = $extend(dorado.widget.Container, {
        ...
        constructor: function (config) {
            config = config || {};
            let panel = this, logger = new dorado.debug.Logger();
...
dorado.debug.initProcedures.push(function () {
    ...
    let mixIf = function (target, source) {
        if (target && source) {
            for (let prop in source) {
        ...
        constructor: function (config) {
            ...
            mixIf(config, {
                ...
                beforeExpand: function (self, arg) {
                    let node = arg.node, id = node.get("data.label"), value = node.get("data.value");
                ...
                onCellValueEdit: function (self, arg) {
                    ...
                    if (entity) {
                        let name = entity.get("label"), value = entity.get("value"), sourceObject = self._sourceObject;
                        if (sourceObject &&
    ...
    dorado.debug.ControlsPanel = $extend(dorado.widget.Panel, {
        ...
        constructor: function (config) {
            ...
            let controlsTree = new dorado.widget.Tree({
                ...
                beforeExpand: function (self, arg) {
                    let node = arg.node, value = node.get("data.value");
        ...
        reload: function () {
            let panel = this, controlsTree = panel._controlsTree, topView = $topView, nodes = panel.getTreeNodes(topView._children, true);
        ...
        getTreeNodes: function (object, onlyview) {
                    ...
                    if (object instanceof dorado.util.KeyedArray) {
                        ...
                        if (object instanceof dorado.util.KeyedList) {
                            ...
                            if (object instanceof dorado.EntityList) {
                                ...
                                if (object instanceof dorado.Entity) {
                                    let data = object.getData(), prop, value, hasChild = false;
                                    for (prop in data) {
                                        try {
            ...
            function getCollectionChildren(object, onlyview) {
                ...
                function doGetNodes(obj) {
                    ...
                    let className = obj.constructor.className || "", $type = className.replace("dorado.widget.", "");
                    let label = $type;
    ...
    dorado.debug.EventsPanel = $extend(dorado.widget.Panel, {
        ...
        constructor: function (config) {
            ...
            let eventsTree = new dorado.widget.Tree({
                ...
                onCurrentChange: function (self, arg) {
                    let current = arg.newCurrent, value, content = "", preview = panel._eventPreviewPanel;
                    if (current) {
                        ...
                        listener: {
                            onClick: function () {
                                ...
                                if (current && current.get("level") === 1) {
                                    let name = current.get("label"), object = current.get("data.object");
                        ...
                        listener: {
                            onClick: function () {
                                ...
                                if (current && current.get("level") === 2) {
                                    let fn = current.get("data.value"), parent = current.get("parent"), name = parent.get("label"), object = parent.get("data.object");
                    ...
                    },
                    {
                        ...
                        listener: {
                            onClick: function () {
                                let current = eventsTree.get("currentNode");
                                if (current && current.get("level") === 2) {
                                    let fn = current.get("data.value"), parent = current.get("parent"), object = parent.get("data.object");
    ...
    let viewChildrenLoaded = false, viewPanel = new dorado.debug.ViewPanel();
    dorado.Debugger.registerModule({
        $type: "Control",
        name: "view",
        caption: "View",
...
dorado.debug.initProcedures.push(function () {
    ...
    dorado.debug.HotkeysPanel = $extend(dorado.widget.SplitPanel, {
        ...
        constructor: function (config) {
            ...
            let keyList = new dorado.widget.ListBox({
                style: "border: 0",
                onCurrentChange: function (self, arg) {
                    let item = self.getCurrentItem(), code = "";
    ...
    let hotkeysPanel = new dorado.debug.HotkeysPanel(), loaded = false;
    dorado.Debugger.registerModule({
        $type: "Control",
        name: "hotkey",
        caption: "Hotkey",
