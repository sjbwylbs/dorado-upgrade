...
(function () {
    ...
    dorado.widget.TagEditor = $extend(dorado.widget.AbstractTextBox, {
        ...
        createDom: function () {
            let context = {}, dom = $DomUtils.xCreate({
                tagName: "DIV",
        ...
        createTextDom: function () {
            let tagEditor = this, editorConfig = {
                tagName: "INPUT",
                className: "editor",
                size: 1,
                style: {},
                ...
                onkeydown: function (evt) {
                    ...
                    switch (evt.keyCode || evt.which) {
                        case 8:
                            ...
                            break;
                        case 13:
                            let defaultTrigger = tagEditor._defaultTrigger, shouldPost = true;
        ...
        refreshTagDoms: function () {
            let inputDom = this._inputDom, tags = this._value || [];
            inputDom.value = "";
            inputDom.style.width = "";
            let i = 0, tagDom, unknowns = this._unknowns || [], requiredTags = this._requiredTags || [];
            for (; i < tags.length; i++) {
                let tag = tags[i];
        ...
        doPost: function () {
            ...
            let value = this._value, unknowns = this._unknowns;
        ...
        getAvailableTags: function () {
            if (this._availableTags) {
                ...
                if (this._availableTagsDataSet) {
                    let tags = this._availableTagsDataSet.queryData(this._availableTagsDataPath);
        ...
        addTags: function (newTags, shouldCheck) {
            let value = this._value || [], indexs = [], unknowns = this._unknowns || [], availableTags = this.getAvailableTags();
            for (let i = 0; i < newTags.length; i++) {
                let tag = newTags[i], isUnknown = false, eventArg = { tag: tag, processDefault: true };
        ...
        doRemoveTag: function (tag) {
            let tagEditor = this, value = tagEditor._value || [];
            let index = value.indexOf(tag);
            if (index >= 0) {
