        ...
        if (callbacks) {
            ...
            this.doGetAsync({
                ...
                callback: function (success, result) {
                    ...
                    let errors, callbacks = this._waitingCallbacks;
...
(function () {
    dorado.DataProvider = $class({
        ...
        getResult: function (arg) {
            let ajaxOptions = this.getAjaxOptions(arg), ajax = dorado.util.AjaxEngine.getInstance(ajaxOptions);
            let result = ajax.requestSync(ajaxOptions);
            if (result.success) {
                let json = result.getJsonData(), data;
        ...
        getResultAsync: function (arg, callback) {
            let ajaxOptions = this.getAjaxOptions(arg), ajax = dorado.util.AjaxEngine.getInstance(ajaxOptions);
            let dataType = this.dataType, supportsEntity = this.supportsEntity, dataTypeRepository = this.dataTypeRepository;
    ...
    dorado.AjaxDataProvider = $extend(dorado.DataProvider, {
        ...
        getAjaxOptions: function (arg) {
            let options = dorado.Object.apply({}, this._baseOptions), jsonData = (options.jsonData = {});
...
(function () {
    dorado.DataResolver = $class({
        ...
        resolve: function (arg) {
            ...
            let result = ajax.requestSync(ajaxOptions);
            if (result.success) {
                let result = result.getJsonData(), returnValue = result ? result.returnValue : null;
                if (returnValue &&
        ...
        resolveAsync: function (arg, callback) {
            let ajaxOptions = this.getAjaxOptions(arg), supportsEntity = this.supportsEntity, ajax = dorado.util.AjaxEngine.getInstance(ajaxOptions);
            let message = this.message;
            if (message == null) {
                message = ajaxOptions.message;
            }
            ...
            ajax.request(ajaxOptions, {
                ...
                callback: function (success, result) {
                    if (success) {
                        ...
                        $callback(callback, true, { returnValue: returnValue, entityStates: result.entityStates }, { scope: this });
                    }
                    else {
                        $callback(callback, false, result.exception, { scope: this });
                    }
...
(function () {
    ...
    dorado.EntityDataType = $extend([dorado.DataType, dorado.EventSupport], {
        ...
        fireEvent: function () {
            let view = this.get("view"), oldView = window.view;
            window.view = view;
            try {
        ...
        updateWrapperType: function () {
            let wrapperType = this._wrapperType, wrapperPrototype = wrapperType.prototype;
            this._propertyDefs.each(function (pd) {
                let name = pd._name;
    ...
    dorado.datatype.DateDataType = $extend(DataType, {
        ...
        doToText: function (data, argument) {
            return data != null && data instanceof Date
                ? data.formatDate(argument || $setting["common.defaultDisplayDateFormat"])
                : "";
        },
    });
    ...
    let datetime = (dorado.$DateTime = new dorado.datatype.DateDataType("DateTime"));
    datetime._code = DataType.DATETIME;
    datetime.doToText = function (data, argument) {
        return data != null && data instanceof Date
            ? data.formatDate(argument || $setting["common.defaultDisplayDateTimeFormat"])
            : "";
    };
...
(function () {
    ...
    function newAggDataType(name, subId) {
        let dataType = new AggregationDataType(name, dorado.LazyLoadDataType.create(this, subId));
    ...
    dorado.LazyLoadDataType.create = function (dataTypeRepository, id) {
        ...
        if (origin instanceof dorado.DataType) {
            ...
            if (origin && origin !== DataTypeRepository.UNLOAD_DATATYPE) {
                ...
                if (subId) {
                    let aggDataType = newAggDataType.call(dataTypeRepository, name, subId);
    ...
    dorado.DataTypePipe = $extend(dorado.DataPipe, {
        ...
        doGetAsync: function (callback) {
            let ajax = dorado.util.AjaxEngine.getInstance(this.loadOptions), dataTypeRepository = this.dataTypeRepository;
            if (callback) {
                ...
                ajax.request(this.getAjaxOptions(), {
                    ...
                    callback: function (success, result) {
                        if (success) {
                            let json = result.getJsonData(), dataTypeJson, context;
                            if (json.$context) {
                ...
                let jsonData = result.getJsonData(), dataType;
    ...
    dorado.DataTypeRepository = DataTypeRepository = $extend(dorado.EventSupport, {
        ...
        parseSingleDataType: function (jsonData) {
            let dataType, name = jsonData.name, type = jsonData.$type;
        ...
        parseJsonData: function (jsonData) {
            let n = 0, dataTypeMap = this._dataTypeMap, dataType;
        ...
        get: function (name, loadMode) {
            let id = name, name = dorado.DataUtil.extractNameFromId(id);
    ...
    dorado.PropertyDef = $extend([dorado.AttributeSupport, dorado.EventSupport], {
            ...
            validators: {
                setter: function (value) {
                    ...
                    for (let i = 0; i < value.length; i++) {
                        ...
                        if (v._propertyDef) {
                            throw new dorado.Exception('Validator alreay belongs to another PropertyDef "' +
        ...
        doSet: function (attr, value, skipUnknownAttribute, lockWritingTimes) {
            ...
            if (this._parent) {
                ...
                if (def && def.notifyObservers) {
                    let observers = this._parent._observers, observer;
        ...
        getMappedKey: function (value) {
            if (!this._mappingRevIndex) {
                let index = (this._mappingRevIndex = {}), mapping = this._mapping;
                for (let i = 0; i < mapping.length; i++) {
    ...
    dorado.Entity = $class({
        ...
        constructor: function (data, dataTypeRepository, dataType) {
            ...
            if (dataType) {
                ...
                for (let i = 0, len = pdItems.length; i < len; i++) {
                    let pd = pdItems[i];
                    if (SHOULD_PROCESS_DEFAULT_VALUE &&
        ...
        _get: function (property, propertyDef, callback, loadMode) {
            function transferAndReplaceIf(entity, propertyDef, value, replaceValue) {
                if (value &&
                    typeof (value instanceof dorado.Entity || value instanceof dorado.EntityList) &&
                    value.parent === entity) {
                    return value;
                }
                ...
                if (replaceValue) {
                    ...
                    if (oldValue !== value) {
                        ...
                        if (oldValue instanceof dorado.Entity ||
            ...
            let value = this._data[property], invokeCallback = true;
            if (value === undefined) {
                if (propertyDef) {
                        ...
                        if (pipe) {
                            ...
                            if (callback || loadMode === "auto") {
                                ...
                                pipe.getAsync({
                                    scope: this,
                                    callback: function (success, result) {
                                        let dummyData = this._data[property], dummyValue;
        ...
        getText: function (property, loadMode) {
            ...
            if (this.ignorePropertyPath) {
                ...
                let properties = property.split("."), result = this;
                for (let i = 0; i < properties.length; i++) {
                    property = properties[i];
        ...
        storeOldData: function () {
            ...
            let data = this._data, oldData = (this._oldData = {});
            for (let p in data) {
                if (data.hasOwnProperty(p)) {
                    let value = data[p];
                    if (value != null && value.isDataPipeWrapper) {
        ...
        _validateProperty: function (dataType, propertyDef, propertyInfo, value, preformAsyncValidator) {
            let messages = [], property = propertyDef._name, validating, propertyDataType = propertyDef.get("dataType");
            if (propertyDef._required && !dataType._validatorsDisabled) {
                ...
                if (propertyDef._validators) {
                    for (let i = 0; i < propertyDef._validators.length; i++) {
                        if (propertyDef._validators[i] instanceof
            ...
            if (propertyDef._validators && !dataType._validatorsDisabled) {
                let entity = this, currentValue = value, validateArg = { property: property, entity: entity }, oldData = this._oldData;
                ...
                for (let i = 0; i < propertyDef._validators.length; i++) {
                    ...
                    if (!validator._revalidateOldValue &&
                        oldData &&
                        ...
                        validator.validate(valueForValidator, validateArg, {
                            callback: function (success, result) {
                                ...
                                if (success) {
                                    ...
                                    let messages = dorado.Toolkits.trimMessages(result, DEFAULT_VALIDATION_RESULT_STATE);
                                    if (originMessages) {
        ...
        _set: function (property, value, propertyDef) {
            ...
            let propertyInfoMap = this._propertyInfoMap, propertyInfo = propertyInfoMap[property];
            if (!propertyInfo) {
            ...
            if (value instanceof dorado.Entity ||
                ...
                if (value &&
                    ...
                    value = dorado.Object.apply({ $state: dorado.Entity.STATE_NEW }, value);
                }
            }
        ...
        _dispatchOperationToSubEntity: function (property, create, method, args) {
            let i = property.indexOf(".");
            let property1 = property.substring(0, i), property2 = property.substring(i + 1);
        ...
        setText: function (property, text) {
            if (!this.ignorePropertyPath && property.indexOf(".") > 0) {
                ...
                let propertyDef = this.getPropertyDef(property), value = text;
                if (propertyDef) {
        ...
        cancel: function (deep) {
            ...
            if (this.state === dorado.Entity.STATE_NEW &&
                ...
                if (this.state !== dorado.Entity.STATE_NONE) {
                    ...
                    if (oldState === dorado.Entity.STATE_DELETED &&
                        this.parent &&
                        this.parent instanceof dorado.EntityList) {
                        let entityList = this.parent;
        ...
        createBrother: function (data, detached) {
            ...
            let brother = new dorado.Entity(null, this.dataTypeRepository, this.dataType);
            if (data) {
                brother.set(data);
            }
        ...
        createChild: function (property, data, detached) {
            ...
            if (this.dataType) {
                ...
                let elementDataType = propertyDef.get("dataType"), aggregationDataType;
                if (elementDataType &&
        ...
        toJSON: function (options, context) {
            let result = {};
            let includeUnsubmittableProperties, includeReferenceProperties, simplePropertyOnly, generateDataType, generateState, generateEntityId, generateOldData, properties, entityFilter;
            ...
            let data = this._data, oldData = this._oldData, oldDataHolder;
            for (let property in data) {
                if (!data.hasOwnProperty(property)) {
                    continue;
                }
                ...
                if (!includeUnsubmittableProperties &&
                    propertyDef &&
        ...
        getValidateState: function (property) {
            let state = "unvalidate", map = this._propertyInfoMap;
            if (map) {
                let propertyInfo = map[property];
                if (propertyInfo) {
                    if (propertyInfo.validating) {
        ...
        validate: function (options) {
            ...
            let simplePropertyOnly = options && options.validateSimplePropertyOnly === false ? false : true;
            let preformAsyncValidator = options
                ? options.preformAsyncValidator
                : false;
            let context = options ? options.context : null;
            let result, topResult, resultCode, topResultCode = -1, hasValidated = false;
            if (force) {
                if (property) {
            ...
            let dataType = this.dataType, propertyInfoMap = this._propertyInfoMap;
            if (context) {
                context.info = [];
                context.ok = [];
                context.warn = [];
            ...
            if (dataType) {
                let entity = this;
                let doValidate = function (pd) {
                    let property = pd._name, propertyInfo = propertyInfoMap[property];
                    if (property.charAt(0) === "$") {
                        return;
                    }
                    if (propertyInfo) {
                    ...
                    hasValidated = true;
                    let messages = entity._validateProperty(dataType, pd, propertyInfo, value, preformAsyncValidator);
                    if (context && messages) {
            ...
            if (!simplePropertyOnly) {
                let data = this._data;
                let doValidateEntity = function (p) {
                    let value = data[p];
                    if (value instanceof dorado.Entity) {
                        if (context) {
                            options.context = {};
        ...
        flush: function (callback) {
            ...
            let arg = { parameter: this.parameter }, oldSupportsEntity = this.dataProvider.supportsEntity;
...
(function () {
    dorado.EntityList = $class({
        ...
        _findPreviousEntry: function (entry, loadPage, pageNo) {
            let previous = entry ? entry.previous : null, pages = this._pages, pageEntry;
        ...
        _findNextEntry: function (entry, loadPage) {
            let next = entry ? entry.next : null, pages = this._pages, pageEntry;
            ...
            if (pageNo > 0 && pageNo <= this.pageCount) {
                ...
                if (!page && loadPage) {
                    ...
                    if (!this._pages.size) {
                        ...
                        let it = this._pages.iterator(), refPage, tempPage;
                        it.last();
        ...
        setCurrent: function (current) {
            ...
            let dataType = this.dataType, elementDataType;
            if (dataType) {
            ...
            if (parent != null &&
                ...
                if (parentPropertyDef._required) {
                    if (mode === "insert" ||
                        ...
                        let propertyInfoMap = parent.parent._propertyInfoMap, propertyInfo = propertyInfoMap[parentProperty];
                        if (propertyInfo) {
        ...
        remove: function (entity, detach) {
            ...
            let dataType = entity.dataType, simpleDetach = entity.state === dorado.Entity.STATE_DELETED;
            if (!simpleDetach) {
                if (dataType) {
        ...
        createChild: function (data, detached) {
            ...
            let child = new dorado.Entity(null, this.dataTypeRepository, elementDataType);
            if (data) {
                child.set(data);
            }
            if (!detached) {
        ...
        _fillPage: function (page, jsonArray, changeCurrent, fireEvent) {
            ...
            try {
                let elementDataType = this.elementDataType, eventArg;
        ...
        cancel: function (deep) {
            let it = this.iterator(true), changed = false;
            while (it.hasNext()) {
                let entity = it.next();
        ...
        clone: function (deep) {
            ...
            let cloned = new dorado.EntityList(null, this.dataTypeRepository, this.dataType);
    ...
    dorado.EntityList.EntityListIterator = $extend(dorado.util.Iterator, {
        ...
        _findFromPage: function (pageNo, reverse) {
            let result = null, entityList = this._entityList, pageCount = entityList.pageCount;
            let page = entityList.getPage(pageNo, !this._simulateUnloadPage);
            if (page && page.loaded) {
                let entry = reverse ? page.last : page.first;
                while (entry) {
                    if (this._includeDeletedEntity ||
        ...
        _findNeighbor: function (entry, pageNo, reverse) {
            ...
            if (entry == null && !this._fixedPageNo) {
                if (this._includeUnloadPage) {
                    ...
                    let entityList = this._entityList, page = oldEntry.data.page;
        ...
        _find: function (reverse) {
            ...
            let result = this._findNeighbor(fromEntry, fromEntry.data.page.pageNo, reverse);
            if (reverse) {
    ...
    LoadPagePipe = $extend(dorado.DataPipe, {
        ...
        constructor: function (entityList, pageNo) {
            this.entityList = entityList;
            let dataType = entityList.dataType, view;
            if (dataType) {
        ...
        invokeDataProvider: function (async, callback) {
            let dataProvider = this.entityList.dataProvider, dataProviderArg = this.dataProviderArg, oldSupportsEntity = dataProvider.supportsEntity;
    ...
    dorado.DataPath = $class({
        ...
        compile: function () {
            ...
            for (let i = 0; i < compiledPath.length; i++) {
                ...
                if ((!section.property || section.property === "*") &&
                    !section.args &&
                    !section.conditions) {
            ...
            try {
                ...
                if (entities instanceof dorado.EntityList) {
                    if (section.visibility === 2) {
                        ...
                        let includeDeleted = section.visibility === 1 ||
                            section.visibility === 3 ||
                            section.visibility === 6;
        ...
        evaluate: function (data, options) {
            let firstResultOnly, acceptAggregation = false, loadMode;
            if (options === true) {
            ...
            let context = new dorado.DataPathContext(this._compiledPath, firstResultOnly);
    ...
    dorado.DataUtil = {
        ...
        convertIfNecessary: function (data, dataTypeRepository, dataType) {
            ...
            if (data instanceof dorado.Entity || data instanceof dorado.EntityList) {
                ...
                if (data.dataType instanceof dorado.AggregationDataType &&
        ...
        sort: function (array, sortParams, comparator) {
            array.sort(function (item1, item2) {
                ...
                for (let i = 0; i < sortParams.length; i++) {
                    let sortParam = sortParams[i], property = sortParam.property;
                    let value1, value2;
                    if (property) {
                        value1 =
                    ...
                    if (value1 == null) {
                        ...
                        if (value2 == null) {
                            ...
                            if (typeof value1 === "string" &&
                                ...
                                let isSimpleChar1 = value1.charCodeAt(0) > 255, isSimpleChar2 = value2.charCodeAt(0) > 255;
...
dorado.validator.RequiredValidator = $extend(dorado.validator.BaseValidator, {
    ...
    doValidate: function (data, arg) {
        let valid = data != null && data !== undefined && data !== "", message = "";
        if (valid) {
...
dorado.validator.LengthValidator = $extend(dorado.validator.BaseValidator, {
    ...
    doValidate: function (data, arg) {
        ...
        let invalid, message = "", len = data.length;
...
dorado.validator.CharLengthValidator = $extend(dorado.validator.BaseValidator, {
    ...
    doValidate: function (data, arg) {
        ...
        let invalid, message = "", len = getBytesLength(data);
        if (this._minLength > 0 && len < this._minLength) {
            invalid = true;
            message += $resource("dorado.data.ErrorContentTooShort", this._minLength);
        }
...
dorado.validator.RangeValidator = $extend(dorado.validator.BaseValidator, {
    ...
    doValidate: function (data, arg) {
        let invalidMin, invalidMax, message = "", subMessage = "", data = typeof data === "number" ? data : parseFloat(data);
...
dorado.validator.EnumValidator = $extend(dorado.validator.BaseValidator, {
    ...
    doValidate: function (data, arg) {
        ...
        if (this._enumValues instanceof Array &&
...
dorado.validator.RegExpValidator = $extend(dorado.validator.BaseValidator, {
    ...
    doValidate: function (data, arg) {
        ...
        let whiteRegExp = toRegExp(this._whiteRegExp), blackRegExp = toRegExp(this._blackRegExp);
