...
(function () {
    let mround = function (r) {
        ...
        clearTimeout, START_EV = hasTouch ? "touchstart" : "mousedown", MOVE_EV = hasTouch ? "touchmove" : "mousemove", END_EV = hasTouch ? "touchend" : "mouseup", RESIZE_EV = "onorientationchange" in window ? "orientationchange" : "resize", CANCEL_EV = hasTouch ? "touchcancel" : "mouseup", WHEEL_EV = vendor === "Moz" ? "DOMMouseScroll" : "mousewheel", trnOpen = "translate" + (has3d ? "3d(" : "("), trnClose = has3d ? ",0)" : ")";
    let RE_DEFAULT_UNIT = /^width|height|top|right|bottom|left|margin.*|padding.*|border-.*$/i, DEFAULT_UNIT = "px";
    let VENDOR_PROPERTY = /^transition|transitionProperty|transitionDuration|transformOrigin|transitionTimingFunction|transform|backgroundClip|boxSizing|borderRadius|transitionDelay$/i;
    let toCamel = function (string) {
        return string.replace(/(-\S)/g, function ($1) {
            return $1.toUpperCase().substring(1, 2);
        });
    };
    let setStyle = vendor === "ms"
    ...
    let scrollbarProperMap = { hori: "horiBar", vert: "vertBar" }, directionMap = { vert: "v", hori: "h" };
    ...
    let iScroll = function (el, options) {
        let scroll = this, doc = document, i;
    ...
    iScroll.prototype = {
        ...
        updateChildrenStyle: function (prop, value) {
            let scroll = this, children = scroll.element.children;
        ...
        getChildrenTransform: function () {
            let scroll = this, children = scroll.element.children, result = [];
            for (let i = 0, l = children.length; i < l; i++) {
                let child = children[i];
                if (child.nodeType === 3 ||
                    child.className.indexOf("ignored") !== -1 ||
        ...
        _refreshScrollbar: function (direction) {
            let scroll = this, doc = document, dom, bar = scroll[scrollbarProperMap[direction]];
        ...
        _refreshScrollbarDom: function (direction, hidden) {
            let scroll = this, position = direction === "hori" ? scroll.x : scroll.y, size, scrollbarMap = { hori: "showHoriScrollbar", vert: "showVertScrollbar" };
            ...
            setStyle(bar.indicator, "transform", trnOpen +
        ...
        _start: function (event) {
            let scroll = this, point = hasTouch ? event.touches[0] : event, matrix, x, y;
            ...
            if (scroll.options.momentum && firstChild) {
                if (scroll.options.useTransform) {
                    matrix = getComputedStyle(firstChild, null)[vendor + "Transform"].replace(/[^0-9-.,]/g, "")
                        .split(",");
                    x = matrix[4] * 1;
                    y = matrix[5] * 1;
                    if (isNaN(x)) {
        ...
        _move: function (event) {
            let scroll = this, point = hasTouch ? event.touches[0] : event, deltaX = point.pageX - scroll.pointX, deltaY = point.pageY - scroll.pointY, newX = scroll.x + deltaX, newY = scroll.y + deltaY, timestamp = event.timeStamp || Date.now();
        ...
        _end: function (event) {
            ...
            let scroll = this, point = hasTouch ? event.changedTouches[0] : event, target, ev, momentumX = { dist: 0, time: 0 }, momentumY = { dist: 0, time: 0 }, duration = (event.timeStamp || Date.now()) - scroll.startTime, newPosX = scroll.x, newPosY = scroll.y, distX, distY, newDuration;
            ...
            if (duration < 300 && scroll.options.momentum) {
                momentumX = newPosX
                    ? scroll._momentum(newPosX - scroll.startX, duration, -scroll.x, scroll.scrollTargetWidth - scroll.elementWidth + scroll.x, scroll.options.bounce ? scroll.elementWidth : 0)
                    : momentumX;
                momentumY = newPosY
                    ? scroll._momentum(newPosY - scroll.startY, duration, -scroll.y, scroll.maxScrollY < 0
                        ? scroll.scrollTargetHeight -
                            scroll.elementHeight +
                            scroll.y -
                            scroll.minScrollY
                        : 0, scroll.options.bounce ? scroll.elementHeight : 0)
                    : momentumY;
                newPosX = scroll.x + momentumX.dist;
                newPosY = scroll.y + momentumY.dist;
                if ((scroll.x > 0 && newPosX > 0) ||
                    (scroll.x < scroll.maxScrollX && newPosX < scroll.maxScrollX)) {
                    momentumX = { dist: 0, time: 0 };
                }
                if ((scroll.y > scroll.minScrollY && newPosY > scroll.minScrollY) ||
        ...
        _resetPosition: function (time) {
            let scroll = this, resetX = scroll.x >= 0
                ? 0
                : scroll.x < scroll.maxScrollX
                    ? scroll.maxScrollX
                    : scroll.x, resetY = scroll.y >= scroll.minScrollY || scroll.maxScrollY > 0
                ? scroll.minScrollY
        ...
        _wheel: function (event) {
            let scroll = this, wheelDeltaX, wheelDeltaY, deltaX, deltaY;
        ...
        _startAnimate: function () {
            let scroll = this, startX = scroll.x, startY = scroll.y, startTime = Date.now(), step, easeOut, animate;
            if (scroll.animating) {
                return;
            }
        ...
        _momentum: function (dist, time, maxDistUpper, maxDistLower, size) {
            let deceleration = 0.0006, speed = Math.abs(dist) / time, newDist = (speed * speed) / (2 * deceleration), outsideDist = 0;
...
(function ($, window, document, undefined) {
    dorado.beforeInit(function () {
        ...
        let dataPropertyName = "virtualMouseBindings", touchTargetPropertyName = "virtualTouchID", virtualEventNames = "mouseover mousedown mousemove mouseup click mouseout mousecancel".split(" "), touchEventProps = "clientX clientY pageX pageY screenX screenY".split(" "), mouseHookProps = $.event.mouseHooks ? $.event.mouseHooks.props : [], mouseEventProps = $.event.props.concat(mouseHookProps), activeDocHandlers = {}, resetTimerID = 0, startX = 0, startY = 0, didScroll = false, clickBlockList = [], blockMouseTriggers = false, blockTouchTriggers = false, eventCaptureSupported = "addEventListener" in document, $document = $(document), nextTouchID = 1, lastTouchID = 0;
        $.vmouse = {
        ...
        function createVirtualEvent(event, eventType) {
            let t = event.type, oe, props, ne, prop, ct, touch, i, j;
            event = $.Event(event);
            ...
            let tagName = ((event.target && event.target.tagName) ||
                "").toLowerCase();
        ...
        function handleTouchStart(event) {
            let touches = getNativeEvent(event).touches, target, flags;
        ...
        function handleTouchMove(event) {
            ...
            let t = getNativeEvent(event).touches[0], didCancel = didScroll, moveThreshold = $.vmouse.moveDistanceThreshold;
            (didScroll =
                didScroll ||
        ...
        function handleTouchEnd(event) {
            ...
            disableTouchBindings();
            let flags = getVirtualBindingFlags(event.target), t;
        ...
        function hasVirtualBindings(ele) {
            let bindings = $.data(ele, dataPropertyName), k;
            if (bindings) {
                for (k in bindings) {
                    if (bindings[k]) {
                        return true;
        ...
        function getSpecialEventObject(eventType) {
            return {
                ...
                teardown: function (data, namespace) {
                    ...
                    let $this = $(this), bindings = $.data(this, dataPropertyName);
                    if (bindings) {
        ...
        if (eventCaptureSupported) {
            document.addEventListener("click", function (e) {
                let cnt = clickBlockList.length, target = e.target, x, y, ele, i, o, touchID;
                if (cnt) {
                    x = e.clientX;
                    y = e.clientY;
