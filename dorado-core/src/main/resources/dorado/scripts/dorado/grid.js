...
(function () {
    ...
    dorado.widget.grid.CheckBoxCellRenderer = $extend(dorado.widget.grid.SubControlCellRenderer, { preventCellEditing: true, createSubControl: function (arg) {
            let self = this;
            let checkbox = new dorado.widget.CheckBox({ iconOnly: true, beforePost: function (control, arg) {
    ...
    dorado.widget.grid.RowSelectorCellRenderer = $extend(dorado.widget.grid.SubControlCellRenderer, { ATTRIBUTES: { checkboxMap: {} }, cellMouseDownListener: function (arg) {
            ...
            let checkbox = new dorado.widget.CheckBox({ iconOnly: true, onValueChange: function (checkbox) {
    ...
    dorado.widget.grid.FilterBarCellRenderer = $extend(dorado.widget.grid.SubControlCellRenderer, { createFilterExpressionEditor: function (arg) {
            ...
            let textEditor = new dorado.widget.TextEditor({ width: "100%", onBlur: function (textEditor) {
...
(function () {
    ...
    let overrides = { constructor: function (itemModel) {
    ...
    dorado.widget.AbstractGrid = $extend([dorado.widget.AbstractList, dorado.widget.grid.ColumnModel], { $className: "dorado.widget.AbstractGrid", ATTRIBUTES: { className: { defaultValue: "d-grid" }, highlightCurrentRow: { defaultValue: true, skipRefresh: true, setter: function (v) {
            ...
            this._columns.destroy();
            $invokeSuper.call(this);
        }, doSetFocus: dorado._NULL_FUNCTION, doGet: function (attr) {
            let c = attr.charAt(0);
            if (c === "#" || c === "&") {
                let col = attr.substring(1);
                return this.getColumn(col);
            ...
            function registerInnerControl(innerGrid) {
                ...
                function findColumnByEvent(grid, row, event) {
                    let column = null;
                    if (row) {
                        let cell = $DomUtils.findParent(event.target, function (parentNode) {
            ...
            let row = $DomUtils.findParent(evt.target, function (parentNode) {
            ...
            if (row) {
                ...
                if (innerGrid.getCurrentItemDom() === row) {
                    let cell = $DomUtils.findParent(evt.target, function (parentNode) {
                        return parentNode.parentNode === row;
                    }, true);
                    if (cell) {
            ...
            if (column) {
                ...
                if (!$cell.data("ui-draggable")) {
                    let grid = this;
                    let options = dorado.Object.apply({ appendTo: "body", helper: function (evt) {
            ...
            function crreateColumnItems(columnsItem, columns) {
                columns.each(function (column) {
                    let item = new dorado.widget.menu.CheckableMenuItem({ $type: "Checkable", caption: column.get("caption") || column.get("name"), checked: column.get("visible"), hideOnClick: false, onCheckedChange: function (self) {
    ...
    dorado.widget.grid.AbstractInnerGrid = $extend(dorado.widget.RowList, { $className: "dorado.widget.grid.AbstractInnerGrid", focusable: false, ATTRIBUTES: { selection: { getter: function (p, v) {
                ...
                } }, useNativeScrollbars: { readOnly: true } }, constructor: function (grid, fixed) {
            this.grid = grid;
            this.fixed = fixed;
            $invokeSuper.call(this, []);
            ...
            if (!action) {
                let headerCell = $DomUtils.findParent(evt.target, function (node) {
...
(function () {
    ...
    dorado.widget.grid.CriterionDropDown = $extend(dorado.widget.DropDown, { $className: "dorado.widget.grid.CriterionDropDown", ATTRIBUTES: { maxWidth: { defaultValue: 330 }, supportsMultiCriterions: { defaultValue: true }, supportsJunction: { defaultValue: true }, avialableOperators: {}, criterion: { defaultValue: [], setter: function (criterion) {
            ...
            if (dropdown._supportsJunction && dropdown._supportsMultiCriterions) {
                let junctionRadio = new dorado.widget.RadioGroup({ value: "and", radioButtons: [{ value: "and", text: $resource("dorado.core.And") }, { value: "or", text: $resource("dorado.core.Or") }], onPost: function (self) {
    ...
    let CriterionControl = $extend(dorado.widget.Control, { $className: "dorado.widget.grid.CriterionControl", ATTRIBUTES: { column: {}, criterion: {} }, constructor: function (dropDown, dropDownBox) {
            $invokeSuper.call(this);
            ...
            let operatorEditor = criterionControl._operatorEditor = new dorado.widget.TextEditor({ width: 100, mapping: getOperatorItems(), onPost: function (self) {
            ...
            let valueEditor = criterionControl._valueEditor = new dorado.widget.TextEditor({ width: 180, onPost: function (self) {
