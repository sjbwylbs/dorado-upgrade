...
(function () {
	let DEBUGGER_SHOW_ON_VISIBLE_KEY = "dorado.Debugger.showOnVisible";
	function setCookie(name: any, value: any, expire: any) {
		let exp = new Date();
	...
	dorado.Debugger = {
		...
		format2HTML: function (text: any) {
			...
			return text.replace(reg1, "&amp;").replace(pattern, function (string: any) {
				return map[string];
			});
		},
		init: function () {
			let tabControl = new dorado.widget.TabControl({
				...
				onTabChange: function (self: any, arg: any) {
					let tab = arg.newTab;
					if (tab && tab._name) {
						let callback = dorado.Debugger.moduleActiveEvent[tab._name];
						if (callback) {
			...
			let dialog = new dorado.widget.Dialog({
				layout: { $type: "Dock", padding: 1 },
						...
						children: [
							{
								...
								listener: {
									onValueChange: function (self: any) {
										if (self._value) {
											setCookie(
												DEBUGGER_SHOW_ON_VISIBLE_KEY,
												true,
												3600 * 24 * 365 * 1000,
											);
										} else {
		...
		extend: function (source: any) {
			for (let prop in source) {
				dorado.Debugger[prop] = source[prop];
			}
		},
		registerModule: function (config: any) {
			let deb = dorado.Debugger;
			config = config || {};
		...
		show: function () {
			$import("tree-grid", function () {
				if (dorado.debug.initProcedures) {
					dorado.debug.initProcedures.each(function (proc: any) {
						proc.call();
					});
	...
	jQuery(document).ready(function () {
		let showOnVisible = getCookie(DEBUGGER_SHOW_ON_VISIBLE_KEY);
		if (showOnVisible) {
...
dorado.debug.initProcedures.push(function () {
	...
	dorado.debug.Logger = $extend(dorado.widget.Control, {
		$className: "dorado.debug.Logger",
		ATTRIBUTES: {
			className: { defaultValue: "d-debugger-logger" },
			lockScrollBar: {},
			level: {
				defaultValue: "debug",
				setter: function (value: any) {
					let logger = this,
						oldValue = logger._level,
						dom = logger._dom,
						i;
					value = value || "debug";
					if (oldValue && dom) {
						if (LOG_LEVELS[oldValue] > LOG_LEVELS[value]) {
							for (i = LOG_LEVELS[value] + 1; i <= LOG_LEVELS[oldValue]; i++) {
								$fly(dom)
									.find(".log-" + LOG_LEVELS[i])
									.css("display", "none");
							}
						} else {
							if (LOG_LEVELS[oldValue] < LOG_LEVELS[value]) {
								for (
									i = LOG_LEVELS[oldValue] + 1;
									i <= LOG_LEVELS[value];
									i++
								) {
									$fly(dom)
		...
		log: function (msg: any, level: any) {
			let logger = this,
				logs = logger._logs;
			...
			if (logger._rendered) {
				let dom = logger._dom,
					logDom = $DomUtils.xCreate({
						...
						style: {
							display:
								LOG_LEVELS[level] > LOG_LEVELS[logger._level] ? "none" : "",
						},
						content: [
							{ tagName: "div", className: "icon" },
							{ tagName: "div", className: "msg", content: msg },
						],
					});
		...
		dir: function (target: any, level: any) {
			let buffer = "{\n";
		...
		createDom: function () {
			let logger = this,
				dom = $DomUtils.xCreate({ tagName: "div" }),
				logs = logger._logs,
				logDom;
			if (logs) {
				for (let i = 0, j = logs.length; i < j; i++) {
					let log = logs[i],
						semicolonIndex = log.indexOf(":"),
						level = log.substring(0, semicolonIndex),
						msg = log.substring(semicolonIndex + 1);
					logDom = $DomUtils.xCreate({
						tagName: "div",
						className: "log log-" + level,
						style: {
	...
	dorado.debug.ConsolePanel = $extend(dorado.widget.Container, {
		$className: "dorado.debug.ConsolePanel",
		constructor: function (config: any) {
			config = config || {};
			let panel = this,
				logger = new dorado.debug.Logger();
			let debugButton = new dorado.widget.SimpleIconButton({
				toggleable: true,
			...
			let infoButton = new dorado.widget.SimpleIconButton({
			...
			let warnButton = new dorado.widget.SimpleIconButton({
			...
			let errorButton = new dorado.widget.SimpleIconButton({
		...
		setLogLevel: function (level: any) {
	...
	let consolePanel = new dorado.debug.ConsolePanel({
		layout: { $type: "Dock", padding: 0 },
	});
...
dorado.debug.initProcedures.push(function () {
	dorado.debug.DebugPanel = $extend(dorado.widget.Panel, {
		$className: "dorado.debug.DebugPanel",
		EVENTS: { onRunCodeError: {} },
		constructor: function (config: any) {
			config = config || {};
			let panel = this;
			let codeTextArea = new dorado.widget.TextArea({
				width: "100%",
					...
					items: [
						{
							...
							listener: {
								onClick: function () {
									let code = "var view = viewMain;\n";
									code += codeTextArea.get("text");
									if (dorado.Debugger.trapError) {
										try {
											// CSP 兼容：调试器功能不支持动态代码执行
											console.warn("CSP-compliant mode: Debugger code execution is not supported.");
											panel.fireEvent("onRunCodeError", panel, {
												errorMsg: "CSP-compliant mode: Dynamic code execution is not supported.",
											});
										} catch (e) {
											let errorMsg;
	...
	dorado.Debugger.registerModule({
		...
		control: new dorado.debug.DebugPanel({
			border: "none",
			layout: new dorado.widget.layout.DockLayout({ padding: 0 }),
			onRunCodeError: function (self: any, arg: any) {
				let errorMsg = arg.errorMsg;
				self._parent._parent.set("currentTab", "console");
				let logDom = dorado.Debugger.log(errorMsg, "error");
			},
		}),
	});
});
dorado.debug.initProcedures.push(function () {
	let ENTITY_STATE_MAP = { 0: "none", 1: "new", 2: "modified", 3: "deleted" };
	let mixIf = function (target: any, source: any) {
		if (target && source) {
			for (let prop in source) {
	...
	function isCollection(object: any) {
		return (
			object instanceof Array ||
			object instanceof dorado.util.KeyedArray ||
			object instanceof dorado.util.KeyedList ||
			object instanceof dorado.EntityList ||
			object instanceof dorado.Entity
		);
	}
	let getFnBody = function (text: any) {
		let entire = typeof text === "function" ? text.toString() : text;
		return jQuery.trim(
			entire.substring(entire.indexOf("{") + 1, entire.lastIndexOf("}")),
		);
	};
	dorado.debug.AttrGrid = $extend(dorado.widget.TreeGrid, {
		...
		constructor: function (config: any) {
			...
			mixIf(config, {
				...
				columns: [
					{ name: "name", property: "name", width: 200 },
					{ name: "value", property: "value" },
				],
				treeColumn: "name",
				beforeExpand: function (self: any, arg: any) {
					let node = arg.node,
						id = node.get("data.label"),
				...
				onCellValueEdit: function (self: any, arg: any) {
					let entity = arg.entity;
					if (entity) {
						let name = entity.get("label"),
							value = entity.get("value"),
							sourceObject = self._sourceObject;
						if (
							sourceObject &&
							dorado.Object.isInstanceOf(sourceObject, dorado.AttributeSupport)
						) {
							if (isFinite(value)) {
		...
		getTreeGridNodes: function (object: any) {
			let prop,
				value,
				hasChild,
				nodes = [];
			if (object instanceof dorado.Entity) {
				...
				if (
					object &&
					dorado.Object.isInstanceOf(object, dorado.AttributeSupport)
				) {
	...
	dorado.debug.ControlsPanel = $extend(dorado.widget.Panel, {
		$className: "dorado.debug.ControlsPanel",
		EVENTS: { onTreeCurrentChange: {} },
		constructor: function (config: any) {
			config = config || {};
			let panel = this;
			let controlsTree = new dorado.widget.Tree({
				expandingMode: "sync",
				$type: "Tree",
				...
				onCurrentChange: function (self: any, arg: any) {
				...
				tools: [
					{
						...
						listener: {
							onClick: function () {
								let current = controlsTree.get("currentNode");
								if (!current) {
									dorado.MessageBox.alert(
										$resource("dorado.baseWidget.DebuggerVariableNodeRequired"),
									);
								} else {
									dorado.MessageBox.prompt(
										$resource("dorado.baseWidget.DebuggerVariableExportInput"),
										function (text: any) {
											if (text) {
												...
												dorado.MessageBox.alert(
													$resource(
														"dorado.baseWidget.DebuggerVariableNameRequired",
													),
												);
											}
										},
		...
		reload: function () {
			let panel = this,
				controlsTree = panel._controlsTree,
				topView = $topView,
				nodes = panel.getTreeNodes(topView._children, true);
		...
		getTreeNodes: function (object: any, onlyview: any) {
			function isCollectionHasElement(object: any) {
				let result = false,
					firstObject;
				...
				if (
					firstObject &&
					(firstObject instanceof dorado.Entity ||
			...
			function isHasChild(object: any) {
				let prop, value;
				if (
					object &&
					dorado.Object.isInstanceOf(object, dorado.AttributeSupport)
			...
			function getObjectChildren(object: any, onlyview: any) {
				let prop,
					value,
					nodes = [],
					temp = [];
			...
			function getCollectionChildren(object: any, onlyview: any) {
				let nodes = [];
				function doGetNodes(obj: any) {
					if (onlyview) {
					...
					let className = obj.constructor.className || "",
						$type = className.replace("dorado.widget.", "");
					let label = $type;
				...
				if (object instanceof dorado.Entity) {
					...
					if (object instanceof dorado.EntityList) {
						object.each(function (entity: any) {
							nodes.push({
								label: "Entity",
	...
	dorado.debug.EventsPanel = $extend(dorado.widget.Panel, {
		$className: "dorado.debug.EventsPanel",
		ATTRIBUTES: {
			sourceObject: {
				setter: function (value: any) {
					let eventsTree = this._eventsTree;
		...
		constructor: function (config: any) {
			config = config || {};
			let panel = this;
			let eventsTree = new dorado.widget.Tree({
				expandingMode: "sync",
				beforeExpand: function (self: any, arg: any) {
					let node = arg.node,
						id = node.get("data.label"),
				...
				onCurrentChange: function (self: any, arg: any) {
					let current = arg.newCurrent,
						value,
						content = "",
						preview = panel._eventPreviewPanel;
					if (current) {
				...
				tools: [
					{
						...
						listener: {
							onClick: function () {
								let current = eventsTree.get("currentNode");
								if (current && current.get("level") === 1) {
									let name = current.get("label"),
										object = current.get("data.object");
									dorado.MessageBox.promptMultiLines(
										$resource("dorado.baseWidget.DebuggerInputListenerCode"),
										function (text: any) {
											// CSP 兼容：不支持动态事件监听器代码
											console.warn("CSP-compliant mode: Dynamic event listener code is not supported.");
											current.clearChildren();
											current.addNodes(
												panel.getEventNodes(current.get("data.value")),
											);
										},
									);
								}
									...
									dorado.MessageBox.promptMultiLines(
										$resource("dorado.baseWidget.DebuggerInputListenerCode"),
										{
											callback: function (text: any) {
												// CSP 兼容：不支持动态事件监听器代码
												console.warn("CSP-compliant mode: Dynamic event listener code is not supported.");
												let preview = panel._eventPreviewPanel;
												preview._dom.innerHTML = dorado.Debugger.format2HTML(
													getFnBody(fn.listener),
												);
											},
		...
		getEventNodes: function (object: any) {
			let nodes = [],
				value,
				hasChild;
			if (object && object.EVENTS) {
				let events = object._events || {};
				for (let prop in object.EVENTS) {
					value = events[prop];
					hasChild = value && value.length > 0;
					nodes.push({
						label: prop,
						hasChild: hasChild,
						data: { label: prop, value: value, object: object },
					});
				}
			} else {
	...
	dorado.debug.ViewPanel = $extend(dorado.widget.SplitPanel, {
		...
		constructor: function (config: any) {
			config = config || {};
			let panel = this;
			let eventsPanel = new dorado.debug.EventsPanel({
				layoutConstraint: "right",
				width: 200,
			});
			let eventPreviewPanel = new dorado.widget.HtmlContainer({});
			eventsPanel._eventPreviewPanel = eventPreviewPanel;
			let attrGrid = new dorado.debug.AttrGrid();
			let controlTreePanel = new dorado.debug.ControlsPanel({
				layoutConstraint: { type: "left", height: "100%" },
				listener: {
					onTreeCurrentChange: function (self: any, arg: any) {
						let current = arg.newCurrent;
						if (current) {
			...
			mixIf(config, {
				...
				mainControl: {
					$type: "TabControl",
					tabs: [
						{ $type: "Control", caption: "Attributes", control: attrGrid },
						{
							$type: "Control",
							caption: "Events",
							control: {
								$type: "Container",
								layout: { $type: "Dock", regionPadding: 1 },
	...
	let viewChildrenLoaded = false,
		viewPanel = new dorado.debug.ViewPanel();
	dorado.Debugger.registerModule({
		$type: "Control",
		name: "view",
		caption: "View",
...
dorado.debug.initProcedures.push(function () {
	...
	dorado.debug.HotkeysPanel = $extend(dorado.widget.SplitPanel, {
		$className: "dorado.debug.HotkeysPanel",
		constructor: function (config: any) {
			config = config || {};
			let panel = this;
			let codePreview = new dorado.widget.HtmlContainer({
				width: "100%",
				height: "100%",
				style: "display: block",
			});
			let keyList = new dorado.widget.ListBox({
				style: "border: 0",
				onCurrentChange: function (self: any, arg: any) {
					let item = self.getCurrentItem(),
						code = "";
		...
		reload: function () {
			...
			if (hotkeys && hotkeys.triggersMap) {
				let triggersMap = hotkeys.triggersMap;
				for (let prop in triggersMap) {
					let target = triggersMap[prop];
					for (let type in target) {
						let keyList = target[type];
						for (let key in keyList) {
							let callbacks = keyList[key] || [];
							for (let i = 0, j = callbacks.length; i < j; i++) {
								result.push(
									new HotKey({ type: type, key: key, callback: callbacks[i] }),
								);
							}
						}
					}
	...
	let hotkeysPanel = new dorado.debug.HotkeysPanel(),
		loaded = false;
	dorado.Debugger.registerModule({
		$type: "Control",
		name: "hotkey",
...
dorado.debug.initProcedures.push(function () {
	let oldAjaxResult = dorado.util.AjaxResult;
	let ajaxResultPool = [];
	...
	dorado.debug.AjaxGrid = $extend(dorado.widget.Grid, {
		...
		constructor: function (config: any) {
			...
			config = {
				columns: [
					{ name: "Name", property: "name", width: 200 },
					{ name: "Url", property: "url", width: 300 },
					{ name: "Method", property: "method" },
					{ name: "Status", property: "status" },
					{ name: "StatusText", property: "statusText" },
					{ name: "Text", property: "text" },
					{ name: "Exception", property: "exception" },
				],
			};
			$invokeSuper.call(this, [config]);
		},
	...
	let ajaxGrid = new dorado.debug.AjaxGrid();
