...
(function () {
	...
	dorado.widget.AbstractList = $extend(dorado.widget.Control, {
		...
		sort: function (sortParams: any) {
			let customComparator,
				list = this;
			if (list.getListenerCount("onCompareItems") > 0) {
				customComparator = function (item1: any, item2: any, sortParams: any) {
					let arg = {
						item1: item1,
						item2: item2,
		...
		filter: function (criterions: any) {
			...
			if (list.getListenerCount("onFilterItem") > 0) {
				customFilter = function (value: any, criterions: any) {
		...
		processItemDrop: function (draggingInfo: any, evt: any) {
			function getItemList(control: any, entity: any) {
				let list;
			...
			if (
				...
				let sourceList = getItemList(sourceControl, object),
				...
				let targetList = this.get("itemModel").getItems(),
					highlight;
				if (!targetList) {
	...
	dorado.widget.ViewPortList = $extend(dorado.widget.AbstractList, {
		...
		refreshItemDoms: function (
			...
			let bookmark = it.createBookmark(),
			...
			if (!keepItemsOutOfViewPort) {
				for (
					let i = itemDomContainer.childNodes.length - 1;
					i >= itemDomCount;
					i--
				) {
					this.removeItemDom(
		...
		removeItemDom: function (dom: any) {
...
(function () {
	...
	dorado.widget.list.ItemModel = $class({
		...
		_startIndex: 0,
		_reverse: false,
		setItemDomSize: function (itemDomSize: any) {
			this._itemDomSize = itemDomSize;
		},
		getStartIndex: function () {
			return this._startIndex > this.getItemCount() ? 0 : this._startIndex;
		},
		setStartIndex: function (startIndex: any) {
			this._startIndex = startIndex;
		},
		isReverse: function () {
			return this._reverse;
		},
		setReverse: function (reverse: any) {
			this._reverse = reverse;
		},
		setScrollSize: function (viewPortSize: any, scrollSize: any) {
		...
		filter: function (filterParams: any, customFilter: any) {
			function getValueComparator(op: any) {
				...
				if (!comparator) {
					...
					switch (escapedOp) {
						case "==":
							comparator = function(v1: any, v2: any) { return v1 == v2; };
							break;
						case "!=":
							comparator = function(v1: any, v2: any) { return v1 != v2; };
							break;
			...
			function filterItem(item: any, filterParam: any) {
				let value;
			...
			function processFilterParams(filterParams: any, junction: any) {
