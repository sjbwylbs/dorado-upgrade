...
(function (window: any, $: any, undefined: any) {
	let defaultOpts = {
			beforeShow: noop,
			move: noop,
			change: noop,
		...
		rgbaSupport = (function () {
			function contains(str: any, substr: any) {
	...
	function paletteTemplate(p: any, color: any, className: any) {
		let html = [];
	...
	function instanceOptions(o: any, callbackContext: any) {
	...
	function spectrum(element: any, o: any) {
		...
		let doc = element.ownerDocument,
			...
			replacer = shouldReplace
				? $(replaceInput).addClass(theme).addClass(opts.className)
				: $([]),
		...
		function initialize() {
			...
			if (localStorageKey && window.localStorage) {
				try {
					...
					if (oldPalette.length > 1) {
						...
						$.each(oldPalette, function (i: any, c: any) {
			...
			offsetElement.bind("click.spectrum touchstart.spectrum", function (e: any) {
				if (!disabled) {
					toggle();
				}
				e.stopPropagation();
			...
			textInput.keydown(function (e: any) {
				if (e.keyCode === 13) {
					setFromTextInput();
				}
			});
			cancelButton.text(opts.cancelText);
			cancelButton.bind("click.spectrum", function (e: any) {
				e.stopPropagation();
				e.preventDefault();
				hide("cancel");
			});
			clearButton.bind("click.spectrum", function (e: any) {
				e.stopPropagation();
				e.preventDefault();
				isEmpty = true;
				move();
			...
			chooseButton.bind("click.spectrum", function (e: any) {
				e.stopPropagation();
				e.preventDefault();
				if (isValid()) {
					updateOriginalInput(true);
			...
			draggable(alphaSlider, function (dragX: any, dragY: any, e: any) {
			...
			draggable(
				dragger,
				function (dragX: any, dragY: any, e: any) {
					if (!e.shiftKey) {
						shiftMovementDirection = null;
					} else {
			...
			function palletElementClick(e: any) {
		...
		function addColorToSelectionPalette(color: any) {
		...
		function drawPalette() {
			...
			let html = $.map(paletteArray, function (palette: any, i: any) {
		...
		function hide(e: any) {
			...
			let colorHasChanged = !tinycolor.equals(get(), colorOnShow);
			if (colorHasChanged) {
		...
		function set(color: any, ignoreFormatChange: any) {
		...
		function updateOriginalInput(fireCallback: any) {
			let color = get(),
				displayColor = "",
		...
		function option(optionName: any, optionValue: any) {
	...
	function getOffset(picker: any, input: any) {
		let extraY = 0;
	...
	function bind(func: any, obj: any) {
	...
	function draggable(element: any, onmove: any, onstart: any, onstop: any) {
	...
	function throttle(func: any, wait: any, debounce: any) {
		let timeout;
		return function () {
			let context = this,
				args = arguments;
	...
	$.fn.spectrum = function (opts: any, extra: any) {
	...
	(function () {
		...
		function tinycolor(color: any, opts: any) {
			color = color ? color : "";
			opts = opts || {};
			...
			return {
				...
				toHex: function (allow3Char: any) {
					return rgbToHex(r, g, b, allow3Char);
				},
				toHexString: function (allow3Char: any) {
					return "#" + rgbToHex(r, g, b, allow3Char);
				},
				toRgb: function () {
				...
				toFilter: function (secondColor: any) {
					let hex = rgbToHex(r, g, b);
					let secondHex = hex;
					let alphaHex = Math.round(parseFloat(a) * 255).toString(16);
					let secondAlphaHex = alphaHex;
					let gradientType =
				...
				toString: function (format: any) {
					let formatSet = !!format;
					format = format || this.format;
					let formattedString = false;
					let hasAlphaAndFormatNotSet = !formatSet && a < 1 && a > 0;
					let formatWithAlpha =
		...
		tinycolor.fromRatio = function (color: any, opts: any) {
		...
		function rgbToRgb(r: any, g: any, b: any) {
			return {
				r: bound01(r, 255) * 255,
				g: bound01(g, 255) * 255,
				b: bound01(b, 255) * 255,
		...
		function rgbToHsv(r: any, g: any, b: any) {
			r = bound01(r, 255);
			g = bound01(g, 255);
			b = bound01(b, 255);
			let max = mathMax(r, g, b),
		...
		function rgbToHex(r: any, g: any, b: any, allow3Char: any) {
			let hex = [
		...
		tinycolor.equals = function (color1: any, color2: any) {
			if (!color1 || !color2) {
				return false;
			}
		...
		tinycolor.desaturate = function (color: any, amount: any) {
		...
		tinycolor.saturate = function (color: any, amount: any) {
			amount = amount === 0 ? 0 : amount || 10;
			let hsl = tinycolor(color).toHsl();
			hsl.s += amount / 100;
			hsl.s = clamp01(hsl.s);
		...
		tinycolor.greyscale = function (color: any) {
			return tinycolor.desaturate(color, 100);
		};
		tinycolor.lighten = function (color: any, amount: any) {
			amount = amount === 0 ? 0 : amount || 10;
			let hsl = tinycolor(color).toHsl();
			hsl.l += amount / 100;
			hsl.l = clamp01(hsl.l);
		...
		tinycolor.darken = function (color: any, amount: any) {
			amount = amount === 0 ? 0 : amount || 10;
			let hsl = tinycolor(color).toHsl();
			hsl.l -= amount / 100;
			hsl.l = clamp01(hsl.l);
		...
		tinycolor.complement = function (color: any) {
			let hsl = tinycolor(color).toHsl();
			hsl.h = (hsl.h + 180) % 360;
			return tinycolor(hsl);
		};
		tinycolor.triad = function (color: any) {
			let hsl = tinycolor(color).toHsl();
			let h = hsl.h;
			return [
				tinycolor(color),
		...
		tinycolor.tetrad = function (color: any) {
			let hsl = tinycolor(color).toHsl();
			let h = hsl.h;
			return [
				tinycolor(color),
		...
		tinycolor.splitcomplement = function (color: any) {
			let hsl = tinycolor(color).toHsl();
			let h = hsl.h;
			return [
				tinycolor(color),
		...
		tinycolor.analogous = function (color: any, results: any, slices: any) {
			results = results || 6;
			slices = slices || 30;
			let hsl = tinycolor(color).toHsl();
			let part = 360 / slices;
		...
		tinycolor.monochromatic = function (color: any, results: any) {
			results = results || 6;
		...
		tinycolor.readability = function (color1: any, color2: any) {
			let a = tinycolor(color1).toRgb();
			let b = tinycolor(color2).toRgb();
			let brightnessA = (a.r * 299 + a.g * 587 + a.b * 114) / 1000;
			let brightnessB = (b.r * 299 + b.g * 587 + b.b * 114) / 1000;
			let colorDiff =
				Math.max(a.r, b.r) -
				Math.min(a.r, b.r) +
				Math.max(a.g, b.g) -
		...
		tinycolor.readable = function (color1: any, color2: any) {
			let readability = tinycolor.readability(color1, color2);
			return readability.brightness > 125 && readability.color > 500;
		};
		tinycolor.mostReadable = function (baseColor: any, colorList: any) {
			let bestColor = null;
			let bestScore = 0;
			let bestIsReadable = false;
			for (let i = 0; i < colorList.length; i++) {
				let readability = tinycolor.readability(baseColor, colorList[i]);
		...
		function convertToPercentage(n: any) {
			if (n <= 1) {
				n = n * 100 + "%";
			}
			return n;
		...
		function stringInputToObject(color: any) {
...
(function () {
	...
	dorado.widget.ColorPickerSupport = $class({
		$className: "dorado.widget.ColorPickerSupport",
		ATTRIBUTES: {
			color: {},
			showInput: { spectrumAttr: true },
			showInitial: { spectrumAttr: true },
			allowEmpty: {},
			showAlpha: { spectrumAttr: true },
			disabled: { spectrumAttr: true },
			showPalette: { spectrumAttr: true },
			showPaletteOnly: { spectrumAttr: true },
			showButtons: { defaultValue: true, writeBeforeReady: true },
			preferredFormat: { writeBeforeReady: true },
			palette: { defaultValue: PALETTES["#sixteen"], writeBeforeReady: true },
		},
	});
	dorado.widget.ColorPicker = $extend(
		[dorado.widget.Control, dorado.widget.ColorPickerSupport],
		{
			$className: "dorado.widget.ColorPicker",
			tabStop: true,
			ATTRIBUTES: {
				className: { defaultValue: "d-color-picker" },
				...
				color: {
					...
					getter: function () {
						if (this._spectrum) {
							let rudeColor = this._spectrum.spectrum("get");
							return rudeColor
								? rudeColor.toString(this._preferredFormat)
								: null;
						} else {
							return this._color;
						}
					},
					setter: function (color: any) {
						if (this._spectrum) {
			...
			EVENTS: { onColorChange: {}, onCursorMove: {} },
			...
			createDom: function () {
				let colorPicker = this,
					inputDom = $DomUtils.xCreate({ tagName: "INPUT" });
				$DomUtils.getInvisibleContainer().appendChild(inputDom);
				colorPicker._spectrum = jQuery(inputDom);
				let options = colorPicker.getBaseSpectrumOption();
				let palette = colorPicker._palette;
				if (palette && palette.length === 1 && PALETTES[palette[0]]) {
					palette = PALETTES[palette[0]];
				}
				let $container = colorPicker._spectrum
					.spectrum(
						dorado.Object.apply(options, {
							...
							move: function (rudeColor: any) {
								colorPicker.onCursorMove(
									rudeColor
										? rudeColor.toString(colorPicker._preferredFormat)
										: null,
								);
							},
							change: function (rudeColor: any) {
			...
			onCursorMove: function (color: any) {
				this.fireEvent("onCursorMove", this, { color: color });
			},
			onColorChange: function (color: any) {
				this.fireEvent("onColorChange", this, { color: color });
			},
		},
	);
	dorado.widget.ColorEditor = $extend(
		[dorado.widget.ColorPicker, dorado.widget.AbstractDataEditor],
		{
			$className: "dorado.widget.ColorEditor",
			_triggerChanged: true,
			ATTRIBUTES: {
				className: { defaultValue: "d-color-editor" },
				value: { skipRefresh: true, path: "color" },
			},
			...
			refreshDom: function (dom: any) {
				...
				let colorEditor = this;
				if (colorEditor._dataSet) {
					if (colorEditor._property) {
						let bindingInfo = colorEditor._bindingInfo;
						if (bindingInfo.entity instanceof dorado.Entity) {
							let color = bindingInfo.entity.get(colorEditor._property);
