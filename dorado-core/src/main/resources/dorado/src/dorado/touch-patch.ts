...
(function () {
	let mround = function (r: any) {
		...
		hasTransitionEnd =
			("on" + vendor === "Moz" ? "" : vendor + "transitionend") in window,
		transitionEndEvent =
			(vendor !== "Moz" ? vendor + "T" : "t") + "ransitionEnd",
		nextFrame =
	...
	let RE_DEFAULT_UNIT =
		...
		DEFAULT_UNIT = "px";
	let VENDOR_PROPERTY =
		/^transition|transitionProperty|transitionDuration|transformOrigin|transitionTimingFunction|transform|backgroundClip|boxSizing|borderRadius|transitionDelay$/i;
	let toCamel = function (string: any) {
		return string.replace(/(-\S)/g, function ($1: any) {
			return $1.toUpperCase().substring(1, 2);
		});
	};
	let setStyle =
		vendor === "ms"
			? function (element: any, styleName: any, styleValue: any) {
					if (!element || !styleName) {
						return false;
					}
					if (typeof styleName !== "string") {
						for (let property in styleName) {
						...
						default:
							try {
								if (
									...
									RE_DEFAULT_UNIT.test(styleName)
								) {
	...
	let scrollbarProperMap = { hori: "horiBar", vert: "vertBar" },
		directionMap = { vert: "v", hori: "h" };
	let merge = function (target: any, source: any) {
		if (!target || !source) {
			return;
		}
	...
	let iScroll = function (el: any, options: any) {
		let scroll = this,
			doc = document,
			i;
		...
		scroll.options = {
			...
			wheelAction: "scroll",
			onRefresh: null,
			onBeforeScrollStart: function (event: any) {
		...
		scroll.horiBar = {};
		scroll.vertBar = {};
		scroll.options.useTransform = hasTransform
			? scroll.options.useTransform
			: false;
		scroll.options.showHoriScrollbar =
			scroll.options.horiScrollEnable && scroll.options.showHoriScrollbar;
		scroll.options.showVertScrollbar =
			scroll.options.vertScrollEnable && scroll.options.showVertScrollbar;
		scroll.options.useTransition =
			hasTransitionEnd && scroll.options.useTransition;
		let styles = {
			"transition-property": scroll.options.useTransform
		...
		if (scroll.options.useTransform) {
			styles["transform"] =
				trnOpen + scroll.x + "px," + scroll.y + "px" + trnClose;
		} else {
	...
	iScroll.prototype = {
		...
		aniTime: null,
		wheelZoomCount: 0,
		updateChildrenStyle: function (prop: any, value: any) {
			let scroll = this,
				children = scroll.element.children;
			for (let i = 0, l = children.length; i < l; i++) {
				let child = children[i];
				if (
					child.nodeType === 3 ||
					child.className.indexOf("ignored") !== -1 ||
					child.className.indexOf("scroll-bar") !== -1
				) {
					continue;
				}
		...
		checkInputs: function (tagName: any) {
			if (
				tagName === "INPUT" ||
				tagName === "TEXTFIELD" ||
				tagName === "TEXTAREA" ||
		...
		_refreshScrollbar: function (direction: any) {
			let scroll = this,
				doc = document,
				dom,
				bar = scroll[scrollbarProperMap[direction]];
			...
			if (!bar.wrapper) {
				...
				if (scroll.options.scrollbarClass) {
					...
					if (direction === "hori") {
						setStyle(dom, {
							...
							bottom: "1px",
							left: "2px",
							right: (scroll.showVertScrollbar ? "7" : "2") + "px",
						});
					} else {
						setStyle(dom, {
							position: "absolute",
							...
							top: "2px",
							right: "1px",
							bottom: (scroll.showHoriScrollbar ? "7" : "2") + "px",
						});
					}
				}
				...
				setStyle(dom, {
					...
					"transition-duration": scroll.options.fadeScrollbar ? "350ms" : "0",
					overflow: "hidden",
					opacity: scroll.options.hideScrollbar ? "0" : "1",
				});
			...
			if (direction === "hori") {
				...
				bar.indicatorSize = Math.max(
					mround((bar.size * bar.size) / scroll.scrollTargetWidth),
					8,
				);
				...
				bar.indicatorSize = Math.max(
					mround((bar.size * bar.size) / scroll.scrollTargetHeight),
					8,
				);
		...
		_refreshScrollbarDom: function (direction: any, hidden: any) {
			let scroll = this,
				position = direction === "hori" ? scroll.x : scroll.y,
				size,
				scrollbarMap = { hori: "showHoriScrollbar", vert: "showVertScrollbar" };
			if (!scroll[scrollbarMap[direction]]) {
				return;
			}
			let bar = scroll[scrollbarProperMap[direction]];
			...
			if (position < 0) {
				if (!scroll.options.fixedScrollbar) {
					...
					bar.indicator.style[direction === "hori" ? "width" : "height"] =
						size + "px";
				}
				position = 0;
			} else {
			...
			setStyle(bar.wrapper, {
				"transition-delay": 0,
				opacity: hidden && scroll.options.hideScrollbar ? "0" : "1",
			});
			setStyle(
				bar.indicator,
				"transform",
				trnOpen +
					(direction === "hori" ? position + "px, 0" : "0, " + position + "px") +
					trnClose,
			);
		},
		_resize: function () {
		...
		_start: function (event: any) {
			let scroll = this,
				point = hasTouch ? event.touches[0] : event,
				matrix,
				x,
				y;
			...
			scroll.movingX = 0;
			scroll.movingY = 0;
			let firstChild = scroll.element.firstChild;
			while (firstChild && firstChild.nodeType !== 1) {
				firstChild = firstChild.nextSibling;
			}
			if (scroll.options.momentum && firstChild) {
				if (scroll.options.useTransform) {
					matrix = getComputedStyle(firstChild, null)
						[vendor + "Transform"].replace(/[^0-9-.,]/g, "")
						.split(",");
					x = matrix[4] * 1;
					y = matrix[5] * 1;
					if (isNaN(x)) {
					...
					if (scroll.options.notMoveElement) {
						if (scroll.resumeHelper) {
							let pos = scroll.resumeHelper();
							x = pos.x;
							y = pos.y;
						}
					} else {
						x =
							getComputedStyle(firstChild, null).left.replace(/[^0-9-]/g, "") *
							1;
						y =
							getComputedStyle(firstChild, null).top.replace(/[^0-9-]/g, "") *
							1;
					}
				}
		...
		_move: function (event: any) {
			...
			if (newX > 0 || newX < scroll.maxScrollX) {
				newX = scroll.options.bounce
					? scroll.x + deltaX / 2
					: newX >= 0 || scroll.maxScrollX >= 0
						? 0
						: scroll.maxScrollX;
			}
			if (newY > scroll.minScrollY || newY < scroll.maxScrollY) {
				newY = scroll.options.bounce
					? scroll.y + deltaY / 2
					: newY >= scroll.minScrollY || scroll.maxScrollY >= 0
						? scroll.minScrollY
						: scroll.maxScrollY;
			}
			scroll.distX += deltaX;
		...
		_end: function (event: any) {
			...
			if (duration < 300 && scroll.options.momentum) {
				momentumX = newPosX
					? scroll._momentum(
							newPosX - scroll.startX,
							duration,
							-scroll.x,
							scroll.scrollTargetWidth - scroll.elementWidth + scroll.x,
							scroll.options.bounce ? scroll.elementWidth : 0,
						)
					: momentumX;
				momentumY = newPosY
					? scroll._momentum(
							newPosY - scroll.startY,
							duration,
							-scroll.y,
							scroll.maxScrollY < 0
								? scroll.scrollTargetHeight -
										scroll.elementHeight +
										scroll.y -
										scroll.minScrollY
								: 0,
							scroll.options.bounce ? scroll.elementHeight : 0,
						)
					: momentumY;
				newPosX = scroll.x + momentumX.dist;
				newPosY = scroll.y + momentumY.dist;
				if (
					(scroll.x > 0 && newPosX > 0) ||
					(scroll.x < scroll.maxScrollX && newPosX < scroll.maxScrollX)
				) {
					momentumX = { dist: 0, time: 0 };
				}
				if (
					(scroll.y > scroll.minScrollY && newPosY > scroll.minScrollY) ||
					(scroll.y < scroll.maxScrollY && newPosY < scroll.maxScrollY)
				) {
					momentumY = { dist: 0, time: 0 };
				}
			}
		...
		_resetPosition: function (time: any) {
			let scroll = this,
				resetX =
					scroll.x >= 0
						? 0
						: scroll.x < scroll.maxScrollX
							...
							: scroll.x,
				resetY =
					scroll.y >= scroll.minScrollY || scroll.maxScrollY > 0
						? scroll.minScrollY
		...
		_transitionEnd: function (event: any) {
			let scroll = this;
		...
		_transitionTime: function (time: any) {
			time += "ms";
		...
		_momentum: function (dist: any, time: any, maxDistUpper: any, maxDistLower: any, size: any) {
		...
		_bind: function (type: any, el: any, bubble: any) {
			(el || this.options.eventElement || this.element).addEventListener(
				type,
				this,
				!!bubble,
			);
		},
		_unbind: function (type: any, el: any, bubble: any) {
			(el || this.options.eventElement || this.element).removeEventListener(
				type,
				this,
				!!bubble,
			);
		...
		viewportSize: function (dir: any) {
			return (
				(dir === "h" ? this.element.clientWidth : this.element.clientHeight) || 1
			);
		},
		scrollSize: function (dir: any) {
			return dir === "h" ? this.element.scrollWidth : this.element.scrollHeight;
		},
		doScrollSize: function (dir: any) {
			let cache = this.getChildrenTransform();
			this.updateChildrenStyle("transform", "");
			let result = this.scrollSize(dir);
		...
		refresh: function () {
			...
			scroll.scrollTargetHeight = mround(
				scroll.doScrollSize("v") + scroll.minScrollY,
			);
			...
			scroll.maxScrollY =
				...
				scroll.scrollTargetHeight +
				scroll.minScrollY +
				(-scroll.options.bottomOffset || 0);
			scroll.movingX = 0;
			scroll.movingY = 0;
			if (scroll.options.onRefresh) {
				scroll.options.onRefresh.call(scroll);
			}
			scroll.horiScrollEnable =
				scroll.options.horiScrollEnable && scroll.maxScrollX < 0;
			scroll.vertScrollEnable =
				scroll.options.vertScrollEnable &&
				((!scroll.options.bounceLock && !scroll.horiScrollEnable) ||
					scroll.scrollTargetHeight > scroll.elementHeight);
			scroll.showHoriScrollbar =
				scroll.horiScrollEnable && scroll.options.showHoriScrollbar;
			scroll.showVertScrollbar =
				scroll.vertScrollEnable &&
				scroll.options.showVertScrollbar &&
				scroll.scrollTargetHeight > scroll.elementHeight;
		...
		scrollTo: function (x: any, y: any, time: any, relative: any) {
			let scroll = this,
				step = x;
			scroll.stop();
			if (!step.length) {
			...
			for (let i = 0, j = step.length; i < j; i++) {
				if (step[i].relative) {
		...
		scrollToElement: function (el: any, time: any) {
			let scroll = this,
				position;
			el = el.nodeType
				? el
				: scroll.scrollTarget
					? scroll.scrollTarget.querySelector(el)
					: el;
			if (!el) {
				return;
			}
			...
			position.left =
				position.left > 0
					? 0
					: position.left < scroll.maxScrollX
						? scroll.maxScrollX
						: position.left;
			position.top =
				position.top > scroll.minScrollY
					? scroll.minScrollY
					: position.top < scroll.maxScrollY
						? scroll.maxScrollY
						: position.top;
			time =
				time === undefined
					? Math.max(Math.abs(position.left) * 2, Math.abs(position.top) * 2)
					: time;
...
(function ($: any, window: any, document: any, undefined: any) {
	dorado.beforeInit(function () {
		if (dorado.touch != null) {
			return;
		}
		if (!dorado.Browser.isTouch) {
			return;
		}
		let dataPropertyName = "virtualMouseBindings",
			touchTargetPropertyName = "virtualTouchID",
			virtualEventNames =
				"mouseover mousedown mousemove mouseup click mouseout mousecancel".split(
					" ",
				),
		...
		function getNativeEvent(event: any) {
			while (event && typeof event.originalEvent !== "undefined") {
				event = event.originalEvent;
			}
			return event;
		}
		function createVirtualEvent(event: any, eventType: any) {
			let t = event.type,
				oe,
				props,
				ne,
		...
		function getMouseEvent(event: any, eventType: any) {
			let t = event.type,
				props,
				ne,
				prop,
		...
		function getVirtualBindingFlags(element: any) {
			let flags = {},
				b,
				k;
			while (element) {
		...
		function getClosestElementWithVirtualBinding(element: any, eventType: any) {
			let b;
			while (element) {
				b = $.data(element, dataPropertyName);
		...
		function triggerVirtualEvent(eventType: any, event: any, flags: any) {
			let ve;
			if (
				...
				if (tagName === "a" || tagName === "input" || tagName === "textarea") {
					let mouseEvent = getMouseEvent(event, eventType);
		...
		function mouseEventCallback(event: any) {
			let touchID = $.data(event.target, touchTargetPropertyName);
			if (!blockMouseTriggers && (!lastTouchID || lastTouchID !== touchID)) {
				let ve = triggerVirtualEvent(event.type, event);
				if (ve) {
		...
		function handleTouchStart(event: any) {
			let touches = getNativeEvent(event).touches,
				target,
				flags;
			if (touches && touches.length === 1) {
				...
				if (flags.hasVirtualBinding) {
					...
					didScroll = false;
					let t = getNativeEvent(event).touches[0];
					startX = t.pageX;
					startY = t.pageY;
		...
		function handleTouchEnd(event: any) {
			...
			disableTouchBindings();
			let flags = getVirtualBindingFlags(event.target),
				t;
			triggerVirtualEvent("mouseup", event, flags);
			if (!didScroll) {
				let ve = triggerVirtualEvent("click", event, flags);
		...
		function hasVirtualBindings(ele: any) {
			let bindings = $.data(ele, dataPropertyName),
				k;
			if (bindings) {
				for (k in bindings) {
					if (bindings[k]) {
		...
		function getSpecialEventObject(eventType: any) {
			return {
				setup: function (data: any, namespace: any) {
					if (!hasVirtualBindings(this)) {
						$.data(this, dataPropertyName, {});
					}
					let bindings = $.data(this, dataPropertyName);
				...
				teardown: function (data: any, namespace: any) {
					--activeDocHandlers[eventType];
		...
		for (let i = 0; i < virtualEventNames.length; i++) {
