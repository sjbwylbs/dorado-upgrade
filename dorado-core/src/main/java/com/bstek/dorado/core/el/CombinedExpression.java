...
public class CombinedExpression extends AbstractExpression {
	 ...
	 */
	public void addSection(JexlExpression expr) {
		sections.add(expr);
	}
	...
	public String getSourceText() {
		...
		for (Object section : sections) {
			if (section instanceof JexlExpression) {
				sb.append(((JexlExpression) section).getSourceText());
			}
	...
	protected Object internalEvaluate() {
		...
		for (Object section : sections) {
			...
			if (section instanceof JexlExpression) {
				JexlExpression expression = (JexlExpression) section;
				try {
					Object value = internalEvaluateExpression(expression, context);
					if (value != null) {
						sb.append(value);
					}
				}
	...
	public String toString() {
		...
		for (Object section : sections) {
			if (section instanceof JexlExpression) {
				...
				sb.append("${").append(((JexlExpression) section).getSourceText()).append('}');
			}
			else {
				sb.append(section);
			}
