...
public abstract class BeanFactoryUtils {
	...
	private static BeanFactoryWrapper getBeanFactoryWrapper(String beanName) {
		...
		if (StringUtils.isNotEmpty(beanName)) {
			...
			throw new IllegalArgumentException("Can not get BeanFactory for [" + beanName + "].");
		}
	...
	public static Object getBean(String beanName) throws Exception {
		...
		return beanFactoryWrapper.getBeanFactory().getBean(beanFactoryWrapper.getBeanName());
	}
	 ...
	 * @throws Exception
	 */
	public static Object getBean(String beanName, MethodInterceptor[] methodInterceptors) throws Exception {
		BeanFactoryWrapper beanFactoryWrapper = getBeanFactoryWrapper(beanName);
		return beanFactoryWrapper.getBeanFactory().getBean(beanFactoryWrapper.getBeanName(), methodInterceptors);
	}
	 ...
	 * @throws Exception
	 */
	public static Object getBean(String beanName, MethodInterceptor methodInterceptor) throws Exception {
		BeanFactoryWrapper beanFactoryWrapper = getBeanFactoryWrapper(beanName);
		return beanFactoryWrapper.getBeanFactory()
			.getBean(beanFactoryWrapper.getBeanName(), new MethodInterceptor[] { methodInterceptor });
	}
	 ...
	 */
	public static BeanWrapper getBean(String beanName, Scope scope) throws Exception {
	 ...
	 * @see com.bstek.dorado.core.bean.BeanWrapper
	 */
	public static BeanWrapper getBean(String beanName, MethodInterceptor[] methodInterceptors, Scope scope,
		...
		Object bean = null;
		if (scope != null) {
			bean = scopeManager.getBean(scope, beanId);
		}
		boolean create = (bean == null);
		if (create) {
			BeanFactoryWrapper beanFactoryWrapper = getBeanFactoryWrapper(beanName);
			bean = beanFactoryWrapper.getBeanFactory().getBean(beanFactoryWrapper.getBeanName(), methodInterceptors);
			if (scope != null) {
				scopeManager.putBean(scope, beanId, bean);
			}
		}
