...
public abstract class EngineStartupListenerManager {
	...
	private static final Log logger = LogFactory.getLog(EngineStartupListenerManager.class);

	private static final Collection<EngineStartupListener> listeners = new TreeSet<>(
			new Comparator<EngineStartupListener>() {
				public int compare(EngineStartupListener listener1, EngineStartupListener listener2) {
