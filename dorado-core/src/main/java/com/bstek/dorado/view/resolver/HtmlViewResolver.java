	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		if (touchUserAgentArray != null) {
			...
			for (String mobile : touchUserAgentArray) {
				if (userAgent.toString().toLowerCase().contains(mobile.toString().toLowerCase())) {
		...
		if (shouldAutoLoadDataConfigResources) {
			if (System.currentTimeMillis() - lastValidateTimestamp > MIN_DATA_CONFIG_VALIDATE_SECONDS * ONE_SECOND) {
				lastValidateTimestamp = System.currentTimeMillis();
		...
		if (StringUtils.isNotEmpty(pageTemplate) && StringUtils.isNotEmpty(pageUri)) {
			throw new IllegalArgumentException("Can not set [view.pageTemplate] and [view.pageUri] at the same time.");
		}
		...
		if (StringUtils.isNotEmpty(pageUri)) {
			...
			org.apache.velocity.context.Context velocityContext = velocityHelper.getContext(view, request, response);
