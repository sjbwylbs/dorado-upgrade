...
public class PageHeaderOutputter implements Outputter {
	...
	private List<ClientSettingsOutputter> clientSettingsOutputters = new ArrayList<>();
	...
	public void addClientSettingsOutputter(ClientSettingsOutputter clientSettingsOutputter) {
		clientSettingsOutputters.add(clientSettingsOutputter);
	}

	protected String getBasePackageNames(DoradoContext context, int clientType, String skin) throws Exception {
		...
		SkinSetting skinSetting = skinSettingManager.getSkinSetting(context, skin);
		if (skinSetting != null) {
	...
	/**
	 * Get CSP nonce from session if available.
	 * Returns empty string if nonce is not available or CSP is disabled.
	 */
	private String getCspNonceAttribute() {
		try {
			...
			String nonce = (String) session.getAttribute("csp_nonce");
			if (nonce != null && !nonce.isEmpty()) {
				return " nonce=\"" + nonce + "\"";
			}
		} catch (Exception e) {
			// Ignore - nonce is optional
		}
		return "";
	}
	...
	public void output(Object object, OutputContext context) throws Exception {
		...
		String nonceAttr = getCspNonceAttribute();
		...
		if (StringUtils.isNotEmpty(view.getTitle())) {
			writer.append("<title>").append(StringEscapeUtils.escapeHtml4(view.getTitle())).append("</title>\n");
		}
		...
		writer.append("$import(\"")
			.append(getBasePackageNames(doradoContext, currentClientType, skin))
			.append("\");\n</script>\n")
			.append("<script language=\"javascript\" type=\"text/javascript\"")
			.append(nonceAttr)
			.append(">\n");
