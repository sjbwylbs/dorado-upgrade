...
public class LongTaskThread extends Thread implements TaskMessageListener {
	...
	public synchronized void addMessageListener(TaskThreadMessageListener listener) {
	...
	public synchronized void removeMessageListener(TaskThreadMessageListener listener) {
		if (messageListeners != null) {
	...
	protected void fireTaskStateChange(TaskStateInfo state) {
		if (messageListeners != null) {
			for (TaskThreadMessageListener listener : messageListeners.toArray(new TaskThreadMessageListener[0])) {
	...
	public synchronized void addExecutionListener(TaskThreadExecutionListener listener) {
	...
	public synchronized void removeExecutionListener(TaskThreadExecutionListener listener) {
		if (executionListeners != null) {
	...
	public void run() {
		...
		try {
			...
			if (task.isActive()) {
				TaskStateInfo stateInfo = new TaskStateInfo(TaskState.terminated);
