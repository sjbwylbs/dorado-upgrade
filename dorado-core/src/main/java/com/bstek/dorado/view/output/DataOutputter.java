...
public class DataOutputter implements Outputter, PropertyOutputter {
	...
	private static final ResourceManager resourceManager = ResourceManagerUtils.get(DataOutputter.class);
	 ...
	 */
	protected void outputExpression(String expression, OutputContext context) throws Exception {
		Writer writer = context.getWriter();
		if (expression.equals("this")) {
			writer.write(expression);
		}
		else {
			writer.write(expression.replaceAll("this.", "dorado.DataPath.create(\""));
	...
	private void outputPagingList(PagingList pagingList, OutputContext context) throws Exception {
		JsonBuilder json = context.getJsonBuilder();
		json.object();
	...
	private void outputPage(Page<?> page, OutputContext context) throws Exception {
		JsonBuilder json = context.getJsonBuilder();
		json.object();
		json.key("$isWrapper").value(true);
	 ...
	 * @throws Exception
	 */
	protected void outputData(Object object, OutputContext context) throws Exception {
		JsonBuilder json = context.getJsonBuilder();
		if (EntityUtils.isSimpleValue(object)) {
			if (object instanceof Date) {
				...
				if (d instanceof Time || d instanceof Timestamp || time % ONE_DAY != 0) {
					json.value(DateUtils.format(com.bstek.dorado.core.Constants.ISO_DATETIME_FORMAT1, d));
				}
				else {
					json.value(DateUtils.format(com.bstek.dorado.core.Constants.ISO_DATE_FORMAT, d));
				}
			}
			else if (object != null && EntityUtils.isEnumValue(object)) {
				json.value(((Enum<?>) object).name());
			}
			else {
				json.value(object);
			}
			...
			if (object instanceof Expression) {
				...
				if (expression instanceof SingleExpression) {
					outputExpression((String) ((SingleExpression) expression).prevaluate(), context);
				}
				else if (expression instanceof CombinedExpression) {
					CombinedExpression combinedExpression = (CombinedExpression) expression;
					int i = 0;
					for (Object section : (Object[]) combinedExpression.prevaluate()) {
						if (section == null) {
							continue;
						}
						...
						if (section instanceof Expression) {
							outputExpression(((Expression) section).getSourceText(), context);
						}
						else {
							writer.write('"');
							writer.write(StringEscapeUtils.escapeEcmaScript(String.valueOf(section)));
							writer.write('"');
						}
						i++;
					}
	...
	protected void outputEntity(Object object, OutputContext context) throws Exception {
		EntityWrapper entity = EntityWrapper.create(object);
		if (!context.isShouldOutputEntityState() && entity.getState() == EntityState.DELETED) {
			return;
		}
		...
		if (dataType == null || dataType.isAcceptUnknownProperty()) {
			...
			for (String property : entity.getPropertySet()) {
				...
				if (propertyDef instanceof LazyPropertyDef) {
					...
					if (lazyPropertyDef.isActiveAtClient() && !entity.isLoaded(property)) {
						continue;
					}
				}
				outputEntityProperty(context, entity, property, ignoreEmptyProperty);
			}
		}
		...
		if (context.isShouldOutputEntityState()) {
			...
			if (state != EntityState.NONE) {
				json.key(JsonUtils.STATE_PROPERTY).value(EntityState.toInt(state));
			}
		}
		json.endObject();
	}

	protected void outputEntityProperty(OutputContext context, EntityWrapper entity, String property,
			boolean ignoreEmptyProperty) throws Exception {
		Object value = null;
		...
		if ((value != null && !OutputUtils.isEscapeValue(value, OutputUtils.ESCAPE_VALUE) || !ignoreEmptyProperty)) {
			if (!EntityEnhancer.hasGetterResultSkiped()
					&& (!simplePropertyValueOnly || EntityUtils.isSimpleValue(value))) {
				JsonBuilder json = context.getJsonBuilder();
	...
	private void outputDataTypeIfNecessary(OutputContext context, Object object) {
		if (context.isShouldOutputDataTypes()) {
			...
			if (object instanceof Collection<?>) {
				AggregationDataType aggDataType = EntityUtils.getDataType((Collection<?>) object);
				if (aggDataType != null) {
