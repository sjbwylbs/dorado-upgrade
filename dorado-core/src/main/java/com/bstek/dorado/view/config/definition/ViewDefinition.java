...
public class ViewDefinition extends ContainerDefinition {
	...
	private Map<String, ComponentDefinition> componentMap = new HashMap<>();

	public ViewDefinition(ComponentTypeRegisterInfo registerInfo) throws Exception {
		super(registerInfo);
		...
		if (methods.length == 1) {
			...
			for (int i = 0; i < parameterNames.length; i++) {
				...
				if (ViewElement.class.isAssignableFrom(parameterType)) {
					...
					else if (DataResolver.class.isAssignableFrom(parameterType)) {
