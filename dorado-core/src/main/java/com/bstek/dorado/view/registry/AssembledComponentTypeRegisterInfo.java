...
public class AssembledComponentTypeRegisterInfo extends ComponentTypeRegisterInfo
		implements LazyInitailizeComponentTypeRegistryInfo {
	...
	public void setVirtualProperties(Map<String, VirtualPropertyDescriptor> virtualProperties) {
	...
	public void setVirtualEvents(Map<String, VirtualEventDescriptor> virtualEvents) {
	...
	public void initialize() throws Exception {
		...
		if (StringUtils.isEmpty(src) && classType == null) {
			throw new IllegalArgumentException("Both src and classType undefined");
		}
		...
		if (StringUtils.isNotEmpty(src)) {
			...
			if (viewConfigFactory != null && viewConfigFactory instanceof ViewConfigDefinitionFactory) {
				ViewConfigDefinitionFactory vcdf = (ViewConfigDefinitionFactory) viewConfigFactory;
				viewConfig = vcdf.create(viewName);
			}
			if (viewConfig == null) {
				throw new IllegalArgumentException("AssembleComponent view [" + viewName + "] not found.");
			}
			...
			if (superComponentDefinition == null) {
				throw new IllegalArgumentException("AssembleComponent [" + name + "] not found in [" + src + "].");
			}
			ComponentTypeRegisterInfo superRegisterInfo = componentTypeRegistry
				.getRegisterInfo(superComponentDefinition.getComponentType());
