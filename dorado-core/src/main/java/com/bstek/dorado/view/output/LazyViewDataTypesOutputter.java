 ...
 *
 */
public class LazyViewDataTypesOutputter extends AbstractDataTypeOutputter {
	...
	public void output(Object object, OutputContext context) throws Exception {
		...
		Set<DataType> outputDataTypes = new LinkedHashSet<>();
		...
		if (includeDataTypes != null) {
			while (includeDataTypeNum < includeDataTypes.size()) {
				...
				for (Map.Entry<String, DataType> entry : includeDataTypes.entrySet()) {
					...
					DataType dataType = entry.getValue();
					DataType outputDataType = getOutputDataType(dataType, context);
					if (outputDataType != null && !outputDataTypes.contains(outputDataType)) {
						outputDataTypes.add(outputDataType);
					}
				}
