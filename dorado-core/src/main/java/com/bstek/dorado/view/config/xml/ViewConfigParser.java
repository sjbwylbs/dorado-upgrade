...
public class ViewConfigParser extends GenericObjectParser {
	...
	protected ObjectDefinition createDefinition(Element element, ParseContext context) throws Exception {
		...
		ViewConfigDefinition viewConfigDefinition = new ViewConfigDefinition(
				(PrivateDataTypeDefinitionManager) viewContext.getDataTypeDefinitionManager(),
				(PrivateDataProviderDefinitionManager) viewContext.getDataProviderDefinitionManager(),
				(PrivateDataResolverDefinitionManager) viewContext.getDataResolverDefinitionManager());
	...
	protected Object doParse(Node node, ParseContext context) throws Exception {
		ViewConfigDefinition viewConfigDefinition = (ViewConfigDefinition) super.doParse(node, context);

		ViewParseContext viewContext = (ViewParseContext) context;
		Element documentElement = (Element) node;
		Map<String, Object> arguments = new HashMap<>();
		Map<String, Object> viewContextAttributes = new HashMap<>();

		Element argumentsElement = ViewConfigParserUtils.findArgumentsElement(documentElement, context.getResource());
		if (argumentsElement != null) {
			Map<String, Object> argumentMap = (Map<String, Object>) dispatchElement(null, argumentsElement,
					viewContext);
		...
		try {
			Element contextElement = ViewConfigParserUtils.findContextElement(documentElement, context.getResource());
			if (contextElement != null) {
				Map<String, Object> attributes = (Map<String, Object>) dispatchElement(null, contextElement,
						viewContext);
			...
			// parse view element
			ViewDefinition viewDefinition = (ViewDefinition) dispatchElement(null, viewElement, viewContext);

			viewDefinition.setDependentResources(viewContext.getDependentResources().toArray(new Resource[0]));
		}
		finally {
