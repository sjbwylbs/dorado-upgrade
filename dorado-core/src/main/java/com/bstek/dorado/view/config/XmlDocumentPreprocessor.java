...
public class XmlDocumentPreprocessor {
	...
	private static Set<String> specialMergeProperties = new HashSet<>();
	...
	private Element findElementById(Document document, String id, PreparseContext context) throws Exception {
		Element element = null;
		Element documentElement = document.getDocumentElement();
		Element modelElement = ViewConfigParserUtils.findModelElement(documentElement, context.getResource());
		if (modelElement != null) {
			element = doFindElementById(modelElement, id);
		}
		if (element == null) {
			Element viewElement = ViewConfigParserUtils.findViewElement(documentElement, context.getResource());
	...
	private ViewConfigInfo getViewConfigModelInfo(String viewName) throws Exception {
		Object viewConfigFactory = viewConfigManager.getViewConfigFactory(viewName);
		if (viewConfigFactory instanceof ViewConfigDefinitionFactory) {
			...
			throw new XmlParseException("Can not find a according ViewDefinitionFactory for [" + viewName + "].");
		}
	}
	...
	private void processImports(Element element, PreparseContext context) throws Exception {
	...
	private void processPlaceHolders(Element element, TemplateContext templateContext) throws Exception {
		if (processPlaceHolders(DomUtils.getChildElements(element), templateContext)) {
			postProcessNodeReplacement(element);
		}
	}
	...
	private void mergeTemplateProperty(Element templteViewElement, Element viewElement, String property) {
	...
	private Element findPropertyElement(Element element, String propertyName) {
		for (Element propertyElement : DomUtils.getChildrenByTagName(element, XmlConstants.PROPERTY)) {
			if (propertyName.equals(propertyElement.getAttribute(XmlConstants.ATTRIBUTE_NAME))) {
	...
	private void gothroughPlaceHolders(Document templateDocument, TemplateContext templateContext) throws Exception {
	...
	protected void mergeMetaData(Document templateDocument, Element templteElement, Element element) {
		...
		if (metaDataElement != null) {
			if (templateMetaDataElement == null) {
				...
				templateMetaDataElement.setAttribute(XmlConstants.ATTRIBUTE_NAME, ViewXmlConstants.ATTRIBUTE_METADATA);
				templteElement.appendChild(templateMetaDataElement);
			}

			for (Element metaPropertyElement : DomUtils.getChildrenByTagName(metaDataElement, XmlConstants.PROPERTY)) {
	...
	private Map<String, Element> getArgumentElements(Document document, PreparseContext context) throws Exception {
		Map<String, Element> map = new HashMap<>();
		Element argumentsElement = ViewConfigParserUtils.findArgumentsElement(document.getDocumentElement(),
				context.getResource());
		if (argumentsElement != null) {
			for (Element argumentElement : DomUtils.getChildrenByTagName(argumentsElement, ViewXmlConstants.ARGUMENT)) {
				map.put(argumentElement.getAttribute(XmlConstants.ATTRIBUTE_NAME), argumentElement);
			}
		}
		return map;
	}
	...
	private Document loadTemplate(String templateViewName, TemplateContext templateContext) throws Exception {
		ViewConfigInfo viewConfigInfo = getViewConfigModelInfo(templateViewName);
		if (viewConfigInfo == null) {
			throw new XmlParseException("Template view not found [" + templateViewName + "].",
	...
	public Document process(String viewName, Document document) throws Exception {
		synchronized (document) {
			PreparseContext preparseContext = new PreparseContext(viewName);

			Element documentElement = document.getDocumentElement();
			String templateSrc = documentElement.getAttribute(ViewXmlConstants.ATTRIBUTE_TEMPALTE);
			if (StringUtils.isNotEmpty(templateSrc)) {
				TemplateContext templateContext = new TemplateContext(templateSrc, preparseContext, document);
				Document templateDocument = loadTemplate(templateSrc, templateContext);
				document = templateDocument;
				documentElement = document.getDocumentElement();
			}

			Element viewElement = ViewConfigParserUtils.findViewElement(documentElement, preparseContext.getResource());
...
class TemplateContext extends PreparseContext {
	...
	public TemplateContext(String viewName, PreparseContext sourceContext, Document sourceDocument) {
		super(viewName);
