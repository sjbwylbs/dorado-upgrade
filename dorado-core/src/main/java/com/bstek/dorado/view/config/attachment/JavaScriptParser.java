...
public class JavaScriptParser {
	...
	private void parseComment(CodeReader reader, boolean isLineComment, JavaScriptContext context) throws IOException {
						...
						if (len == FUNCTION_LEN && reserveWord.toString().equals(FUNCTION)) {
							...
							if (StringUtils.isNotEmpty(functionName)) {
								if (context.isHasAnnotation()) {
									...
									functionInfo.setShouldRegisterToGlobal(context.getCurrentShouldRegisterToGlobal());
									functionInfo.setShouldRegisterToView(context.getCurrentShouldRegisterToView());
									context.addFunctionInfo(functionInfo);
								}
							}
						}
						else if (len == ANONYMOUS_FUNCTION_LEN && reserveWord.toString().equals(ANONYMOUS_FUNCTION)) {
							reader.moveCursor(-1);
							...
							if (StringUtils.isEmpty(functionName) && context.isHasAnnotation()) {
								...
								if (currentBindingInfo != null) {
									...
									modifiedSource.append(source.substring(copyStart, copyEnd))
										.append(FUNCTION)
										.append(SPACE)
										.append(functionName);
