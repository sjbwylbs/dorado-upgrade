...
public class SocketLongTaskConnector implements SocketConnectionListener, SocketReceiveListener,
		TaskThreadExecutionListener, TaskThreadMessageListener {

	private static final Log logger = LogFactory.getLog(SocketLongTaskConnector.class);
	...
	public synchronized void addListener(SocketLongTaskConnectorListener listener) {
		if (listeners == null) {
	...
	public synchronized void removeListener(SocketLongTaskConnectorListener listener) {
		if (listeners != null) {
	...
	protected void fireClose() {
		if (listeners != null) {
			for (SocketLongTaskConnectorListener listener : listeners.toArray(new SocketLongTaskConnectorListener[0])) {
	...
	public void onStateChange(LongTaskThread taskThread, TaskStateInfo state) {
		try {
			socket.send(new Message("state", new LongTaskSocketServer.TaskStatePacket(taskThread)));
		}
		catch (Exception e) {
			logger.error(e, e);
		}
