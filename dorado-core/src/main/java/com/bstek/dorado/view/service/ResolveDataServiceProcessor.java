...
public class ResolveDataServiceProcessor extends DataServiceProcessorSupport {
	...
	public static final String RESOLVER_NAME_ATTRIBUTE = ResolveDataServiceProcessor.class.getName() + ".resolverName";

	private Outputter simplePropertyValueOnlyDataOutputter;

	private DataResolverManager dataResolverManager;

	public void setSimplePropertyValueOnlyDataOutputter(Outputter simplePropertyValueOnlyDataOutputter) {
	...
	protected DataResolver getDataResolver(String dataResolverName) throws Exception {
		DataResolver dataResolver;
		// 判断是否View中的私有DataResolver
		if (dataResolverName.startsWith(PRIVATE_VIEW_OBJECT_PREFIX)) {
			ParsedDataObjectName parsedName = new ParsedDataObjectName(dataResolverName);
			ViewConfig viewConfig = getViewConfig(DoradoContext.getCurrent(), parsedName.getViewName());
			dataResolver = viewConfig.getDataResolver(parsedName.getDataObject());
		}
		else {
			dataResolver = dataResolverManager.getDataResolver(dataResolverName);
		}
		return dataResolver;
	}
	...
	protected void doExecute(Writer writer, ObjectNode objectNode, DoradoContext context) throws Exception {
		String dataResolverName = JsonUtils.getString(objectNode, "dataResolver");
		...
		if (dataResolver == null) {
			throw new IllegalArgumentException("Unknown DataResolver [" + dataResolverName + "].");
		}

		ArrayNode jsonDataItems = (ArrayNode) objectNode.get("dataItems");
		DataItems dataItems = null;
		Map<String, UpdateInfo> updateInfos = new HashMap<>();
		...
		try {
			...
			for (Iterator it = jsonDataItems.iterator(); it.hasNext();) {
				...
				String refreshModeText = JsonUtils.getString(item, "refreshMode");
				RefreshMode refreshMode = (StringUtils.isEmpty(refreshModeText)) ? RefreshMode.value
						: RefreshMode.valueOf(refreshModeText);

				boolean autoResetEntityState = JsonUtils.getBoolean(item, "autoResetEntityState", true);

				JsonConvertContext jsonContext = new JsonConvertContextImpl(true, false, this);
				Object data = JsonUtils.toJavaObject(item.get("data"), null, null, true, jsonContext);

				dataItems.put(alias, data);
				updateInfos.put(alias, new UpdateInfo(refreshMode, autoResetEntityState, jsonContext));
			}
		...
		try {
			for (UpdateInfo updateInfo : updateInfos.values()) {
				...
				Collection<Object> entities = updateInfo.getJsonContext().getEntityCollection();
				if (entities != null) {
			...
			outputEntityStates(jsonBuilder, dataItems, updateInfos, outputContext, lastTimeStamp);
		}
		finally {
	...
	protected void outputEntityStates(JsonBuilder json, DataItems dataItems, Map<String, UpdateInfo> updateInfos,
			...
			if (RefreshMode.cascade.equals(refreshMode)) {
				if (item instanceof Collection<?>) {
					for (Object entity : ((Collection<?>) item)) {
						outputEntityDataAndState(json, entity, true, true, context, lastTimeStamp);
					}
				}
					...
					outputEntityDataAndState(json, item, true, true, context, lastTimeStamp);
				}
			}
			else {
				boolean shouldOutputData = RefreshMode.value.equals(refreshMode);
				for (Object entity : updateInfo.getJsonContext().getEntityCollection()) {
					outputEntityDataAndState(json, entity, shouldOutputData, false, context, lastTimeStamp);
				}
			}
		}
		json.endObject();
	...
	protected void outputEntityDataAndState(JsonBuilder json, Object entity, boolean shouldOutputData,
			boolean shouldOutputSubEntity, OutputContext context, long lastTimeStamp) throws Exception {
		EntityWrapper entityWrapper = EntityWrapper.create(entity);
		EntityState state = entityWrapper.getState();
		shouldOutputData = (shouldOutputData && state != EntityState.DELETED
				&& (shouldOutputSubEntity || entityWrapper.getTimeStamp() > lastTimeStamp));
		if (!shouldOutputData) {
...
class UpdateInfo {
	...
	public UpdateInfo(RefreshMode refreshMode, boolean autoResetEntityState, JsonConvertContext jsonContext) {
...
class JsonConvertContextImpl implements JsonConvertContext {
	...
	public JsonConvertContextImpl(boolean collectEntities, boolean collectEntityLists,
			DataTypeResolver dataTypeResolver) {
		if (collectEntities) {
			entityCollection = new ArrayList<>();
		}
		if (collectEntityLists) {
			entityListCollection = new ArrayList<>();
		}
