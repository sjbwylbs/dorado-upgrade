...
public class ObjectOutputter implements Outputter {
	...
	private Map<String, PropertyConfig> propertieConfigs = new HashMap<>();
	...
	protected void outputObjectProperties(Object object, OutputContext context) throws Exception {
		...
		if (defaultPropertyConfig != null && !defaultPropertyConfig.isIgnored()) {
			defaultPropertyOutputter = (PropertyOutputter) defaultPropertyConfig.getOutputter();
		}
		...
		for (String property : ((Map<String, ?>) beanMap).keySet()) {
			...
			if (propertyConfig != null) {
				...
				if (outputter != null && outputter instanceof VirtualPropertyOutputter) {
					continue;
				}
				...
				if (propertyOutputter != null && (hasEscapeValue || !propertyOutputter.isEscapeValue(value))) {
				...
				if (defaultPropertyOutputter != null && value != null && EntityUtils.isSimpleValue(value)
		...
		for (Map.Entry<String, PropertyConfig> entry : propertiesConfigs.entrySet()) {
			Object propertyOutputter = entry.getValue().getOutputter();
			if (propertyOutputter != null && propertyOutputter instanceof VirtualPropertyOutputter) {
				String property = entry.getKey();
				((VirtualPropertyOutputter) propertyOutputter).output(object, property, context);
			}
		}
	}
