...
public class ViewXmlParserHelper extends XmlParserHelper {
	...
	protected void doInitObjectParser(Context context, ObjectParser objectParser, XmlNodeInfo xmlNodeInfo,
		...
		if (Component.class.isAssignableFrom(beanType) && objectParser instanceof ComponentParser) {
			Widget widget = beanType.getAnnotation(Widget.class);
			if (widget != null) {
				((ComponentParser) objectParser).setComponentType(widget.name());
			}
		}
	}
	...
	protected XmlParserInfo getPropertyXmlParserInfo(Context context, Class<?> beanType, String propertyName,
		...
		if (xmlProperty != null && typeInfo != null && Object.class.equals(typeInfo.getType())
				&& typeInfo.isAggregated()) {
			XmlParser propertyParser = beanFactory.getBean(ITEMS_PARSER, XmlParser.class);
			return new XmlParserInfo(propertyName, propertyParser);
		}
		else {
			return super.getPropertyXmlParserInfo(context, beanType, propertyName, typeInfo, xmlProperty);
		}
	}
	...
	protected List<XmlParserInfo> getSubNodeXmlParserInfosByPropertyType(Context context, Class<?> propertyType)
			throws Exception {
		if (Component.class.equals(propertyType) || Control.class.equals(propertyType)
				|| DataControl.class.equals(propertyType) || FloatControl.class.equals(propertyType)) {
			List<XmlParserInfo> xmlParserInfos = new ArrayList<>();
			xmlParserInfos
				.add(new XmlParserInfo(WILCARD, beanFactory.getBean(COMPONENT_PARSER_DISPATCHER, XmlParser.class)));
			return xmlParserInfos;
		}
		else {
			return super.getSubNodeXmlParserInfosByPropertyType(context, propertyType);
		}
	}
