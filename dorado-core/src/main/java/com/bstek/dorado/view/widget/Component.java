 ...
 *
 */
@XmlNode(definitionType = "com.bstek.dorado.view.config.definition.ComponentDefinition",
		parser = "spring:dorado.componentParser")
@ClientObject(prototype = "dorado.widget.Component", shortTypeName = "Component",
...
class ComponentClientEventHolder extends ClientEventHolder {
	...
	public ComponentClientEventHolder(Component component) {
		super((Class<? extends ClientEventSupported>) ProxyBeanUtils.getProxyTargetType(component));
			...
			if (componentTypeRegisterInfo == null) {
				...
				if (!valid) {
					throw new IllegalArgumentException("Unrecognized client event ["
							+ componentTypeRegisterInfo.getName() + "," + eventName + "].");
				}
			}
		}
	}
