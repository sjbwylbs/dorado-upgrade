...
public class LongPollingServiceProcessor extends AbstractRemoteServiceProcessor {
	...
	private static final String NEW_SOCKET_GROUP_ID_KEY = LongPollingServiceProcessor.class.getName() + ".newGroupId";
	...
	protected void doExecute(Writer writer, ObjectNode objectNode, DoradoContext context) throws Exception {
		synchronized (this) {
			if (!inited) {
				inited = true;
				longPollingGroups = new ConcurrentHashMap<>();
	...
	protected Object processHandShake(Writer writer, ObjectNode objectNode, DoradoContext context) throws Exception {
		...
		if (StringUtils.isEmpty(groupId)) {
			groupId = (String) context.getAttribute(Context.THREAD, NEW_SOCKET_GROUP_ID_KEY);
			if (StringUtils.isEmpty(groupId)) {
				groupId = UUID.randomUUID().toString();
				context.setAttribute(Context.THREAD, NEW_SOCKET_GROUP_ID_KEY, groupId);
			}
		}
	...
	protected Object processDisconnect(Writer writer, ObjectNode objectNode, DoradoContext context) throws Exception {
		LongPollingSocket socket = getSocket(objectNode);
