...
@XmlNode(nodeName = "DataSet", definitionType = "com.bstek.dorado.view.config.definition.DataSetDefinition",
		parser = "spring:dorado.dataSetParser", clientTypes = { ClientType.DESKTOP, ClientType.TOUCH })
@ClientObject(prototype = "dorado.widget.DataSet", shortTypeName = "DataSet",
		properties = @ClientProperty(propertyName = "data", outputter = "spring:dorado.dataSetDataPropertyOutputter"))
@ClientEvents({ @ClientEvent(name = "beforeLoadData"), @ClientEvent(name = "onLoadData"),
...
public class DataSet extends Component {
	...
	public Object getData(boolean autoLoad) throws Exception {
		if (autoLoad && data == null && dataProvider != null) {
			if (pageSize > 0) {
				data = new PagingList(dataProvider, dataType, parameter, pageSize);
			}
			else {
