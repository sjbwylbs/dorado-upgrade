...
public class ViewServiceResolver extends AbstractTextualResolver {
	...
	public void addMethodInterceptor(MethodInterceptor methodInterceptor) {
		if (viewServiceInvoker != null) {
			throw new IllegalStateException("Can not add MethodInterceptor after the ViewServiceInvoker initialized.");
		}
	 ...
	 * @throws Exception
	 */
	protected void processTask(Writer writer, ObjectNode objectNode, DoradoContext context,
		...
		getViewServiceInvoker().invoke(action, processor, writer, objectNode, context, response);
	}
	 ...
	 * @param e
	 */
	protected void outputException(JsonBuilder jsonBuilder, String message, Throwable throwable) {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (contentType == null || contentType.contains(JAVASCRIPT_TOKEN)) {
				...
				Document document = getXmlDocumentBuilder(context).loadDocument(new InputStreamResource(in, uri));
			...
			if (t instanceof ClientRunnableException) {
				...
				writer.append("var fn=(function(){").append(((ClientRunnableException) t).getScript()).append("});");
			}
			else {
