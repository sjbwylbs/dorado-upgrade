...
public class BootPackagesResolver extends WebFileResolver {
	...
	private static class PackageResource implements Resource {
		...
		public PackageResource(PackagesConfig packagesConfig, String skin, int clientType, Locale locale)
				throws Exception {
			...
			if (TempFileUtils.isSupportsTempFile()) {
				...
				buf.append(ClientType.toString(clientType)).append('-').append(skin).append('-');
				if (WebConfigure.getBoolean("view.useMinifiedJavaScript")) {
					buf.append("min-");
				}
				File file = TempFileUtils.createTempFile(buf.toString(), JAVASCRIPT_SUFFIX);
		 ...
		 * @throws IOException
		 */
		protected void outputPackagesConfig(Writer writer, PackagesConfig packagesConfig, int targetClientType)
				throws Exception {
			...
			outputProperty(writer, CLIENT_PACKAGES_CONFIG, packagesConfig, "defaultCharset", null);
			outputProperty(writer, CLIENT_PACKAGES_CONFIG, packagesConfig, "defaultContentType", null);

			JsonBuilder jsonBuilder = new JsonBuilder(writer);
			jsonBuilder.setPrettyFormat(Configure.getBoolean("view.outputPrettyJson"));
			...
			if (packages != null) {
				...
				for (Map.Entry<String, Package> entry : packages.entrySet()) {
					...
					if (pkg.getClientType() != 0) {
						if (!ClientType.supports(pkg.getClientType(), targetClientType)) {
							continue;
						}
					}
					jsonBuilder.key(entry.getKey());
					outputPackage(jsonBuilder, packagesConfig, pkg, targetClientType);
				}
				jsonBuilder.endObject();
				writer.append(";\n");
			}
		...
		protected void outputPattern(JsonBuilder jsonBuilder, Pattern pattern, String skin) throws Exception {
			jsonBuilder.object();
			...
			jsonBuilder.key("url").value(PathUtils.concatPath(baseUri, "${fileName}.dpkg?" + parameters));
			if (pattern.isMergeRequests()) {
				jsonBuilder.escapeableKey("mergeRequests").value(pattern.isMergeRequests());
			}
			jsonBuilder.endObject();
		}

		protected void outputPackage(JsonBuilder jsonBuilder, PackagesConfig packagesConfig, Package pkg,
				int targetClientType) throws Exception {
			jsonBuilder.object();
			...
			if (depends != null && depends.size() > 0) {
				...
				for (String depend : depends) {
					...
					if (dependPkg.getClientType() != 0) {
						if (!ClientType.supports(dependPkg.getClientType(), targetClientType)) {
							continue;
						}
					}
			...
			if (!pkg.isMergeRequests() && !OutputUtils.isEscapeValue(pkg.getFileNames())) {
				jsonBuilder.key("fileNames").value(pkg.getFileNames());
			}
			jsonBuilder.endObject();
		}

		private void outputProperty(Writer writer, String owner, Object object, String property, Object escapeValue)
				throws Exception {
		...
		public String getDescription() {
			return (adapteeResource != null) ? adapteeResource.getDescription() : null;
		}
		...
		public String getFilename() {
			return (adapteeResource != null) ? adapteeResource.getFilename() : null;
		}
		...
		public long getTimestamp() throws IOException {
			return (adapteeResource != null) ? adapteeResource.getTimestamp() : 0L;
		}
	...
	protected String getResourceCacheKey(HttpServletRequest request) throws Exception {
		...
		buf.append('-')
			.append(request.getParameter("clientType"))
			.append('-')
			.append(request.getParameter("skin"))
			.append('-')
	...
	protected ResourcesWrapper createResourcesWrapper(HttpServletRequest request, DoradoContext context)
		...
		Resource[] bootResourceArray = getResourcesByFileName(context, resourcePrefix, bootFile, JAVASCRIPT_SUFFIX);

		Resource[] resourceArray = new Resource[bootResourceArray.length + 1];
		System.arraycopy(bootResourceArray, 0, resourceArray, 0, bootResourceArray.length);

		PackagesConfig packagesConfig = getPackagesConfigManager().getPackagesConfig();
		Resource resource = new PackageResource(packagesConfig, skin, clientType, localeResolver.resolveLocale());
