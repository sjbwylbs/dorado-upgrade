...
public class PackagesConfigManager {
	...
	private ResourceManager getResourceManager() {
		if (resourceManager == null) {
			resourceManager = ResourceManagerUtils.get(PackagesConfigManager.class);
		}
	...
	public synchronized PackagesConfig getPackagesConfig() throws Exception {
		if (packagesConfig == null) {
			if (configLocations != null) {
				...
				packagesConfig = loadConfigs(ResourceUtils.getResources(locations));
				...
				for (Package pkg : packages.values()) {
					...
					for (String depends : pkg.getDepends()) {
						if (packages.get(depends) == null) {
							throw new IllegalArgumentException(getResourceManager()
								.getString("dorado.common/unknownDependsPackage", name, depends));
						}
					}
					...
					for (String dependedBy : pkg.getDependedBy()) {
						...
						if (dependedPackage == null) {
							throw new IllegalArgumentException(getResourceManager()
								.getString("dorado.common/unknownDependedByPackage", name, dependedBy));
						}
	...
	protected PackagesConfig loadConfigs(Resource[] resources) throws Exception {
		...
		PackagesConfigParseContext context = new PackagesConfigParseContext(packagesConfig);
	 ...
	 * @throws Exception
	 */
	protected void loadConfig(Resource resource, PackagesConfigParseContext context) throws Exception {
