...
public class LoadDataServiceProcessor extends DataServiceProcessorSupport {
	...
	public static final String PROVIDER_NAME_ATTRIBUTE = ResolveDataServiceProcessor.class.getName() + ".providerName";
	...
	protected DataProvider getDataProvider(String dataProviderName) throws Exception {
		DataProvider dataProvider;
		// 判断是否View中的私有DataProvider
		if (dataProviderName.startsWith(PRIVATE_VIEW_OBJECT_PREFIX)) {
			ParsedDataObjectName parsedName = new ParsedDataObjectName(dataProviderName);
			...
			dataProvider = dataProviderManager.getDataProvider(dataProviderName);
		}
		return dataProvider;
	}

	private void processPreload(Object data, DataPreloadConfig dataPreloadConfig, int depth) throws Exception {
	...
	private void processEntityPreload(Object entity, DataPreloadConfig dataPreloadConfig, int depth) throws Exception {
		Object value = EntityUtils.getValue(entity, dataPreloadConfig.getProperty());
		if (value != null) {
			...
			if (dataPreloadConfig.getChildPreloadConfigs() != null) {
				for (DataPreloadConfig childDataPreloadConfig : dataPreloadConfig.getChildPreloadConfigs()) {
	...
	protected void preload(Object data, Collection<DataPreloadConfig> dataPreloadConfigs) throws Exception {
		...
		if (rudeCriteria.has("orders")) {
			...
			if (orders != null) {
				for (Iterator<JsonNode> it = orders.iterator(); it.hasNext();) {
					...
					Order order = new Order(JsonUtils.getString(rudeCriterion, "property"),
							JsonUtils.getBoolean(rudeCriterion, "desc"));
	...
	protected Criterion parseCriterion(ObjectNode rudeCriterion) throws Exception {
		...
		if (StringUtils.isNotEmpty(junction)) {
			...
			return filterCriterionParser.createFilterCriterion(property, dataType, expression);
		}
	}
	...
	protected void doExecute(Writer writer, ObjectNode objectNode, DoradoContext context) throws Exception {
		String dataProviderName = JsonUtils.getString(objectNode, "dataProvider");
		...
		if (objectNode.has("resultDataType")) {
			resultDataTypeName = JsonUtils.getString(objectNode, "resultDataType");
		}

		JsonNode rudeParameter = objectNode.get("parameter");
		ObjectNode rudeSysParameter = (ObjectNode) objectNode.get("sysParameter");
		...
		if (rudeSysParameter != null) {
			if (rudeSysParameter.has("preloadConfig")) {
				dataPreloadConfigs = (Collection<DataPreloadConfig>) JsonUtils
					.toJavaObject(rudeSysParameter.remove("preloadConfig"), getDataType("[DataPreloadConfig]"));
			}
		...
		if (rudeSysParameter != null) {
			sysParameter = (MetaData) jsonToJavaObject(rudeSysParameter, null, null, false);
			if (rudeCriteria != null && rudeCriteria instanceof ObjectNode) {
				sysParameter.put("criteria", getCriteria((ObjectNode) rudeCriteria));
			}
		...
		if (dataProvider == null) {
			throw new IllegalArgumentException("Unknown DataProvider [" + dataProviderName + "].");
		}
