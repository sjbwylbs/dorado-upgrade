...
public class JsonBuilder {
	...
	private JsonBuilder append(String s, boolean skipNull, boolean quote) {
		...
		if (state == 'i' || state == 'o' || state == 'k' || state == 'a' || state == 'v') {
			try {
				if (state == 'a') {
					if (comma) {
						write(',');
	...
	public JsonBuilder value(Object o) {
		if (o == null) {
			...
			if (o instanceof Float || o instanceof Double) {
				if (o instanceof Float) {
					if ((Float.isNaN((Float) o))) {
						...
						outputValue(getNumberFormat().format(((Number) o).floatValue()), false);
					}
				}
				else {
					if ((Double.isNaN((Double) o))) {
						...
						outputValue(getNumberFormat().format(((Number) o).doubleValue()), false);
					}
				}
			}
			else {
			...
			if (s.length() == 0) {
				...
				outputValue(StringEscapeUtils.escapeEcmaScript(s), true);
			}
		}
		return this;
	}
