...
public class AssembledComponentDefOutputter implements VirtualPropertyOutputter {
	...
	public void output(Object object, String property, OutputContext context) throws Exception {
		...
		if (componentTypeRegisterInfo != null) {
			...
			Map<String, VirtualPropertyDescriptor> virtualProperties = componentTypeRegisterInfo.getVirtualProperties();
			if (virtualProperties != null && !virtualProperties.isEmpty()) {
				json.escapeableKey("ATTRIBUTES").escapeableObject();
				for (Map.Entry<String, VirtualPropertyDescriptor> entry : virtualProperties.entrySet()) {
					VirtualPropertyDescriptor propertyDescriptor = entry.getValue();
					...
					if (VirtualPropertyAvialableAt.both.equals(avialableAt)
							|| VirtualPropertyAvialableAt.client.equals(avialableAt)) {
			...
			Map<String, VirtualEventDescriptor> virtualEvents = componentTypeRegisterInfo.getVirtualEvents();
