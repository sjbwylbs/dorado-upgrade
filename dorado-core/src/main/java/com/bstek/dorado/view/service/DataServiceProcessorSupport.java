...
public abstract class DataServiceProcessorSupport implements ServiceProcessor, DataTypeResolver {
	...
	public static class ParsedDataObjectName {
		...
		public ParsedDataObjectName(String name) {
			...
			String viewSectionAlias = name.substring(PRIVATE_VIEW_OBJECT_PREFIX.length(), i);
			dataObject = name.substring(i + 1);

			String viewSection = StringAliasUtils.getOriginalString(viewSectionAlias);
			if (viewSection == null) {
				throw new IllegalArgumentException("Invalid ViewAlias [" + viewSectionAlias + "].");
			}
	...
	protected static final String VIEW_STATE_ATTRIBUTE_KEY = ViewState.class.getName();
	...
	@Override
	public final void execute(Writer writer, ObjectNode objectNode, DoradoContext context, HttpServletResponse response)
			throws Exception {
		ViewState originViewState = (ViewState) context.getAttribute(VIEW_STATE_ATTRIBUTE_KEY);
	...
	protected abstract void doExecute(Writer writer, ObjectNode objectNode, DoradoContext context) throws Exception;

	protected void outputResult(Object result, OutputContext context) throws Exception {
		DoradoContext doradoContext = DoradoContext.getCurrent();
		Map<String, Object> viewContext = DoradoContextUtils.getViewContext(doradoContext);
		...
		if (context.isShouldOutputDataTypes() || viewContext != null) {
			...
			if (context.isShouldOutputDataTypes()) {
				...
				if (includeDataTypes != null && !includeDataTypes.isEmpty()) {
					Map<String, DataType> outputDataTypes = new HashMap<>();
	...
	protected void outputDataTypes(Map<String, DataType> includeDataTypes, OutputContext outputContext)
			throws Exception {
		includeDataTypesOutputter.output(includeDataTypes, outputContext);
	}

	protected void outputData(Object result, OutputContext outputContext) throws Exception {
		dataOutputter.output(result, outputContext);
	}

	protected ViewConfig getViewConfig(DoradoContext context, String viewName) throws Exception {
			...
			if (dataType == null) {
				...
				if (dtn.getSubDataTypes().length == 1) {
					...
					if (subName.startsWith(PRIVATE_VIEW_OBJECT_PREFIX)) {
						...
						name = new StringBuffer().append(dtn.getOriginDataType())
							.append('[')
							.append(parsedName.getDataObject())
							.append(']')
							.toString();
					}
				}
	...
	protected Object jsonToJavaObject(JsonNode jsonNode, DataType dataType, Class<?> targetType, boolean proxy)
			throws Exception {
		JsonConvertContext jsonContext = new JsonConvertContextImpl(false, false, this);
		return JsonUtils.toJavaObject(jsonNode, dataType, targetType, proxy, jsonContext);
