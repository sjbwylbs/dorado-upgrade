...
public class XmlViewConfigDefinitionFactory implements ViewConfigDefinitionFactory {

	private static final Log logger = LogFactory.getLog(XmlViewConfigDefinitionFactory.class);
	...
	private byte getNameDelimMode() {
		if (nameDelimMode == 0) {
			String setting = Configure.getString("view.viewNameDelim", nameDelimDot);
	...
	protected String getResourcePath(String viewName, String pathSubfix) throws Exception {
	...
	protected Resource getResource(String viewName, String pathSubfix) throws Exception {
		...
		if (resources.length == 0) {
			...
			throw new IllegalArgumentException("More than one resources found by path [" + path + "].");
		}
		return resources[0];
	}

	private String getResourcePath(String viewName, String pathPrefix, String pathSubfix) {
	...
	protected ViewConfigInfo doGetViewConfigInfo(String viewName) throws Exception {
		...
		Element viewElement = ViewConfigParserUtils.findViewElement(document.getDocumentElement(),
				preparseContext.getResource());
		Resource tempResource;

		if (xmlPreprocessor.getPropertyValue(viewElement, ViewXmlConstants.ATTRIBUTE_PAGE_TEMPALTE) == null) {
			tempResource = getResource(viewName, ".html");
			if (tempResource.exists()) {
				viewElement.setAttribute(ViewXmlConstants.ATTRIBUTE_PAGE_TEMPALTE, getResourcePath(viewName, ".html"));
			}
		}

		tempResource = getResource(viewName, ".js");
		if (tempResource.exists()) {
			String originJavaScriptFile = xmlPreprocessor.getPropertyValue(viewElement,
					ViewXmlConstants.ATTRIBUTE_JAVASCRIPT_FILE);
			String javaScriptFile = getResourcePath(viewName, ".js");
			viewElement.setAttribute(ViewXmlConstants.ATTRIBUTE_JAVASCRIPT_FILE,
					(StringUtils.isEmpty(originJavaScriptFile) ? javaScriptFile
							: originJavaScriptFile + ',' + javaScriptFile));
		}

		tempResource = getResource(viewName, ".css");
		if (tempResource.exists()) {
			String originStyleSheetFile = xmlPreprocessor.getPropertyValue(viewElement,
					ViewXmlConstants.ATTRIBUTE_STYLESHEET_FILE);
			String styleSheetFile = getResourcePath(viewName, ".css");
			viewElement.setAttribute(ViewXmlConstants.ATTRIBUTE_STYLESHEET_FILE,
					(StringUtils.isEmpty(originStyleSheetFile) ? styleSheetFile
	...
	protected ViewConfigDefinition doCreate(String viewName) throws Exception {
		...
		String viewObjectNamePrefix = ViewXmlConstants.PATH_VIEW_SHORT_NAME + Constants.PRIVATE_DATA_OBJECT_SUBFIX
				+ StringAliasUtils.getUniqueAlias(viewName) + ViewXmlConstants.PATH_COMPONENT_PREFIX;
		...
		privateDataTypeDefinitionManager.setDataObjectIdPrefix(viewObjectNamePrefix);
		parseContext.setDataTypeDefinitionManager(privateDataTypeDefinitionManager);
		...
		privateDataProviderDefinitionManager.setDataObjectIdPrefix(viewObjectNamePrefix);
		parseContext.setDataProviderDefinitionManager(privateDataProviderDefinitionManager);
		...
		privateDataResolverDefinitionManager.setDataObjectIdPrefix(viewObjectNamePrefix);
		parseContext.setDataResolverDefinitionManager(privateDataResolverDefinitionManager);

		Element documentElement = document.getDocumentElement();
		ViewConfigDefinition viewConfigDefinition = (ViewConfigDefinition) xmlParser.parse(documentElement,
				parseContext);
		...
		for (Map.Entry<String, DataTypeDefinition> entry : parsedDataTypes.entrySet()) {
			dataTypeDefinitionManager.registerDefinition(entry.getKey(), entry.getValue());
		}
		...
		for (Map.Entry<String, DataProviderDefinition> entry : parseContext.getParsedDataProviders().entrySet()) {
			dataProviderDefinitionManager.registerDefinition(entry.getKey(), entry.getValue());
			logger.info("Registered data provider key " + entry.getKey() + "key " + entry.getValue());
		}
		...
		for (Map.Entry<String, DataResolverDefinition> entry : parseContext.getParsedDataResolvers().entrySet()) {
			dataResolverDefinitionManager.registerDefinition(entry.getKey(), entry.getValue());
			logger.info("Registered data resolver key " + entry.getKey() + "key " + entry.getValue());
		}
