...
public class ComponentParserDispatcher implements XmlParser {
	...
	public Object parse(Node node, ParseContext context) throws Exception {
		...
		if (registerInfo != null) {
			...
			componentParser = (ComponentParser) xmlParserHelper.getXmlParser(classType);
			...
			if (registerInfo instanceof AssembledComponentTypeRegisterInfo) {
				...
				componentParser.setAssembledComponentDefinition(superComponentDefinition);
			}
		}
		else {
			throw new XmlParseException("Unrecognized Component type [" + type + "].", element, context);
		}
		...
		ComponentTypeRegisterInfo originRegisterInfo = viewContext.getCurrentComponentTypeRegisterInfo();
