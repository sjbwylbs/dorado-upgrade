...
public class ContainerParser extends ControlParser {
	...
	protected Object internalParse(Node node, ParseContext context) throws Exception {
		...
		Node layoutNode = element.getAttributeNode(ViewXmlConstants.ATTRIBUTE_LAYOUT);
		if (layoutNode != null) {
		...
		if (layout != null) {
			layoutTypeInfo = layoutTypeRegistry.getRegisterInfo(layout.getType());
		}
		viewContext.setCurrentLayout(new ViewParseContext.LayoutInfo(element, layoutTypeInfo));

		ContainerDefinition containerDefinition = (ContainerDefinition) super.internalParse(node, context);
		if (layout != null) {
	...
	protected Map<String, Object> parseProperties(Element element, ParseContext context) throws Exception {
		...
		boolean restored = (layoutInfo != null && layoutInfo.getContainerElement() == element);
		if (restored) {
			viewContext.restoreCurrentLayout();
		}
		try {
