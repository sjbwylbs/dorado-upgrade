...
public class ComponentDefinition extends ListenableObjectDefinition implements Identifiable {
	...
	@Override
	protected BeanWrapper createObject(CreationInfo creationInfo, Object[] constructorArgs,
		...
		BeanWrapper wrapper = super.createObject(creationInfo, constructorArgs, methodInterceptors, context);
		if (assembledComponentDefinition != null) {
			BeanExtender.setExProperty(wrapper.getBean(), "$assembledComponentInfo", registerInfo);
		}
		return wrapper;
	}
	...
	protected Object doCreate(CreationContext context, Object[] constructorArgs) throws Exception {
		...
		if (assembledComponentDefinition != null) {
			ExpressionHandler expressionHandler = (ExpressionHandler) Context.getCurrent()
				.getServiceBean("expressionHandler");
			jexlContext = expressionHandler.getJexlContext();

			originAcomp = (AssembledComponentExpressionObject) jexlContext.get("acomp");
			AssembledComponentExpressionObject acomp = new AssembledComponentExpressionObject(getProperties());
			...
			if (!assembledComponentParentInited) {
				...
				if (parentReferences == null) {
					...
					DirectDefinitionReference<ComponentDefinition> definitionReference = new DirectDefinitionReference<>(
					...
					System.arraycopy(parentReferences, 0, newParentReferences, 1, parentReferences.length);
					setParentReferences(newParentReferences);
				}
			}
		}
		try {
			Component component = (Component) super.doCreate(context, constructorArgs);
			return component;
		}
		finally {
			if (assembledComponentDefinition != null) {
	...
	protected void doInitObject(Object object, CreationInfo creationInfo, CreationContext context) throws Exception {
		if (assembledComponentDefinition != null) {
			for (String property : ((AssembledComponentTypeRegisterInfo) registerInfo).getVirtualProperties()
				.keySet()) {
	...
	protected boolean invokePrivateListener(Object object, String listenerName, String methodName,
		...
		try {
			...
			retval = MethodAutoMatchingUtils.invokeMethod(methods, interceptor, requiredTypes, requiredArgs, exactTypes,
