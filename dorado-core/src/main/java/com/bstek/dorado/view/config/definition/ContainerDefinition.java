...
public class ContainerDefinition extends ControlDefinition {
	...
	private List<ComponentDefinition> children = new ArrayList<>();
	...
	protected void doInitObject(Object object, CreationInfo creationInfo, CreationContext context) throws Exception {
		...
		List<ComponentDefinition> allChilldrens = (List<ComponentDefinition>) creationInfo.getUserData("children");
		if (allChilldrens != null) {
			for (Object childDefinition : allChilldrens) {
				Component child = (Component) DefinitionUtils.getRealValue(childDefinition, context);
	...
	@SuppressWarnings("unchecked")
	protected void initChildrenCreationInfo(CreationInfo creationInfo, ObjectDefinition definition) {
		ContainerDefinition containerDefinition = (ContainerDefinition) definition;

		if (containerDefinition.getLayout() != null) {
			creationInfo.getProperties().put("layout", containerDefinition.getLayout());
		}

		List<ComponentDefinition> allChilldrens = (List<ComponentDefinition>) creationInfo.getUserData("children");
		if (allChilldrens == null) {
			allChilldrens = new ArrayList<>();
	...
	protected Object clone() throws CloneNotSupportedException {
		...
		List<ComponentDefinition> children = new ArrayList<>();
		for (ComponentDefinition componentDefinition : definition.children) {
			children.add(((componentDefinition != null) ? CloneUtils.clone(componentDefinition) : null));
		}
