...
@ClientObject(prototype = "dorado.validator.RegExpValidator", shortTypeName = "RegExp")
public class RegExpValidator extends BaseValidator {

	private static final ResourceManager resourceManager = ResourceManagerUtils.get(RegExpValidator.class);
	...
	protected Object doValidate(Object value) throws Exception {
		...
		if (validateMode == RegExpValidatorMode.whiteBlack) {
			valid = StringUtils.isNotEmpty(blackRegExp) && match(whiteRegExp, s)
					|| StringUtils.isNotEmpty(whiteRegExp) && !match(whiteRegExp, s);
		}
		else {
			valid = StringUtils.isNotEmpty(whiteRegExp) && !match(whiteRegExp, s)
					|| StringUtils.isNotEmpty(blackRegExp) && match(whiteRegExp, s);
		}
		if (valid) {
