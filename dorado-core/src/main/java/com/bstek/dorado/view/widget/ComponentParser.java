...
public class ComponentParser extends GenericObjectParser implements Cloneable {
	...
	protected Object internalParse(Node node, ParseContext context) throws Exception {
		ComponentDefinition component = (ComponentDefinition) super.internalParse(node, context);
	 ...
	 * @param component 组件配置声明对象
	 * @param context 解析上下文
	 */
	protected void registerComponent(ComponentDefinition component, ViewParseContext context) {
		ViewDefinition viewDefinition = context.getViewConfigDefinition().getViewDefinition();
	...
	protected ObjectDefinition createDefinition(Element element, ParseContext context) throws Exception {
		Constructor<? extends ObjectDefinition> constructor = getDefinitionType().getConstructor(PARAMETER_TYPES);
		return constructor
			.newInstance(new Object[] { ((ViewParseContext) context).getCurrentComponentTypeRegisterInfo() });
	}
