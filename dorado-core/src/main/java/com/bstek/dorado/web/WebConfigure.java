 ...
 * .bbb.ccc的属性，那么通过WebConfigure读到的将是会话中的属性值，而不是原先的Configure中的值。 WebConfigure总是按照下面的顺序来搜索配置
...
public abstract class WebConfigure {
	 ...
	 * 在Request或Session中设置一个同名的值为null的属性，并不能达到这一目的。 应该为该属性设置一个特殊的值用于代表null，即 {@link #NULL}
	 */
	...
	private static ConfigureWrapper store = new ConfigureWrapper(Configure.getStore());
	...
	public static ConfigureStore getStore(String scope) {
		if (DoradoContext.APPLICATION.equals(scope) || StringUtils.isEmpty(scope)) {
			return store;
		}
		else {
			DoradoContext context = DoradoContext.getCurrent();
			Map<String, Object> localConfigureMap = (Map<String, Object>) context.getAttribute(WebConfigure.STORE_KEY);
			return (localConfigureMap != null) ? new MapConfigureStore(localConfigureMap) : EMPTY_STORE;
		}
	}
...
class ConfigureWrapper extends ConfigureStore {
	...
	public boolean contains(String key) {
		...
		if (context instanceof DoradoContext) {
			...
			localConfigureMap = (Map<String, Object>) doradoContext.getAttribute(DoradoContext.SESSION,
					WebConfigure.STORE_KEY);
			if (localConfigureMap != null) {
				b = localConfigureMap.containsKey(key);
				if (b) {
					return true;
	...
	public void remove(String scope, String key) {
		if (DoradoContext.APPLICATION.equals(scope) || StringUtils.isEmpty(scope)) {
			...
			DoradoContext context = DoradoContext.getCurrent();
			Map<String, Object> localConfigureMap = (Map<String, Object>) context.getAttribute(scope,
					WebConfigure.STORE_KEY);
			if (localConfigureMap == null) {
				localConfigureMap = new HashMap<>();
				context.setAttribute(scope, WebConfigure.STORE_KEY, localConfigureMap);
			}
