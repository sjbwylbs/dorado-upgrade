 ...
 * 通过HttpContext用户可以取到当前线程相关的{@link jakarta.servlet.http.HttpServletRequest}对象。
 * </p>
 *
 */
public class DoradoContext extends SpringContextSupport {
	...
	protected ResourceLoader getResourceLoader() {
		if (resourceLoader == null) {
			try {
				String resourceLoaderClass = Configure.getString(RESOURCE_LOADER_PROPERTY);
				Assert.notEmpty(resourceLoaderClass, "\"" + RESOURCE_LOADER_PROPERTY + "\" not configured.");
				...
				if (WebResourceLoader.class.isAssignableFrom(cl)) {
					...
					if (wac == null) {
						...
						resourceLoader = (ResourceLoader) cl.getDeclaredConstructor().newInstance();
						((WebResourceLoader) resourceLoader).setWebApplicationContext(wac);
					}
				}
				else {
					resourceLoader = (ResourceLoader) cl.getDeclaredConstructor().newInstance();
	...
	public static WebApplicationContext getAttachedWebApplicationContext() {
		...
		if (context instanceof DoradoContext) {
			...
			throw new UnsupportedOperationException("Method not supported in current Thread.");
		}
	}
	...
	/**
	 * 返回当前线程相关的{@link jakarta.servlet.http.HttpServletRequest}对象。
	 */
	 ...
	 * @param request 请求对象。
	 * @param servletContext Spring中的WebApplicationContext对象。
	 * @return 上下文对象。
	 */
	public static DoradoContext init(ServletContext servletContext, HttpServletRequest request) {
		DoradoContext context = (DoradoContext) request.getAttribute(ATTRIBUTE_KEY);
		if (context == null) {
	...
	public static DoradoContext init(ServletContext servletContext, boolean attachToThread) {
	...
	public static void dispose(HttpServletRequest request) {
		...
		Context context = dettachFromThreadLocal();
			...
			if (hasAttribute(request.getAttributeNames(), key)) {
				...
				if (session != null) {
					if (hasAttribute(session.getAttributeNames(), key)) {
						...
						if (hasAttribute(servletContext.getAttributeNames(), key)) {
	...
	protected void throwsViewContextNotAvailable() {
		throw new IllegalStateException("The ViewContext is currently not available.");
	}

	/**
	 * 返回指定范围内某属性的值。
	 * @param scope 范围。可使用的值包括{@link #THREAD}、{@link #REQUEST}、{@link #VIEW}、
