...
public class ResourceTypeParser implements XmlParser {
	...
	public Object parse(Node node, ParseContext context) throws Exception {
		List<Element> typeElements = DomUtils.getChildrenByTagName((Element) node, "resource-type");
	...
	private void parseTypeElement(Element typeElement, ParseContext context) throws Exception {
		String type = typeElement.getAttribute("type");
		Assert.notEmpty(type);
		String contentType = typeElement.getAttribute("content-type");
		boolean compressible = Boolean.parseBoolean(typeElement.getAttribute("compressible"));

		ResourceTypeManager resourceTypeManager = ((ResourceTypeParseContext) context).getResourceTypeManager();
		resourceTypeManager.registerResourceType(new ResourceType(type, contentType, compressible));
	}
