...
public class ResourceTypeLoader extends EngineStartupListener {
	...
	protected synchronized void loadConfigs(ResourceTypeManager resourceTypeManager, Resource[] resources)
			throws Exception {
		...
		for (Resource resource : resources) {
			if (resource.exists()) {
				...
				resourceTypeParser.parse(document.getDocumentElement(), context);
			}
		}
	}
	...
	public void onStartup() throws Exception {
		if (StringUtils.isNotEmpty(configLocation)) {
			loadConfigs(resourceTypeManager, ResourceUtils.getResources(configLocation));
		}

		if (configLocations != null) {
			String[] locations = new String[configLocations.size()];
			configLocations.toArray(locations);
			loadConfigs(resourceTypeManager, ResourceUtils.getResources(locations));
		}
	}
