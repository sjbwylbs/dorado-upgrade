...
public abstract class Definition implements Cloneable {
	 ...
	 * <li>配置声明对象的引用 {@link com.bstek.dorado.config.definition.DefinitionReference}
	 * ，在最终初始化对象时将被转换成具体的数据或对象。</li>
	 * </ul>
	 */
	...
	/**
	 * 将一系列属性值初始化到给定的对象中。
	 * @param object 被初始化的对象。
	 * @param properties 属性值的集合。
	 ...
	 * @param object 被初始化的对象。
	 * @param property 属性名。
	 * @param value 属性值。@see {@link #getProperties()}
	 * @param context 创建最终对象的上下文。
	 * @throws Exception
	 */
		...
		if (object instanceof Map) {
			...
			if (propertyDescriptor != null) {
				...
				if (writeMethod != null) {
					...
					throw new NoSuchMethodException("Property [" + property + "] of [" + object + "] is not writable.");
				}
			}
			else {
	 ...
	 * @param operation 初始化操作
	 */
	public void addInitOperation(Operation operation) {
		if (initOperations == null) {
			initOperations = new ArrayList<>();
			unmodifiableInitOperations = Collections.unmodifiableList(initOperations);
		}
	...
	public List<Operation> getInitOperations() {
		return (unmodifiableInitOperations != null) ? unmodifiableInitOperations : Collections.EMPTY_LIST;
	}

	/**
	 * 执行一系列初始化操作。
	 * @param object 被初始化的对象
	 * @param operations 初始化操作的集合
	 * @param context 创建最终对象的上下文
	 * @throws Exception
	 */
	...
	protected Object clone() throws CloneNotSupportedException {
		...
		if (initOperations != null) {
			...
			definition.unmodifiableInitOperations = Collections.unmodifiableList(definition.unmodifiableInitOperations);
		}
		return definition;
	}
