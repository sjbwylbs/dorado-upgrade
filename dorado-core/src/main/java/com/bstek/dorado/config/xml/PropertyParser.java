...
public class PropertyParser extends ConfigurableDispatchableXmlParser {
	...
	protected Object getRealValue(Object value) {
		return (value instanceof Expression) ? ((Expression) value).evaluate() : value;
	}
	...
	protected Object doParse(Node node, ParseContext context) throws Exception {
		...
		if (node instanceof Element) {
			...
			if (!valueParsed) {
				Element propertyElement = DomUtils.getChildByTagName(element, XmlConstants.PROPERTY);
				if (propertyElement != null && XmlConstants.ATTRIBUTE_VALUE
					.equals(propertyElement.getAttribute(XmlConstants.ATTRIBUTE_NAME))) {
		...
		if (value instanceof Expression) {
			...
			if (shouldEvaluateExpression() && expression.getEvaluateMode().equals(EvaluateMode.onInstantiate)) {
