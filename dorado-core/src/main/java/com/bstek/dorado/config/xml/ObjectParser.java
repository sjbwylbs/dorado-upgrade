...
public class ObjectParser extends ConfigurableDispatchableXmlParser {
	...
	/**
	 * 设置要生成配置声明对象的实现类类型。例如:com.bstek.dorado.data.config.definition. DataTypeDefinition 。
	 ...
	 * 注意，有些配置声明对象并不支持复杂的实例化方式。如果将{@link #setSupportsExpressionImpl(boolean)} 属性设置为false，
	 * 那么此解析器将仅支持直接以Class名描述的实例化方式。
	 *
	 ...
	 * 例如根据parent="hr.Employee,hr.Manager"，可得到两个分别指向hr.Employee和hr. Manager这两个配置声明对象的引用。
	 * @param parentNameText parent属性的配置信息
	 * @param context 解析上下文
	...
	protected void initDefinition(ObjectDefinition definition, Element element, ParseContext context) throws Exception {
		...
		if (StringUtils.isNotEmpty(realImpl)) {
			/*
			 * if (StringUtils.isNotEmpty(parent)) { throw new XmlParseException("[" +
			 * XmlConstants.ATTRIBUTE_PARENT + "] attribute should be empty", element,
			 * context); }
			 */
		...
		if (StringUtils.isNotEmpty(parent)) {
			if (!inheritable) {
				throw new XmlParseException("[" + XmlConstants.ATTRIBUTE_PARENT + "] attribute not supported.", element,
						context);
			}
