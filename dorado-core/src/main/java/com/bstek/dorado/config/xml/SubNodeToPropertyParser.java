...
public class SubNodeToPropertyParser extends ConfigurableDispatchableXmlParser {
	...
	public void setElementType(String elementType) throws ClassNotFoundException, LinkageError {
	...
	protected Object doParse(Node node, ParseContext context) throws Exception {
		...
		return (list != null && !list.isEmpty()) ? new SetElementOperation(property, list.get(0)) : null;
	}
...
class SetElementOperation implements DefinitionInitOperation {
	...
	private static Set<String> NATIVE_DEFINITION_PROPERTIES = new HashSet<>();

	static {
		for (PropertyDescriptor propertyDescriptor : PropertyUtils.getPropertyDescriptors(Definition.class)) {
			NATIVE_DEFINITION_PROPERTIES.add(propertyDescriptor.getName());
		}

		for (PropertyDescriptor propertyDescriptor : PropertyUtils.getPropertyDescriptors(ObjectDefinition.class)) {
			NATIVE_DEFINITION_PROPERTIES.add(propertyDescriptor.getName());
		}
	}
