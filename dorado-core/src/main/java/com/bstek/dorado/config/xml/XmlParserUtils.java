...
public abstract class XmlParserUtils {
	...
	public static String getSimpleValue(Node node) {
		if (node instanceof Attr) {
			...
			if (len == 0) {
				return element.getAttribute(XmlConstants.VALUE);
			} else if (len == 1) {
	 ...
	 * @return 子节点(Element)或属性(Attribute)。
	 */
	public static Node getPropertyNode(Element element, String propertyName, String childElementName) {
		...
		for (Element child : children) {
			...
			if (!skipPropertyElement && XmlConstants.PROPERTY.equals(nodeName)) {
				if (propertyName.equals(child.getAttribute(XmlConstants.ATTRIBUTE_NAME))) {
					propertyNode = child;
