...
public class CompositePropertyParser extends ObjectParser {
	...
	protected Map<String, Object> parseSubProperties(List<Element> childElements, ParseContext context)
			throws Exception {
	...
	protected void initDefinition(ObjectDefinition definition, Map<String, Object> properties)
	...
	protected Object doParse(Node node, ParseContext context) throws Exception {
		...
		if (node instanceof Element) {
			...
			if (element.getChildNodes().getLength() == 0) {
				...
				List<Element> childElements = DomUtils.getChildrenByTagName(element, XmlConstants.PROPERTY);
		...
		if (textParser != null && StringUtils.isNotEmpty(text)) {
			...
			if (StringUtils.isNotEmpty(text)) {
				...
				properties = (Map<String, Object>) textParser.parse(text.toCharArray(), textParseContext);
			}
		}

		if (properties != null) {
			ObjectDefinition definition = getDefinitionType().getDeclaredConstructor().newInstance();
