...
public abstract class DispatchableTextParser implements TextParser {
	...
	private Map<String, TextParser> attributeParsers = new HashMap<>();

	private Map<String, TextParser> subParsers = new HashMap<>();
	...
	/**
	 * 向当前解析器中注册一个属性解析器。
	 * @param constraint 要注册的属性解析器的约束条件。如果此参数的值为null，那么系统将使用{@link #WILDCARD} 作为内部使用的约束条件。
	 * @param parser 属性解析器
	 */
	public void registerAttributeParser(String constraint, TextParser parser) {
		Assert.notNull(parser, "[parser] is required");
		constraint = StringUtils.defaultIfEmpty(constraint, WILDCARD);
	...
	/**
	 * 根据约束条件返回一个匹配的属性解析器。
	 * @param constraint 约束条件。 如果无法直接通过约束条件找到一个匹配的子解析器，那么此方法将尝试返回WILDCARD相关的子解析器。
	 */
	...
	protected String parseHeader(char[] charArray, TextParseContext context) throws TextParseException {
		...
		for (int currentIndex = startIndex; currentIndex < charArray.length && status != AFTER_HEADER; currentIndex++) {
			char c = charArray[currentIndex];
			context.setCurrentIndex(currentIndex);
	 ...
	 * @throws Exception
	 */
	protected Object dispatchAttribute(String constraint, char[] charArray, TextParseContext context) throws Exception {
