...
public class Logger {
	...
	static {
		String level = Configure.getString("dorado.console.logLevel"), runMode = Configure.getString("core.runMode");
		if (StringUtils.isNotEmpty(level)) {
			level = level.toUpperCase().trim();
		}
		else {
			level = StringUtils.isNotEmpty(runMode) && !"production".equalsIgnoreCase(runMode) ? "DEBUG" : "OFF";
		}
	...
	/**
	 * Convenience method to return a named logger, without the application having to care
	 * about factories.
	 * @param clazz Class from which a log name will be derived
	 * @exception LogConfigurationException if a suitable <code>Log</code> instance cannot
	 * be returned
	 */
	 ...
	 * Convenience method to return a named logger, without the application having to care
	 * about factories.
	 * @param name Logical name of the <code>Log</code> instance to be returned (the
	 * meaning of this name is only known to the underlying logging implementation that is
	 * being wrapped)
	 * @exception LogConfigurationException if a suitable <code>Log</code> instance cannot
	 * be returned
	 */
