...
public class DefaultAuthenticationManager implements AuthenticationManager, InitializingBean {
	...
	public boolean authenticate(String name, String password) {
		...
		boolean v = false;
		for (User user : users) {
			v = user.getName().equals(name) && user.getPassword().equals(password);
			if (v) {
				break;
			}
		}
		DoradoContext ctx = DoradoContext.getCurrent();
		ctx.setAttribute(DoradoContext.SESSION, Constants.S_DORADO_CONSOLE_LOGIN_STATUS, Boolean.valueOf(v));
	...
	public boolean isAuthenticated(HttpServletRequest request) {
		...
		try {
			loginStatus = Boolean
				.valueOf((Boolean) request.getSession().getAttribute(Constants.S_DORADO_CONSOLE_LOGIN_STATUS));
		}
		catch (Exception e) {
	...
	private void initProperties() {
		...
		try {
			...
			int length = Long.valueOf(Configure.getLong("dorado.console.password.length", DEFAULT_PASSWORD_LENGTH))
				.intValue();
			if (length < 1) {
				ConsoleUtils.outputLoadingInfo("Invalid Password Length, Generating a default length password...");
		...
		consoleStartedMessagesOutputter.addMessageOutputter(new ConsoleStartedMessageOutputter() {
			...
			public void output(Writer writer) throws Exception {
				for (User user : users) {
					writer.append(String.format("Console administrator [ Account: %s, Password: %s ]", user.getName(),
							user.getPassword()));
				}
			}
		});
