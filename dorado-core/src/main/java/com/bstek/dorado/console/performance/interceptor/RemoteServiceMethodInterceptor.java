...
public class RemoteServiceMethodInterceptor extends PatternMethodInterceptor {
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
			...
			if (arg instanceof ObjectNode) {
				...
				try {
					...
					if (StringUtils.isNotEmpty(serviceName) && logLevel < 4) {
						ExposedServiceManager exposedServiceManager = (ExposedServiceManager) DoradoContext.getCurrent()
						...
						JsonNode paramJson = objectNode.get("parameter"), metaJson = objectNode.get("sysParameter");
						parameter = paramJson == null ? "" : paramJson.toString();
						metaData = metaJson == null ? "" : metaJson.toString();
		...
		if (StringUtils.isNotEmpty(serviceName) && !PathUtils.match(namePattern, serviceName)) {

			if (logLevel < 4) {
				executeLogOutputter.outStartLog(TYPE, serviceName,
						String.format("parameter=%s,metaData=%s", parameter, metaData));
			}
			...
			PerformanceMonitor.getInstance().monitoredProcess(serviceName, startTime, endTime, "RemoteService");
