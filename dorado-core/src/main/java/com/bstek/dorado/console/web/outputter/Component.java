...
public class Component extends Category {
	...
	public List<Node> initNodes() {
		ComponentTypeRegistry registry = (ComponentTypeRegistry) DoradoContext.getAttachedWebApplicationContext()
			.getBean("dorado.componentTypeRegistry");
		Collection<ComponentTypeRegisterInfo> registerInfos = registry.getRegisterInfos();
		List<Node> nodes = new ArrayList<>();
		Node node;
		for (ComponentTypeRegisterInfo registerInfo : registerInfos) {
			node = new Node();
