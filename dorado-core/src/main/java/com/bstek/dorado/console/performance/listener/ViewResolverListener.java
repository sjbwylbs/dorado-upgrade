 ...
 *
 */
public class ViewResolverListener implements com.bstek.dorado.view.resolver.ViewResolverListener {
	...
	public void beforeResolveView(String viewName) throws Exception {
		...
		if (!PathUtils.match(viewNamePattern, viewName.replace('/', '.'))) {
			DoradoContext.getAttachedRequest().setAttribute(Constants.R_DORADO_CONSOLE_REQUEST_STARTTIME, startTime);
			if (Logger.getLogLevel() < 4) {
				executeLogOutputter.outStartLog(TYPE, viewName, DoradoContext.getAttachedRequest().getRequestURI());
			}
		}
	}
	...
	public void afterResolveView(String viewName, View view) throws Exception {
		if (!PathUtils.match(viewNamePattern, viewName.replace('/', '.'))) {
			...
			PerformanceMonitor.getInstance().monitoredProcess(viewName, startTime, endTime, "CreateDoradoView");
			if (Logger.getLogLevel() < 4) {
