...
public class OutputterController {
	...
	private Comparator<PropertyConfig> propertyConfigComparator = new Comparator<>() {
		@Override
	...
	@DataProvider
	public List<PropertyConfig> getPropertyConfigs(String beanName) throws ClassNotFoundException, Exception {
		List<PropertyConfig> propertyConfigs = null;
		if (beanName != null) {
			propertyConfigs = new ArrayList<>();
			ObjectOutputter outputter = (ObjectOutputter) clientOutputHelper.getOutputter(Class.forName(beanName));

			Map<String, com.bstek.dorado.view.output.PropertyConfig> configs = outputter.getPropertieConfigs();
			for (Entry<String, com.bstek.dorado.view.output.PropertyConfig> entry : configs.entrySet()) {
				Object propertyOutputter = entry.getValue().getOutputter();
				String property = entry.getKey();
				PropertyConfig config = new PropertyConfig();
				config.setName(property);
				if (propertyOutputter != null) {
					config.setOutputterName(propertyOutputter.getClass().getName());
				}
