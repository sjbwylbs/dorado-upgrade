...
public class PerformanceController implements InitializingBean {
	...
	private static final Logger logger = Logger.getLog(PerformanceController.class);
	...
	public void onReady(View view) {
		...
		DoradoContext.getCurrent().setAttribute(DoradoContext.VIEW, V_PERFORMANCEDATATYPE, type);
	}
	...
	public List<ProcessVO> getLastProcessList() {
		String type = (String) DoradoContext.getCurrent().getAttribute(DoradoContext.VIEW, V_PERFORMANCEDATATYPE);
	...
	public List<MonitoredTargetVO> getTargetList() throws Exception {
		String type = (String) DoradoContext.getCurrent().getAttribute(DoradoContext.VIEW, V_PERFORMANCEDATATYPE);
		List<MonitoredTargetVO> monitoredTargets = new ArrayList<>();
		Collection<MonitoredTarget> targets = monitor.getMonitoredTargets().values();
		MonitoredTargetVO vo;
		MonitoredTarget target;
		Category category;
		String name;
		Map<String, Category> map = monitor.getCategoryMap();
		for (Iterator<MonitoredTarget> iterator = targets.iterator(); iterator.hasNext();) {
			...
			if (category.getCount() > 0) {
				long timeLength = lastProcess.getTime() - category.getFirstProcess().getTime();
