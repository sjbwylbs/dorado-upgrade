 ...
 *
 */
public class DataResolveMethodInterceptor extends AbstractDataResolverMethodInterceptor {
	...
	protected Object invokeResolve(MethodInvocation methodInvocation, DataResolver dataResolver, DataItems dataItems,
		...
		if (logLevel < 4) {
			...
			executeLogOutputter.outStartLog(TYPE, resolverName,
					String.format("dataItems=[ %s ], parameter={ %s }", buffer, parameter));
		}
		...
		PerformanceMonitor.getInstance().monitoredProcess(dataResolver.getName(), startTime, endTime, "DataResolve");
		return object;
