...
public class ViewServiceSecurityInterceptor extends PatternMethodInterceptor {
		...
		for (Object arg : invocation.getArguments()) {
			if (arg instanceof ObjectNode) {
				...
				if (serviceName != null && match(serviceNamePattern, serviceName)) {
					Boolean loginStatus = Setting.getAuthenticationManager()
						.isAuthenticated(DoradoContext.getAttachedRequest());
					if (!loginStatus) {
						throw new AccessDeniedException(AccessDeniedException.LOGIN_TIME_OUT_MESSAGE);
					}
	...
	public static boolean match(String pattern, String text) {
		return (StringUtils.isNotEmpty(text)) ? matcher.match(pattern, text) : false;
	}
