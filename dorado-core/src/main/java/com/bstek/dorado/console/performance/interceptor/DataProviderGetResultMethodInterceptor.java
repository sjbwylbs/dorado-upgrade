 ...
 *
 */
public class DataProviderGetResultMethodInterceptor extends AbstractDataProviderGetResultMethodInterceptor {
	...
	/**
	 * @param executeLogOutputter the executeLogOutputter to set
	 */
	...
	protected Object invokeGetResult(MethodInvocation methodinvocation, DataProvider dataProvider, Object parameter,
		...
		if (logLevel < 4) {
			executeLogOutputter.outStartLog(TYPE, providerName, String.format("parameter={ %s }", parameter));
		...
		if (logLevel < 4) {
			...
			if (result instanceof Expression) {
				...
				result = String.format("{%s , size = %s}", Collection.class.getName(), ((Collection) result).size());
			}
			executeLogOutputter.outEndLog(TYPE, providerName, String.format("result= %s ", result));
		}

		long endTime = System.currentTimeMillis();
		PerformanceMonitor.getInstance().monitoredProcess(providerName, startTime, endTime, "DataProviderGetResult");
		return object;
	}
	...
	protected Object invokeGetPagingResult(MethodInvocation methodinvocation, DataProvider dataProvider,
			Object parameter, Page page, DataType datatype) throws Throwable {
		String providerName = dataProvider.getName();
		int logLevel = Logger.getLogLevel();
		if (PathUtils.match(namePattern, providerName)) {
			return methodinvocation.proceed();
		...
		String message;
		if (logLevel < 4) {
			message = String.format("parameter= %s ,page={ pageNo = %s , pageSize = %s}", parameter, page.getPageNo(),
					page.getPageSize());
			executeLogOutputter.outStartLog("DataProvider GetPagingResult", providerName, message);
		}
		Object object = methodinvocation.proceed();
		if (logLevel < 4) {
			message = String.format("{Entities Size= %s ,pageCount= %s}", page.getEntityCount(), page.getPageCount());
			executeLogOutputter.outEndLog("DataProvider GetPagingResult", providerName, message);
		}
