...
public abstract class ProxyBeanUtils {
	 ...
	 * @return 新的方法拦截器数组
	 */
	public static MethodInterceptor[] appendMethodInterceptor(MethodInterceptor[] originMethodInterceptors,
		...
		if (originMethodInterceptors == null) {
			...
			System.arraycopy(originMethodInterceptors, 0, newMethodInterceptors, 0, originMethodInterceptors.length);
	 ...
	 * @return 新的方法拦截器数组
	 */
	public static MethodInterceptor[] appendMethodInterceptors(MethodInterceptor[] originMethodInterceptors,
		...
		if (originMethodInterceptors == null) {
			...
			newMethodInterceptors = new MethodInterceptor[originMethodInterceptors.length + methodInterceptors.length];
			System.arraycopy(originMethodInterceptors, 0, newMethodInterceptors, 0, originMethodInterceptors.length);
			System.arraycopy(methodInterceptors, 0, newMethodInterceptors, originMethodInterceptors.length,
	 ...
	 * @throws Exception
	 */
	public static Object createBean(Class<?> cl, MethodInterceptor methodInterceptor) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static Object createBean(Class<?> cl, MethodInterceptor[] methodInterceptors) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static Object createBean(Class<?> cl, MethodInterceptor[] methodInterceptors, Class<?>[] argTypes,
			Object[] args) throws Exception {
		Object proxyBean;
		if (methodInterceptors != null && methodInterceptors.length > 0) {
			BaseMethodInterceptorDispatcher mipd = new BaseMethodInterceptorDispatcher(methodInterceptors);
			if (defaultByteCodeProvider == JAVASSIST) {
				...
				if (argTypes == null || argTypes.length == 0) {
					proxyBean = proxyCl.getDeclaredConstructor().newInstance();
				}
				else {
					Constructor<?> constr = proxyCl.getConstructor(argTypes);
					if (constr == null) {
						throw new NoSuchMethodException("Can not found a suitable constructor for [" + proxyCl + "].");
					}
			...
			if (argTypes == null || argTypes.length == 0) {
				proxyBean = cl.getDeclaredConstructor().newInstance();
			}
			else {
				Constructor<?> constr = cl.getConstructor(argTypes);
				if (constr == null) {
					throw new NoSuchMethodException("Can not found a suitable constructor for [" + cl + "].");
				}
				proxyBean = constr.newInstance(args);
			}
		}
	...
	public static boolean isProxy(Class<?> cl) {
		boolean b = net.sf.cglib.proxy.Enhancer.isEnhanced(cl) || ProxyFactory.isProxyClass(cl);
		if (!b) {
	...
	public static Object getProxyTarget(Object bean) throws Exception {
		Object target = bean;
		MethodInterceptorDispatcher methodInterceptorDispatcher = ProxyBeanUtils.getMethodInterceptorDispatcher(bean);
		if (methodInterceptorDispatcher != null
				&& methodInterceptorDispatcher instanceof MethodInterceptorProxyDispatcher) {
			target = ((MethodInterceptorProxyDispatcher) methodInterceptorDispatcher).getTarget();
		}
		return target;
	}
	 ...
	 * @throws Exception
	 */
	public static Object proxyBean(Object target, MethodInterceptor methodInterceptor) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static Object proxyBean(Object target, MethodInterceptor[] methodInterceptors) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static Object proxyBean(Object target, Class<?> cl, MethodInterceptor[] methodInterceptors,
			Class<?>[] argTypes, Object[] args) throws Exception {
		MethodInterceptorProxyDispatcher mipd = new MethodInterceptorProxyDispatcher(target, methodInterceptors);
	...
	private static Object proxyBeanWithJavassist(Object target, Class<?> cl, Class<?>[] argTypes, Object[] args,
	...
	private static Object proxyBeanWithCglib(Object target, Class<?> cl, Class<?>[] argTypes, Object[] args,
			MethodInterceptorProxyDispatcher mipd) {
	 ...
	 * @throws Exception
	 */
	public static MethodInterceptorDispatcher getMethodInterceptorDispatcher(Object object) {
		...
		if (isProxy(object)) {
			if (object instanceof ProxyObject) {
				...
				net.sf.cglib.proxy.Callback[] callbacks = ((net.sf.cglib.proxy.Factory) object).getCallbacks();
