 ...
 */
public class DeepCloneableLinkedHashMap<K, V> extends LinkedHashMap<K, V> implements Cloneable {
	...
	public Object clone() {
		...
		for (Map.Entry<K, V> entry : map.entrySet()) {
			...
			if (value != null && value instanceof Cloneable) {
				try {
					Method method = value.getClass().getMethod(CLONE_METHOD, CLONE_PARAM_CLASSES);
					if (method != null) {
						entry.setValue((V) method.invoke(value, CLONE_PARAM_OBJECTS));
					}
				}
				catch (Exception ex) {
					// do nothing
