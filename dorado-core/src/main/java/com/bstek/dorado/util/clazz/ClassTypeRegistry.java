...
public class ClassTypeRegistry<T> {
	...
	private Map<Class<?>, T> typeMap = new HashMap<>();

	private Map<Class<?>, Class<?>> typeMatchingCache = new HashMap<>();

	private Map<Class<?>, T> valueMatchingCache = new HashMap<>();
	...
	public Class<?> getMatchingType(Class<?> type) {
		...
		if (matchingType == null) {
			...
			typeMatchingCache.put(type, (matchingType == null) ? NULL_CLASS : matchingType);
		}
	...
	public T getMatchingValue(Class<?> type) {
		...
		if (matchingValue == null) {
			...
			valueMatchingCache.put(type, (matchingValue == null) ? (T) NULL_OBJECT : matchingValue);
		}
