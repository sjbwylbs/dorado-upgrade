...
public class MethodInterceptorChain {
	...
	public MethodInterceptorChain(MethodInterceptor[] subMethodInterceptors, MethodInterceptor finalMethodInterceptor,
	...
	public MethodInterceptor next(MethodInvocation methodInvocation) {
		...
		while (index < subMethodInterceptors.length) {
			...
			if (filter != null && !filter.filter(methodInterceptor, methodInvocation)) {
