 ...
 * 使用此分发器即可以保证应用中定义的org.aopalliance.intercept.MethodInterceptor方法拦截器 可以同时应用于Cglib和Spring
 * AOP，又可以实现对方法的迭代拦截（即同时启用多个拦截器拦截被代理对象的同一个方法）。
 * </p>
 *
 */
public abstract class MethodInterceptorDispatcher
	...
	/**
	 * 创建一个用于org.aopalliance.intercept. MethodInterceptor中invoke方法的MethodInvocation参数。
	 * @param object 被代理对象
	 * @param method 被代理方法
	 * @param args 被代理方法的参数
	 * @param methodInterceptorChain 用于迭代所有子方法拦截器的迭代器（包含
	...
	public Object intercept(Object object, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
		...
		if (subMethodInterceptors != null && filterMethod(method)) {
			MethodInterceptorChain methodInterceptorChain = new MethodInterceptorChain(subMethodInterceptors,
					createCglibFinalMethodInterceptor(methodProxy), getMethodInterceptorFilter(object, method, args));
	...
	protected Object getObjectForSerialization(Object object) throws Exception {
		...
		if (cl != object.getClass()) {
			Object beanForSerialization = cl.getDeclaredConstructor().newInstance();
