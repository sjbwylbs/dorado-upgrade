...
public final class TempFileUtils {
	...
	public static File getTempDir() throws IOException {
		...
		if (tempDir == null) {
			...
			if (!tempDir.mkdirs()) {
				throw new IOException("Make directory \"" + tempDir.getAbsolutePath() + "\" failed.");
			}
			else {
				...
				try {
					...
					if (lock == null) {
						throw new IllegalStateException("Lock file \"" + tempDir + "\" failed.");
					}
				}
				catch (OverlappingFileLockException e) {
					throw new IllegalStateException("\"" + tempDir + "\" is already locked.", e);
				}
				finally {
					raf.close();
				}
	...
	public static File createTempFile(String fileNamePrefix, String fileNamesuffix) throws IOException {
		if (!supportsTempFile) {
			throw new IOException("Temp file is forbidden.");
		}

		File file = File.createTempFile(fileNamePrefix, fileNamesuffix, getTempDir());
		file.deleteOnExit();
		return file;
	}
