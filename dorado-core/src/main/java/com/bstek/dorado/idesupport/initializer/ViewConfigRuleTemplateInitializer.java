...
public class ViewConfigRuleTemplateInitializer implements RuleTemplateInitializer, RuleTemplateBuilderAware {
	...
	public void initRuleTemplate(RuleTemplate ruleTemplate, InitializerContext initializerContext) throws Exception {
		...
		sortFactor = 2000;
		List<RuleTemplate> componentRuleTemplates = new ArrayList<>();
		for (ComponentTypeRegisterInfo registerInfo : componentTypeRegistry.getRegisterInfos()) {
			...
			if (componentRuleTemplate == null) {
				componentRuleTemplate = new AutoRuleTemplate(name, (isAssembledComponent) ? null : classType.getName());
