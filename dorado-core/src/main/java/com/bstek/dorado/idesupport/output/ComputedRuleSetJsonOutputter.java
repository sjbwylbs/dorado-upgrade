...
public class ComputedRuleSetJsonOutputter {
	...
	protected void outputPackageInfos(JsonBuilder jsonBuilder, RuleSet ruleSet, OutputContext context)
			throws Exception {
		Map<String, PackageInfo> finalPackageInfos = new LinkedHashMap<>(
	...
	protected void outputRule(JsonBuilder jsonBuilder, Rule rule, OutputContext context) throws Exception {
		jsonBuilder.object();

		outputKeys(jsonBuilder, rule,
		...
		if (StringUtils.isNotEmpty(className)) {
			...
			if (Component.class.isAssignableFrom(type)) {
				...
				ComponentTypeRegisterInfo registerInfo = componentTypeRegistry.getRegisterInfo(type);
				if (registerInfo != null && StringUtils.isNotEmpty(registerInfo.getDependsPackage())) {
					jsonBuilder.key("dependsPackage").value(registerInfo.getDependsPackage());
				}
			}
			...
			if (outputter instanceof ClientObjectOutputter) {
				...
				if (StringUtils.isNotEmpty(coo.getShortTypeName())) {
					jsonBuilder.key("jsShortType").value(coo.getShortTypeName());
				}
			}
		}

		Collection<Property> primitiveProperties = rule.getPrimitiveProperties().values();
	...
	private Set<Rule> findAllSubRules(Rule rule) {
		Set<Rule> subRules = new TreeSet<>(new Comparator<Rule>() {
			@Override
	...
	protected void outputProperty(JsonBuilder jsonBuilder, Property property, OutputContext context) throws Exception {
		jsonBuilder.object();
		outputKeys(jsonBuilder, property,
	...
	protected void outputClientEvent(JsonBuilder jsonBuilder, ClientEvent clientEvent, OutputContext context)
			throws Exception {
		jsonBuilder.object();
		outputKeys(jsonBuilder, clientEvent, "name,clientTypes,deprecated,reserve");
	...
	protected void outputChild(JsonBuilder jsonBuilder, Child child, OutputContext context) throws Exception {
		for (Rule subRule : child.getConcreteRules()) {
			...
			outputKeys(jsonBuilder, child, "property,fixed,clientTypes,deprecated,reserve");
	...
	private void outputKeys(JsonBuilder jsonBuilder, Object object, String propertyNames) throws Exception {
		for (String propertyName : StringUtils.split(propertyNames, ',')) {
			outputKey(jsonBuilder, object, propertyName);
		}
	}

	private void outputKey(JsonBuilder jsonBuilder, Object object, String propertyName) throws Exception {
