...
public class ModelRuleTemplateInitializer implements RuleTemplateInitializer {
	...
	public void setValidatorTypeRegistry(ValidatorTypeRegistry validatorTypeRegistry) {
	...
	public void setDataResolverTypeRegistry(DataResolverTypeRegistry dataResolverTypeRegistry) {
	...
	public void initRuleTemplate(RuleTemplate ruleTemplate, InitializerContext initializerContext) throws Exception {
		...
		for (DataTypeTypeRegisterInfo dataTypeTypeInfo : dataTypeTypeRegistry.getTypes()) {
			String dataTypeName = dataTypeTypeInfo.getClassType().getSimpleName();
			...
			boolean isNew = false;
			RuleTemplate newRuleTemplate = ruleTemplateManager.getRuleTemplate(ruleName);
		...
		for (DataProviderTypeRegisterInfo dataProviderTypeInfo : dataProviderTypeRegistry.getTypes()) {
			String providerName = dataProviderTypeInfo.getClassType().getSimpleName();

			boolean isNew = false;
			RuleTemplate newRuleTemplate = ruleTemplateManager.getRuleTemplate(providerName);
			if (newRuleTemplate == null) {
		...
		for (DataResolverTypeRegisterInfo dataResolverTypeInfo : dataResolverTypeRegistry.getTypes()) {
			String resolverName = dataResolverTypeInfo.getClassType().getSimpleName();

			boolean isNew = false;
			RuleTemplate newRuleTemplate = ruleTemplateManager.getRuleTemplate(resolverName);
			if (newRuleTemplate == null) {
			...
			newRuleTemplate.setSortFactor(++sortFactor);
			newRuleTemplate.setScope("public");
			newRuleTemplate.setType(dataResolverTypeInfo.getClassType().getName());
		...
		RuleTemplate validatorRuleTemplate = ruleTemplateManager.getRuleTemplate("Validator");
		if (validatorRuleTemplate == null) {
			validatorRuleTemplate = new AutoRuleTemplate("Validator", Validator.class.getName());
			validatorRuleTemplate.setGlobal(true);
			ruleTemplateManager.addRuleTemplate(validatorRuleTemplate);
		}

		RuleTemplate abstractValidatorRuleTemplate = ruleTemplateManager.getRuleTemplate("AbstractValidator");
		if (abstractValidatorRuleTemplate == null) {
			abstractValidatorRuleTemplate = new AutoRuleTemplate("AbstractValidator",
					AbstractValidator.class.getName());
			ruleTemplateManager.addRuleTemplate(abstractValidatorRuleTemplate);
		}
		abstractValidatorRuleTemplate.setParents(new RuleTemplate[] { validatorRuleTemplate });

		for (ValidatorTypeRegisterInfo validatorTypeRegisterInfo : validatorTypeRegistry.getTypes()) {
			String validatorName = validatorTypeRegisterInfo.getClassType().getSimpleName();
			RuleTemplate newRuleTemplate = ruleTemplateManager.getRuleTemplate(validatorName);
			if (newRuleTemplate == null) {
