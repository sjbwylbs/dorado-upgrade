...
public class DefaultDataTypeManager extends DataTypeManagerSupport {
	...
	private static final Log logger = LogFactory.getLog(DefaultDataTypeManager.class);
	...
	private class DataTypeWrapper {
		...
		public boolean equals(Object obj) {
			if (obj instanceof DataTypeWrapper) {
				...
				return (type.equals(dtw.getType()) && dataType.equals(dtw.getDataType()));
			}
			else {
				return false;
			}
	 ...
	 */
	@Override
	protected DataTypeDefinition getDataTypeDefinition(Type type) throws Exception {
		DataTypeDefinition dataType = null;
		if (type instanceof Class<?> && ((Class<?>) type).isArray()) {
			Class<?> cl = (Class<?>) type;
			DataTypeDefinition componentDataType = getDataTypeDefinitionByType(cl.getComponentType());
	...
	protected DataTypeDefinition getDataTypeDefinitionByType(Class<?> type) {
		...
		if (size == 1) {
			...
			logger.warn("More than one matching DataTypes found for [" + type.getName() + "]!");
		}
		return dataType;
	}
	...
	private Set<DataTypeWrapper> findMatchingDataTypeForInterface(Class<?> type) {
		Set<DataTypeWrapper> dtws = new HashSet<>();
		...
		if (parents != null) {
			for (Definition parent : parents) {
				if (parent == parentDataType) {
					...
					boolean isChild = isChildTypeOf((DataTypeDefinition) parent, parentDataType);
					if (isChild) {
						return true;
					}
				}
	...
	private void filterMatchingDataTypes(Set<DataTypeWrapper> dtws) {
		...
		for (DataTypeWrapper dtw1 : dtwArray) {
			for (DataTypeWrapper dtw2 : dtws) {
				if (dtw1 != dtw2) {
					if (dtw1.getType().isAssignableFrom(dtw2.getType()) || isChildTypeOf(dtw2.getDataType(), dtw1.getDataType())) {
						dtws.remove(dtw1);
						break;
					}
				}
