...
public class DataParseContext extends ParseContext {
	...
	private Map<String, NodeWrapper> configuredDataTypes = new HashMap<>();

	private Map<String, DataTypeDefinition> parsedDataTypes = new HashMap<>();

	private Map<String, NodeWrapper> configuredDataProviders = new HashMap<>();

	private Map<String, DataProviderDefinition> parsedDataProviders = new HashMap<>();

	private Map<String, NodeWrapper> configuredDataResolvers = new HashMap<>();

	private Map<String, DataResolverDefinition> parsedDataResolvers = new HashMap<>();

	private Set<Node> parsingNodes = new LinkedHashSet<>();
	...
	private Stack<String> privateObjectNameStack = new Stack<>();

	private Stack<DefinitionReference<DataTypeDefinition>> currentDataTypeStack = new Stack<>();
	 ...
	 * @param name DataProvider的名称
	 * @return 配置声明对象的引用
	 */
	public DefinitionReference<DataProviderDefinition> getDataProviderReference(String name, DataParseContext context) {
		name = getFinalDataObjectName(name, context);
		name = name.trim();
		return new DefaultDefinitionReference<>(dataProviderDefinitionManager, name);
	}

	/**
	 * 根据DataResolver的名称生成一个指向某DataResolver配置声明对象的引用。
	 * @param name DataResolver的名称
	 * @return 配置声明对象的引用
	 */
	public DefinitionReference<DataResolverDefinition> getDataResolverReference(String name, DataParseContext context) {
		name = getFinalDataObjectName(name, context);
		name = name.trim();
		return new DefaultDefinitionReference<>(dataResolverDefinitionManager, name);
	}
