...
public class DirectDataProvider extends AbstractDataProvider {
	...
	@Override
	protected Object internalGetResult(Object parameter, DataType resultDataType) throws Exception {
	...
	protected void internalGetPagingResult(Object parameter, Page page, DataType resultDataType) throws Exception {
		// 此处未处理resultDataType，因为运行时的动态代理会去处理
		if (result == null) {
		...
		if (result instanceof Collection) {
			if (result instanceof List) {
				...
				page.setEntities(list.subList(page.getFirstEntityIndex(), page.getLastEntityIndex()));
				page.setEntityCount(list.size());
			}
			else {
				List<Object> list = new ArrayList<>();
				int index = 0, firstIndex = page.getFirstEntityIndex(), lastIndex = page.getLastEntityIndex();
