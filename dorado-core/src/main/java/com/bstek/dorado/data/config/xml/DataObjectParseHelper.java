...
public class DataObjectParseHelper {
	 ...
	 * @throws Exception
	 */
	public DefinitionReference<DataTypeDefinition> getDataTypeByName(String name, DataParseContext context,
			boolean subDataTypeAllowed) throws Exception {
		DefinitionReference<DataTypeDefinition> definitionReference = null;
		DataTypeDefinitionManager dataTypeDefinitionManager = context.getDataTypeDefinitionManager();
		DataTypeDefinition dataType = dataTypeDefinitionManager.getDefinition(name);
		if (dataType == null) {
		...
		if (definitionReference == null && dataType == null) {
			Map<String, NodeWrapper> dataTypeMap = context.getConfiguredDataTypes();
			...
			if (nodeWrapper != null) {
				...
				if (context.getParsingNodes().contains(refDataTypeElement)) {
					...
					dataType = (DataTypeDefinition) dataTypeParser.parse(refDataTypeElement, context);
		...
		if (definitionReference == null) {
			if (dataType == null) {
				throw new XmlParseException("Invalid DataType reference \"" + name + "\".", context);
			}
			definitionReference = context.getDataTypeReference(name);
		}
	 ...
	 * @param propertyName 可能定义DataType引用信息的属性名。
	 * @param childTagName 可能定义DataType引用信息的子节点名。可为null，表忽略。
	 * 注意，如果propertyName和childTagName参数所代表的定义方式中都能够找到关于DataType的描述 ，将以childTagName中的定义为准。
	 * @param element XML节点。
	 * @param context 解析上下文。
	 * @return 指向DataType配置声明的引用。
	 * @throws Exception
	 */
	public DefinitionReference<DataTypeDefinition> getReferencedDataType(String propertyName, Element element,
	 ...
	 * @param propertyName 可能定义DataProvider引用信息的属性名。
	 * @param childTagName 可能定义DataProvider引用信息的子节点名。可为null，表忽略。
	 ...
	 * @throws Exception
	 */
	public DefinitionReference<DataProviderDefinition> getReferencedDataProvider(String propertyName, Element element,
			DataParseContext context) throws Exception {
	 ...
	 * @param propertyName 可能定义DataResolver引用信息的属性名。
	 * @param childTagName 可能定义DataResolver引用信息的子节点名。可为null，表忽略。
	 ...
	 * @throws Exception
	 */
	public DefinitionReference<DataResolverDefinition> getReferencedDataResolver(String propertyName, Element element,
			DataParseContext context) throws Exception {
