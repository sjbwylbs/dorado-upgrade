...
public class DataResolverDefinitionReference implements DefinitionReference<DataResolverDefinition> {

	private static Log logger = LogFactory.getLog(DataResolverDefinitionReference.class);
	...
	public DataResolverDefinition getDefinition() {
		Context current = Context.getCurrent();
		DataResolverDefinitionManager dtfm = (DataResolverDefinitionManager) current.getAttribute(Context.THREAD,
		...
		if (definition == null) {
			throw new IllegalArgumentException("Unrecognized DataResolver \"" + name + "\".");
		}
		return definition;
	}
