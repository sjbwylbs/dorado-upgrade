...
public abstract class MethodAutoMatchingUtils {
	...
	private static Map<Object, Method[]> methodCache = new Hashtable<>();
	...
	private static Collection<ParameterFactory> getSystemOptionalParameters() throws Exception {
		if (systemOptionalParameters == null) {
			...
			if (systemOptionalParameters == null) {
				systemOptionalParameters = new ArrayList<>();
			}
		}
	 ...
	 * 注意，此方法将忽略方法参数的顺序。
	 * @param methods 方法数组。
	 * @param requiredTypes 必须提供的方法参数类型。
	 * @param exactTypes 类型必须严格匹配的方法参数类型。
	 * @param optionalTypes 可选的方法参数类型。
	 * @param returnType 返回值类型，如果为null则表示忽略对返回值类型的判断。
	 * @return 找到的方法的描述对象。
	 * @throws MethodAutoMatchingException 如果找到了一个以上的匹配方法或没有找到任何匹配的方法将抛出此异常。
	 */
				...
				if (methodDescriptor != null) {
					...
					if (matchingRate == tmpMatchingRate) {
						...
						throw new MoreThanOneMethodsMatchsException(
								resourceManager.getString("dorado.common/tooMoreMatchingMethodsErrorHeader"), detail);
					}
	 ...
	 * 注意，此方法将忽略方法参数的顺序。
	 * @param methods 方法数组。
	 * @param requiredParameterNames 必须提供的方法参数名数组。
	 * @param optionalParameterNames 可选的方法参数名数组。
	 * @return 找到的方法的描述对象。
	 * @throws MethodAutoMatchingException 如果找到了一个以上的匹配方法或没有找到任何匹配的方法将抛出此异常。
	 */
	...
	/**
	 * 根据方法描述对象调用方法。
	 * @param methodDescriptor 方法的描述对象
	 * @param object 反射的对象。
	 * @param parameters 可选的参数数组。
	 * @return 方法执行后的返回值。
	 * @throws Exception
	 */
			...
			if (arg != null) {
				...
				if (!defType.isAssignableFrom(argType)) {
					if (Number.class.isAssignableFrom(defType) && Number.class.isAssignableFrom(argType)) {
						arg = NumberUtils.convertNumberToTargetClass((Number) arg, (Class<? extends Number>) defType);
					}
	 ...
	 * @param requiredParameterTypes 必须提供的方法参数类型。
	 * @param requiredParameters 必须的方法参数。
	 * @param exactParameterTypes 类型必须严格匹配的方法参数类型。
	 * @param exactParameters 类型必须严格匹配的方法参数。
	 * @param optionalParameterTypes 可选的方法参数类型。
	 * @param optionalParameters 可选的方法参数。
	 * @param returnType 返回值类型，如果为null则表示忽略对返回值类型的判断。
	 * @return 方法执行后的返回值。
	 ...
	 * @param methods 方法数组。
	 * @param object 方法的宿主对象。
	 * @param requiredParameterNames 必选参数名的数组。
	 * @param requiredParameters 必选参数的数组。
	 * @param optionalParameterNames 可选参数名的数组。
	 * @param optionalParameters 可选参数的数组。
	 * @return 方法执行后的返回值。
	...
	private static String getExceptionMessage(String resourceKey, Method[] methods, Type[] requiredTypes,
		...
		return resourceManager.getString(resourceKey, methodInfo.getClassName(), methodInfo.getMethodName(), required,
				exact, optional, ((returnType != null) ? returnType.toString() : "*"));
	}
