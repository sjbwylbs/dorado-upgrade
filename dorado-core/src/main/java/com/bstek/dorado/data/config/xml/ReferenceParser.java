...
public class ReferenceParser extends PropertyDefParser {
	...
	protected void initDefinition(ObjectDefinition definition, Element element, ParseContext context) throws Exception {
		...
		DefinitionReference<DataProviderDefinition> dataProviderRef = dataObjectParseHelper
			.getReferencedDataProvider(DataXmlConstants.ATTRIBUTE_DATA_PROVIDER, element, (DataParseContext) context);
		if (dataProviderRef != null) {
			definition.setProperty(DataXmlConstants.ATTRIBUTE_DATA_PROVIDER, dataProviderRef);
		}
	}
