...
public class DataResolverDefinition extends InterceptableDefinition
		implements DefinitionManagerAware<DataResolverDefinitionManager> {
	...
	@Override
	public void setDefinitionManager(DataResolverDefinitionManager definitionManager) {
	...
	protected MethodInterceptor[] getMethodInterceptors(CreationInfo creationInfo, CreationContext context)
		...
		if (customMethodInterceptors != null) {
			...
			if (superInterceptors != null) {
				...
				System.arraycopy(superInterceptors, 0, interceptors, 2, superInterceptors.length);
			}
			else {
			...
			interceptors[0] = defaultResolverMethodInterceptor;
			System.arraycopy(superInterceptors, 0, interceptors, 1, superInterceptors.length);
		}
		return interceptors;
	}
...
class RootCustomDataResolverInterceptor extends BaseMethodInterceptorDispatcher {
	...
	public RootCustomDataResolverInterceptor(MethodInterceptor[] subMethodInterceptors) {
