...
public class DataTypeParser extends GenericObjectParser implements ObjectParserInitializationAware {
	...
	protected DefinitionReference<DataTypeDefinition>[] getParentDefinitionReferences(String parentNameText,
		...
		for (int i = 0; i < parentNames.length; i++) {
			...
			DefinitionReference<DataTypeDefinition> dataTypeRef = dataObjectParseHelper.getDataTypeByName(parentName,
					dataContext, true);
	...
	protected void initDefinition(ObjectDefinition definition, Element element, ParseContext context) throws Exception {
		...
		dataTypeRef = dataObjectParseHelper.getReferencedDataType(DataXmlConstants.ATTRIBUTE_ELEMENT_DATA_TYPE, element,
				dataContext);
		if (dataTypeRef != null) {
			definition.setProperty(DataXmlConstants.ATTRIBUTE_ELEMENT_DATA_TYPE, dataTypeRef);
		}
	}
	...
	protected Object internalParse(Node node, ParseContext context) throws Exception {
		...
		DataTypeDefinition dataType = (DataTypeDefinition) super.internalParse(node, context);

		Class<?> matchType = (Class<?>) dataType.removeProperty(DataXmlConstants.ATTRIBUTE_MATCH_TYPE);
		dataType.setMatchType(matchType);

		Class<?> creationType = (Class<?>) dataType.removeProperty(DataXmlConstants.ATTRIBUTE_CREATION_TYPE);
		if (creationType != null) {
			if (matchType != null && !matchType.isAssignableFrom(creationType)) {
				throw new XmlParseException("The CreationType [" + creationType
						+ "] is not a sub type of the MatchType [" + matchType + "].", element, context);
			}
			dataType.setCreationType(creationType);
		}

		final String DEFAULT_DATATYPE_PARENT = Configure.getString("data.defaultEntityDataTypeParent", "Entity");

		String name = element.getAttribute(XmlConstants.ATTRIBUTE_NAME);
		if (dataType.getParentReferences() == null && !DEFAULT_DATATYPE_PARENT.equals(name)) {
			...
			if (useDefaultParent) {
				DataParseContext dataContext = (DataParseContext) context;
				DefinitionReference<?> dataTypeRef = dataContext.getDataTypeReference(DEFAULT_DATATYPE_PARENT);
	...
	@Override
	public void postObjectParserInitialized(ObjectParser objectParser) throws Exception {
		defaultImpl = getImpl();
		setImpl(null);
	}
