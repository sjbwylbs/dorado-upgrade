...
public class DataResolverTypeRegistry {
	...
	private Map<String, DataResolverTypeRegisterInfo> typeMap = new LinkedHashMap<>();

	private String defaultType;

	/**
	 * 设置默认的DataResolver的类型名。<br>
	 * 如果用户在调用的{@link #getTypeRegistryInfo(String)}方法时， 没有指定DataResolver的类型名（传递null值），
	 * 那么管理器将使用此属性指定的DataResolver的类型名来完成内部的处理。
	 * @param defaultType 默认的DataResolver的类型名
	 */
