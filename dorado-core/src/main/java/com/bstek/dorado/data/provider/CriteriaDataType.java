...
public class CriteriaDataType extends AbstractDataType implements JsonConvertor {
	...
	private Criterion jsonToCriterion(ObjectNode criterionNode, JsonConvertContext context) throws Exception {
		...
		if (StringUtils.isEmpty(junctionValue)) {
			...
			String dataTypeName = JsonUtils.getString(criterionNode, "dataType");
			...
			if ("expression".equals(type)) {
				...
				criterion = pfc;
				pfc.setOtherProperty(JsonUtils.getString(criterionNode, "value"));
			}
			else {
				SingleValueFilterCriterion vfc = new SingleValueFilterCriterion();
				criterion = vfc;
				vfc.setDataType(dataType);
				vfc.setValue(JsonUtils.toJavaObject(criterionNode.get("value"), dataType));
			}
			criterion.setProperty(JsonUtils.getString(criterionNode, "property"));
			criterion.setFilterOperator(toFilterOperator(JsonUtils.getString(criterionNode, "operator")));
			return criterion;
		}
		else {
			Junction junction;
			...
			if (valueNode != null && !valueNode.isNull()) {
				...
				for (JsonNode jsonNode : criterionsNode) {
					Criterion criterion = jsonToCriterion((ObjectNode) jsonNode, context);
	...
	private Order jsonToOrder(ObjectNode orderNode, JsonConvertContext context) throws Exception {
	...
	@Override
	public Object fromJSON(JsonNode jsonNode, JsonConvertContext context) throws Exception {
		...
		if (valueNode != null && !valueNode.isNull()) {
			ArrayNode criterionsNode = (ArrayNode) valueNode;
			for (JsonNode node : criterionsNode) {
				ObjectNode criterionNode = (ObjectNode) node;
		...
		if (valueNode != null && !valueNode.isNull()) {
			...
			for (JsonNode node : ordersNode) {
				ObjectNode orderNode = (ObjectNode) node;
