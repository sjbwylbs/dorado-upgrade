...
public class PropertyDefParser extends GenericObjectParser {
	...
	private static class AddPropertyDefOperation implements DefinitionInitOperation {
	...
	protected void initDefinition(ObjectDefinition definition, Element element, ParseContext context) throws Exception {
		...
		DefinitionReference<DataTypeDefinition> dataTypeRef = dataObjectParseHelper
			.getReferencedDataType(DataXmlConstants.ATTRIBUTE_DATA_TYPE, element, dataContext);
		if (dataTypeRef != null) {
			definition.setProperty(DataXmlConstants.ATTRIBUTE_DATA_TYPE, dataTypeRef);
		}
	...
	protected Object doParse(Node node, ParseContext context) throws Exception {
		PropertyDefDefinition definition = (PropertyDefDefinition) super.doParse(node, context);
