...
public abstract class EntityEnhancer {
	 ...
	 * @param create 如果原始值集合上不存在，则创建一个全新的原始值集合。
	 */
	...
	protected void markPropertyHasRead(String property) {
		if (propertiesHasRead == null) {
			propertiesHasRead = new ConcurrentHashMap<>();
	...
	protected boolean isPropertyHasRead(String property) {
		return (propertiesHasRead == null) ? false : propertiesHasRead.containsKey(property);
	}
	...
	private Object readReference(Object entity, Reference referenceProperty, Object originResult) throws Throwable {
		if (!referenceProperty.shouldIntercept() || isGetterInterceptionDisabled()) {
			...
			try {
				...
				if (pageSize > 0) {
					return new PagingList(dataProvider, dataType, parameter, pageSize);
				}
				else {
	...
	public abstract Object readProperty(Object entity, String property, boolean ignoreInterceptors) throws Throwable;
