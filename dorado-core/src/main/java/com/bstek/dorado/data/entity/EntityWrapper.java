...
public class EntityWrapper {
	...
	private void throwNotValidEntity() {
		throw new IllegalArgumentException("Wrapped object is not a valid dorado entity.");
	}
	...
	protected <T> T get(String property, boolean ignoreInterceptors) {
		...
		try {
			if (entityEnhancer != null) {
				result = entityEnhancer.readProperty(entity, property, ignoreInterceptors);
			}
			else {
			...
			if (entityEnhancer != null) {
				for (Map.Entry entry : (Set<Map.Entry>) properties.entrySet()) {
					...
					if (key instanceof String) {
						entityEnhancer.writeProperty(entity, (String) key, entry.getValue());
					}
				}
			}
			else {
	...
	/**
	 * 返回数据实体中某属性的原始值。<br>
	 * 此方法一般仅对状态{@link com.bstek.dorado.data.entity.EntityState#MODIFIED} 的数据实体有效，
	 * 即返回该属性在被修改之前的值。如果该属性并未被修改，那么此方法的返回值应该就是属性当前值。
	 * @param property 属性名。
	 * @return 原始值
	 */
	public <T> T getOldValue(String property) {
		...
		if (entityEnhancer != null) {
			...
			if (oldValues != null) {
				...
				throw new IllegalStateException("Entity does not have OldValue.");
			}
		}
		return (T) value;
	}
