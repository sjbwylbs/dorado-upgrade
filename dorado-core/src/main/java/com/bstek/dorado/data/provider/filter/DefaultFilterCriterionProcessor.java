...
public class DefaultFilterCriterionProcessor implements FilterCriterionProcessor {
	...
	private static final FilterOperator[] OPERATORS = new FilterOperator[] { FilterOperator.like,
			FilterOperator.likeStart, FilterOperator.likeEnd, FilterOperator.eq, FilterOperator.gt, FilterOperator.ge,
			FilterOperator.lt, FilterOperator.le, FilterOperator.ne };

	@Override
	public void doProcess(SingleValueFilterCriterion criterion) throws Exception {
		...
		if (filterOperator == null) {
			...
			if ((dataType == null || !(dataType instanceof NumberDataType || dataType instanceof BooleanDataType
					|| dataType instanceof DateDataType)) && len > 1) {
				char firstChar = expression.charAt(0), lastChar = expression.charAt(len - 1);
		...
		Object value = (dataType != null) ? dataType.fromText(expression) : expression;
