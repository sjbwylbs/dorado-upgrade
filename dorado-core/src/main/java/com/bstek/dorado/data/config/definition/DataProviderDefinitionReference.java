...
public class DataProviderDefinitionReference implements DefinitionReference<DataProviderDefinition> {

	private static Log logger = LogFactory.getLog(DataProviderDefinitionReference.class);
	...
	public DataProviderDefinition getDefinition() {
		Context current = Context.getCurrent();
		DataProviderDefinitionManager dtfm = (DataProviderDefinitionManager) current.getAttribute(Context.THREAD,
		...
		if (definition == null) {
			throw new IllegalArgumentException("Unrecognized DataProvider \"" + name + "\".");
		}
		return definition;
	}
