...
public abstract class BeanEntityEnhancer extends EntityEnhancer {
	...
	private static Set<Class<?>> cachedTypes = Collections.synchronizedSet(new HashSet<Class<?>>());

	private static Map<Class<?>, Map<String, Boolean>> propertiesCache = new Hashtable<>();

	private static Map<Class<?>, Map<Method, String>> readMethodsCache = new Hashtable<>();

	private static Map<Class<?>, Map<Method, String>> writeMethodsCache = new Hashtable<>();
	...
	public BeanEntityEnhancer(EntityDataType dataType, Class<?> beanType) throws Exception {
		super(dataType);
	...
	protected synchronized void buildReflectionCahce() throws Exception {
		synchronized (cachedTypes) {
			if (!cachedTypes.contains(beanType)) {
				...
				Map<String, Boolean> properties = new Hashtable<>();
				Map<Method, String> readMethods = new Hashtable<>();
				Map<Method, String> writeMethods = new Hashtable<>();
				...
				for (PropertyDescriptor propertyDescriptor : propertyDescriptors) {
					...
					if (writeMethod != null) {
						if (writeMethod.getDeclaringClass() != beanType) {
							writeMethod = beanType.getMethod(writeMethod.getName(), writeMethod.getParameterTypes());
						}
	...
	@Override
	protected Set<String> doGetPropertySet(Object entity, boolean excludeExProperties) {
		Map<String, Object> exProperties = getExProperties(false);
		if (excludeExProperties || exProperties == null || exProperties.isEmpty()) {
			return properties.keySet();
		}
		else {
			Set<String> propertySet = new HashSet<>(properties.keySet());
	...
	protected Object tryConvertValue(Class<?> propertyType, Object value) {
		if (java.util.Date.class.isAssignableFrom(propertyType) && value instanceof java.util.Date) {
	...
	@Override
	public Object readProperty(Object entity, String property, boolean ignoreInterceptors) throws Throwable {
