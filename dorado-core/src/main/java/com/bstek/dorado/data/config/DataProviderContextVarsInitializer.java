...
public class DataProviderContextVarsInitializer implements ContextVarsInitializer {

	private static DataProviderManager dataProviderManager;

	private static DataProviderManager getDataProviderManager() throws Exception {
		if (dataProviderManager == null) {
			dataProviderManager = (DataProviderManager) Context.getCurrent().getServiceBean("dataProviderManager");
		}
	...
	public void initializeContext(Map<String, Object> vars) throws Exception {
		Method method = getClass().getDeclaredMethod("getDataProvider", new Class[] { String.class });
