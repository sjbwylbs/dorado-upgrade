...
public class DataTypeDefinition extends ListenableObjectDefinition implements Namable {
	...
	protected Object doCreate(CreationContext context, Object[] constructorArgs) throws Exception {
		...
		if (cachable == Boolean.TRUE || (cachable != Boolean.FALSE
				&& !(dataType instanceof EntityDataType || dataType instanceof AggregationDataType))) {
	...
	protected void injectResourceString(EntityDataType dataType, String key, String resourceString) throws Exception {
		...
		for (int i = 0; i < len; i++) {
			...
			if (isObject) {
				...
				object = MethodUtils.invokeExactMethod(object, methodName, (Object) new String[] { section });
				if (object == null) {
					break;
				}
	...
	protected void initCreationInfo(CreationInfo creationInfo, ObjectDefinition definition, boolean processConstrInfos)
		...
		if (allPropertyDefs == null) {
			allPropertyDefs = new LinkedHashMap<>();
	...
	protected void setObjectProperty(Object object, String property, Object value, CreationContext context)
			throws Exception {
		if (DataXmlConstants.ATTRIBUTE_ELEMENT_DATA_TYPE.equals(property) && object instanceof EntityDataType) {
			return;
		}
