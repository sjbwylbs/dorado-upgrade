...
public class ListenableObjectDefinition extends GenericObjectDefinition {
	...
	@Override
	protected void initCreationInfo(CreationInfo creationInfo, ObjectDefinition definition, boolean processConstrInfos)
			throws Exception {
		...
		creationInfo.setUserData("listener", dataObjectDefinition.getListener());
	}

	@Override
	protected final void initObject(Object object, CreationInfo creationInfo, CreationContext context)
			throws Exception {
	...
	protected void preinitObject(Object object, CreationInfo creationInfo, CreationContext context) throws Exception {
	...
	protected void doInitObject(Object object, CreationInfo creationInfo, CreationContext context) throws Exception {
		super.initObject(object, creationInfo, context);
	}

	protected boolean filterGlobalListener(GenericObjectListener<?> listener, Object object) {
		boolean accept = true;
		...
		if (accept && StringUtils.isNotEmpty(pattern)) {
			if (object instanceof Namable) {
				...
				if (accept && StringUtils.isNotEmpty(excludePattern)) {
					accept = !PathUtils.match(excludePattern, ((Namable) object).getName());
				}
			}
			else if (object instanceof Identifiable) {
				accept = PathUtils.match(pattern, ((Identifiable) object).getId());
				String excludePattern = listener.getExcludePattern();
				if (accept && StringUtils.isNotEmpty(excludePattern)) {
					accept = !PathUtils.match(excludePattern, ((Identifiable) object).getId());
				}
			}
		}
		return accept;
	...
	private boolean invokeBeforeInitListener(Object object, String listeners, CreationContext context)
			throws Exception {
		boolean retval = true;
		// notify global listeners
		for (GenericObjectListener listener : GenericObjectListenerRegistry.getListeners()) {
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void invokeOnInitListener(Object object, String listeners, CreationContext context) throws Exception {
		// notify global listeners
		for (GenericObjectListener listener : GenericObjectListenerRegistry.getListeners()) {
			if (filterGlobalListener(listener, object)) {
	...
	protected boolean invokePrivateListeners(Object object, String listeners, CreationContext context,
		...
		if (StringUtils.isNotEmpty(listeners)) {
			for (String listenerExpression : StringUtils.split(listeners, ",")) {
				...
				if (StringUtils.isEmpty(methodName)) {
					throw new IllegalArgumentException("The methodName of \"" + listenerName + "\" is undefined.");
				}

				boolean isNameStartsWithBefore = methodName.startsWith("before");
	...
	protected boolean invokePrivateListener(Object object, String listenerName, String methodName,
		...
		Object retval = MethodAutoMatchingUtils.invokeMethod(methods, interceptor, requiredTypes, requiredArgs,
				exactTypes, exactArgs, optionalTypes, optionalArgs, returnType);
