 ...
 * 将这个匹配关系保存到用于缓存的Map当中。以后，当系统再次发出相同Java数据类型的匹配请求时， DataTypeManagerSupport将直接从缓存中提取匹配结果。
 * </p>
 * <p>
...
public abstract class DataTypeManagerSupport implements DataTypeManager {
	 ...
	 */
	private Map<Type, DataTypeDefinition> cachedTypesMap = new Hashtable<>();
	 ...
	 * @param dataTypeDefinition DataType的配置声明对象
	 * @return DataType对象
	 */
	...
	@Override
	public DataType createDataType(String name, String parents) throws Exception {
		return doCreateDataType(name, parents);
	}

	protected DataType doCreateDataType(String name, String parents) throws Exception {
		if (getDataType(name) != null) {
			throw new IllegalArgumentException("DataType [" + name + "] already exists.");
		}

		if (StringUtils.isEmpty(parents)) {
			throw new IllegalArgumentException("Argument \"parents\" undefined.");
		}
		...
		for (String parent : parentNames) {
			DataTypeDefinition parentDefinition = dataTypeDefinitionManager.getDefinition(parent);
	 ...
	 * 此功能已由DataTypeManagerSupport实现。 因此在此方法只需要考虑那些不能直接通过DataTypeDefinitionManager完成匹配的情况。
	 * </p>
	 * @param type 给定的Class类型
	 * @return 匹配的DataType
	 * @throws Exception
	 */
	protected abstract DataTypeDefinition getDataTypeDefinition(Type type) throws Exception;
