...
public class DirectDataProviderDefinition extends DataProviderDefinition {
	...
	protected BeanWrapper createObject(CreationInfo creationInfo, Object[] constructorArgs,
		...
		Object dataType = creationInfo.getProperties().get(DataXmlConstants.ATTRIBUTE_DATA_TYPE);
		if (dataType instanceof DataTypeDefinition) {
			dataType = new DirectDefinitionReference<>((DataTypeDefinition) dataType);
		}
		createContext.setCurrentDataTypeDefinition((DefinitionReference<DataTypeDefinition>) dataType);
		try {
			return super.createObject(creationInfo, constructorArgs, methodInterceptors, context);
		}
		finally {
