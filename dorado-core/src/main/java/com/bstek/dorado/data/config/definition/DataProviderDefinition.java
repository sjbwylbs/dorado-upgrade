 ...
 */
public class DataProviderDefinition extends InterceptableDefinition
		implements DefinitionManagerAware<DataProviderDefinitionManager> {
	...
	@Override
	public void setDefinitionManager(DataProviderDefinitionManager definitionManager) {
	...
	protected MethodInterceptor[] getMethodInterceptors(CreationInfo creationInfo, CreationContext context)
		...
		if (customMethodInterceptors != null) {
			interceptors = new MethodInterceptor[] { getResultMethodInterceptor,
					new RootCustomDataProviderInterceptor(customMethodInterceptors) };
		}
		else {
			interceptors = new MethodInterceptor[] { getResultMethodInterceptor };
		}

		MethodInterceptor[] superInterceptors = super.getMethodInterceptors(creationInfo, context);
		if (superInterceptors != null) {
			MethodInterceptor[] oldInterceptors = interceptors;
			interceptors = new MethodInterceptor[oldInterceptors.length + superInterceptors.length];
			System.arraycopy(oldInterceptors, 0, interceptors, 0, oldInterceptors.length);
			System.arraycopy(superInterceptors, 0, interceptors, oldInterceptors.length, superInterceptors.length);
		}
		return interceptors;
	}
...
class RootCustomDataProviderInterceptor extends BaseMethodInterceptorDispatcher {
	...
	public RootCustomDataProviderInterceptor(MethodInterceptor[] subMethodInterceptors) {
	...
	protected boolean filterMethod(Method method) {
		...
		return methodName.equals(METHOD_NAME) || methodName.equals(PAGING_METHOD_NAME);
	}
