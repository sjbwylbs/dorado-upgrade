 ...
 *
 */
public class DataProviderDefinitionManager extends DefaultDefinitionManager<DataProviderDefinition> {
	...
	public DataProviderDefinitionManager(DefinitionManager<DataProviderDefinition> parent) {
		super(parent);
	}

	@Override
	public synchronized void registerDefinition(String name, DataProviderDefinition definition) {
		...
		if (super.getDefinition(name) != null) {
			throw new IllegalStateException("The DataProvider [" + name + "] is already registered!");
		}
		super.registerDefinition(name, definition);
	}

	public void addDataProviderMethodInterceptor(MethodInterceptor methodInterceptor) {
	...
	public MethodInterceptor[] getDataProviderMethodInterceptors() {
		...
		if (!dataProviderMethodInterceptors.isEmpty()) {
			methodInterceptors = dataProviderMethodInterceptors.toArray(new MethodInterceptor[0]);
		}
