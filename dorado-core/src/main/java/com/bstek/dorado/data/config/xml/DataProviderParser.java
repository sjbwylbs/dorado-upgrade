...
public class DataProviderParser extends GenericObjectParser {
	...
	protected DefinitionReference<DataProviderDefinition>[] getParentDefinitionReferences(String parentNameText,
			ParseContext context) throws Exception {
		DefinitionReference<DataProviderDefinition> parentReference = ((DataParseContext) context)
			.getDataProviderReference(parentNameText, (DataParseContext) context);
	...
	protected void initDefinition(ObjectDefinition definition, Element element, ParseContext context) throws Exception {
		...
		String interceptor = (String) dataProvider.removeProperty(XmlConstants.ATTRIBUTE_INTERCEPTOR);
		...
		DefinitionReference<DataTypeDefinition> dataTypeRef = dataObjectParseHelper
			.getReferencedDataType(DataXmlConstants.ATTRIBUTE_RESULT_DATA_TYPE, element, (DataParseContext) context);
		if (dataTypeRef != null) {
			definition.setProperty(DataXmlConstants.ATTRIBUTE_RESULT_DATA_TYPE, dataTypeRef);
		}
	}
