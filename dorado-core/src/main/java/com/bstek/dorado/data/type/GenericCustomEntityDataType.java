...
public abstract class GenericCustomEntityDataType<T> extends EntityDataTypeSupport implements CustomEntityDataType<T> {
	...
	public Set<String> getPropertySet() {
		if (!isAcceptUnknownProperty()) {
			...
			throw new UnsupportedOperationException("getPropertySet() not implemented for this case.");
		}
	}
	...
	protected T createDataObject(Map<String, Object> map) throws Exception {
		...
		return (T) creationType.getDeclaredConstructor().newInstance();
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	protected void writePropertyWithRudeType(T customEntity, String property, Object value) throws Exception {
			...
			if (dataType != null) {
				if (dataType instanceof CustomEntityDataType) {
					if (value instanceof Map) {
						value = ((CustomEntityDataType) dataType).fromMap((Map<String, Object>) value);
					}
				}
					...
					if (elementDataType != null && elementDataType instanceof CustomEntityDataType) {
						...
						for (Object element : (Collection) value) {
							if (element instanceof Map) {
								newCollection.add(customEntityDataType.fromMap((Map<String, Object>) element));
							}
							else {
