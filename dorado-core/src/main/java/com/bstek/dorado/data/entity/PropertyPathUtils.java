...
public abstract class PropertyPathUtils {
	...
	public static Object getValueByPath(EntityDataType dataType, Object object, String propertyPath) throws Exception {
			...
			if (tempEntity == null) {
				...
				if (tempEntity == null) {
					...
					if (subEntityType.isAssignableFrom(Map.class)) {
						...
						tempEntity = subEntityType.getDeclaredConstructor().newInstance();
					}
					currentDataType = null;
				}
	...
	public static DataType getPropertyDataTypeByPath(EntityDataType dataType, String propertyPath) throws Exception {
		String[] paths = StringUtils.split(propertyPath, '.');
		DataType currentDataType = dataType;
		for (String path : paths) {
			if (currentDataType instanceof EntityDataType) {
				PropertyDef propertyDef = ((EntityDataType) currentDataType).getPropertyDef(path);
				if (propertyDef != null) {
