...
public class DataTypeDefinitionManager extends DefaultDefinitionManager<DataTypeDefinition> {
	...
	/**
	 * 以DataType的相关Java类型作为键值的DataType集合， 即以DataType的getMatchType()的返回值作为键值的DataType集合。
	 */
	private Map<Type, DataTypeDefinition> typeMap = new Hashtable<>();

	private Map<String, Type> nameTypeMap = new HashMap<>();
	...
	private boolean isConflicting(DataTypeDefinition dataType, DataTypeDefinition conflictingDataType) {
		...
		if (parents != null) {
			for (Definition parent : parents) {
				if (parent == conflictingDataType) {
					...
					conflicting = isConflicting((DataTypeDefinition) parent, conflictingDataType);
				}
				if (!conflicting) {
					break;
				}
	...
	public synchronized void registerDefinition(String name, DataTypeDefinition definition) {
		...
		if (registered != null && registered.getId().equals(definition.getId())) {
			throw new IllegalStateException("The DataType [" + name + "] is already registered!");
		}
	...
	public synchronized DataTypeDefinition getDefinition(String name) {
		...
		if (definition == null) {
			...
			if (dataTypeName.hasSubDataType()) {
				...
				Definition parentDataType = getDefinition(dataTypeName.getDataType());
				if (parentDataType != null) {
					String[] subDataTypes = dataTypeName.getSubDataTypes();
					for (String subDataType : subDataTypes) {
						if (getDefinition(subDataType) == null) {
	...
	protected DataTypeDefinition createAggregationDataType(DataTypeName dataTypeName) {
		DataTypeDefinition definition = new DataTypeDefinition();
		String name = dataTypeName.getFullName();
		definition.setName(name);

		DefinitionReference<?> dataTypeRef = new DataTypeDefinitionReference(dataTypeName.getDataType());
		definition.setParentReferences(new DefinitionReference<?>[] { dataTypeRef });
		...
		if (subDataTypeNames.length == 1) {
			...
			DefinitionReference<?> elementDataType = new DataTypeDefinitionReference(subDataTypeNames[0]);
			definition.setProperty(DataXmlConstants.ATTRIBUTE_ELEMENT_DATA_TYPE, elementDataType);
		}
		else {
	...
	public void registerMatchType(DataTypeDefinition dataTypeDefinition) throws Exception {
		...
		if (matchType != null) {
			...
			if (conflictingDataType != null) {
				boolean conflicting = isConflicting(dataTypeDefinition, conflictingDataType);
				if (conflicting) {
					throw new IllegalArgumentException("DataType \"" + dataTypeDefinition.getName()
							+ "\"'s relavantType is conflict with DataType \"" + conflictingDataType.getName() + "\"!");
				}
			}
		}
