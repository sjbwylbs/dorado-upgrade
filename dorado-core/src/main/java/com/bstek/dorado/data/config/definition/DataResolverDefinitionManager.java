...
public class DataResolverDefinitionManager extends DefaultDefinitionManager<DataResolverDefinition> {
	...
	public DataResolverDefinitionManager(DefinitionManager<DataResolverDefinition> parent) {
		super(parent);
	}

	@Override
	public synchronized void registerDefinition(String name, DataResolverDefinition definition) {
		...
		if (super.getDefinition(name) != null) {
			throw new IllegalStateException("The DataResolver [" + name + "] is already registered!");
		}
		super.registerDefinition(name, definition);
	}

	public void addDataResolverMethodInterceptor(MethodInterceptor methodInterceptor) {
	...
	public MethodInterceptor[] getDataResolverMethodInterceptors() {
		...
		if (!dataResolverMethodInterceptors.isEmpty()) {
			methodInterceptors = dataResolverMethodInterceptors.toArray(new MethodInterceptor[0]);
		}
		...
		if (parent != null && parent instanceof DataResolverDefinitionManager) {
			...
			if (parentMethodInterceptors != null) {
				if (methodInterceptors != null) {
					...
					System.arraycopy(parentMethodInterceptors, 0, finalMethodInterceptors, 0,
							parentMethodInterceptors.length);
					System.arraycopy(methodInterceptors, 0, finalMethodInterceptors, parentMethodInterceptors.length,
							methodInterceptors.length);
