...
public abstract class ReloadableDataConfigManagerSupport extends DataConfigManagerSupport {
	...
	private static Log logger = LogFactory.getLog(ReloadableDataConfigManagerSupport.class);
	...
	/**
	 * 将当前已装载的配置文件资源描述对象包装成可支持资源重装载的资源描述对象并返回。 如果某个资源本身已是可支持资源重装载的资源描述对象，那么此函数会跳过对这个对象的处理。
	 */
	protected Set<RefreshableResource> getRefreshableResources() throws IOException {
		Set<Resource> resources = getResources();
		synchronized (resources) {
			Set<RefreshableResource> refreshableResources = new LinkedHashSet<>(resources.size());
			for (Resource resource : resources) {
				...
				if (refreshableResource.getTimestamp() != 0) {
					refreshableResource.setMinValidateSeconds(minResourceValidateSeconds);
	...
	public synchronized boolean validateAndReloadConfigs() {
		...
		if (refreshableResources != null) {
			for (RefreshableResource refreshableResource : refreshableResources) {
				...
				try {
					if (resourceToUnload != null) {
						Resource[] resource = new Resource[resourceToUnload.size()];
				...
				try {
					if (resourceToReload != null) {
						Resource[] resource = new Resource[resourceToReload.size()];
