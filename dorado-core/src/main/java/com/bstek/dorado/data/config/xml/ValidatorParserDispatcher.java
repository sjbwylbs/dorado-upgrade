...
public class ValidatorParserDispatcher extends GenericParser {
	...
	protected Object doParse(Node node, ParseContext context) throws Exception {
		...
		String type = element.getAttribute(DataXmlConstants.ATTRIBUTE_VALIDATOR_TYPE);
		ValidatorTypeRegisterInfo registryInfo = validatorTypeRegistry.getTypeRegisterInfo(type);
		if (registryInfo != null) {
			parser = xmlParserHelper.getXmlParser(registryInfo.getClassType());
		}
		else {
			throw new XmlParseException("Unrecognized Validator type[" + type + "].", element, context);
		}
		if (parser == null) {
			throw new XmlParseException("Can not get Parser for Validator of [" + type + "] type.", element, context);
		}
