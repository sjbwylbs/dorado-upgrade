...
public class EntityExpressionMethodInterceptor extends ExpressionMethodInterceptor {
	...
	/**
	 * @param expressionProperties EL表达式的集合。其中Map的键值为相应的对象属性名，值为EL表达式。
	 */
	public EntityExpressionMethodInterceptor(Map<String, Expression> expressionProperties) {
	...
	protected void discoverInterceptingMethods(Class<?> clazz) throws Exception {
		...
		interceptingReadMethods = new HashMap<>();
		interceptingWriteMethods = new HashMap<>();
		...
		for (PropertyDescriptor propertyDescriptor : propertyDescriptors) {
			...
			if (expressionProperties.containsKey(property)) {
				DataType dataType = dataTypeManager.getDataType(propertyDescriptor.getPropertyType());
				...
				if (readMethod != null) {
					...
					getterMethods.put(readMethod, new ReadMethodDescriptor(property, dataType));
				}
				if (writeMethod != null) {
