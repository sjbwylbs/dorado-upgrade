...
class IdeObjectTest {
	...
	void should_have_runtime_retention_when_inspected() {
		Retention retention = IdeObject.class.getAnnotation(Retention.class);
	...
	void should_target_type_when_inspected() {
		Target target = IdeObject.class.getAnnotation(Target.class);
	...
	void should_be_inherited_when_inspected() {
		Inherited inherited = IdeObject.class.getAnnotation(Inherited.class);
		assertThat(inherited).isNotNull();
	}

	@Test
	void should_have_true_as_default_visible() throws Exception {
		Method method = IdeObject.class.getDeclaredMethod("visible");
		assertThat(method.getDefaultValue()).isEqualTo(true);
	}

	@Test
	void should_have_empty_string_as_default_labelProperty() throws Exception {
		Method method = IdeObject.class.getDeclaredMethod("labelProperty");
